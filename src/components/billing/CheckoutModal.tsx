...
/** Toss v2 Standard (`js.tosspayments.com/v2/standard`) — loaded dynamically; not on `Window` typings */
type TossPaymentsV2 = (clientKey: string) => {
  payment: (args: { customerKey: string }) => {
...
export default function CheckoutModal({
  ...
  async function handleTossPayment(method: 'CARD' | 'TOSSPAY' | 'TRANSFER' = 'CARD') {
    ...
    try {
      ...
      const tossFn = (window as unknown as { TossPayments?: TossPaymentsV2 }).TossPayments;
      if (!tossFn) throw new Error('TossPayments SDK not available');
      const tossPayments = tossFn(process.env.NEXT_PUBLIC_TOSS_CLIENT_KEY ?? '');
