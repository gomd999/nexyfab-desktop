...
const dict = {
    ko: { pricing: '요금', factories: '공장 찾기', login: '로그인', dashboard: '대시보드', quickQuote: '빠른 견적', shapeGen: '3D 모델링' },
    en: { pricing: 'Pricing', factories: 'Find Factories', login: 'Sign In', dashboard: 'Dashboard', quickQuote: 'Quick Quote', shapeGen: '3D Modeler' },
    ja: { pricing: '料金', factories: '工場を探す', login: 'ログイン', dashboard: 'ダッシュボード', quickQuote: 'クイック見積もり', shapeGen: '3Dモデリング' },
    cn: { pricing: '价格', factories: '找工厂', login: '登录', dashboard: '控制台', quickQuote: '快速报价', shapeGen: '3D建模' },
    es: { pricing: 'Precios', factories: 'Fábricas', login: 'Iniciar Sesión', dashboard: 'Panel', quickQuote: 'Cotización Rápida', shapeGen: 'Modelado 3D' },
    ar: { pricing: 'الأسعار', factories: 'ابحث عن مصنع', login: 'تسجيل الدخول', dashboard: 'لوحة التحكم', quickQuote: 'عرض سعر سريع', shapeGen: 'نمذجة ثلاثية الأبعاد' },
};
...
export default function Header() {
    ...
    useEffect(() => {
        const load = () => {
            ...
            try { setCurrentUser(stored ? JSON.parse(stored) : null); } catch { setCurrentUser(null); }
        };
        load();
        ...
        <>
            <header dir={isRtl ? 'rtl' : undefined} role="banner" style={{
                ...
                <div style={{
                    ...
                    {/* Logo */}
                    <Link href={`/${lang}`} style={{ textDecoration: 'none', fontSize: '22px', fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1, flexShrink: 0 }}>
                            ...
                            {currentUser ? (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    >
                                        ...
                                        {isRtl ? 'خروج' : lang === 'ja' ? 'ログアウト' : lang === 'cn' ? '退出' : lang === 'es' ? 'Salir' : '로그아웃'}
                                    </button>
                                </div>
                            ) : (
