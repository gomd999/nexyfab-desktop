...
export default function NexyfabNav({ lang }: NexyfabNavProps) {
  ...
  return (
    <>
      <style precedence="default" href="nexyfab-nav">{`
        .nf-nav {
          width: 200px;
          min-width: 200px;
        }
      ...
      >
        ...
        <div style={{ flex: 1, padding: '8px 0' }}>
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link
                prefetch={item.href === '/shape-generator'}
                key={item.href}
                href={href}
                style={{
                  display: 'flex',
              ...
              >
                <span style={{ fontSize: 14, flexShrink: 0 }}>{item.icon}</span>
                <span className="nf-nav-label" style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
                  {label}
                  {item.planBadge === 'team' && user?.plan !== 'pro' && user?.plan !== 'enterprise' && (
                    <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-1 rounded ml-1">PRO</span>
                  )}
                </span>
