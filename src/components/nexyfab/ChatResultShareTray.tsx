...
export const RESULT_SHARE_COPY: Record<ShareLang, ShareCopy> = {
  kr: {
    ...
    package: '도면·검토서·CAD 원본을 묶은 전체 패키지', source: '수정 가능한 OpenSCAD 원본',
  ...
  en: {
    ...
    package: 'Complete package with drawings, reviews, and CAD sources', source: 'Editable OpenSCAD source',
  ...
  ja: {
    ...
    package: '図面・検討書・CADソースをまとめた全体パッケージ', source: '編集可能なOpenSCADソース',
  ...
  cn: {
    ...
    package: '包含图纸、审查报告和CAD源文件的完整包', source: '可编辑的OpenSCAD源文件',
  ...
  es: {
    ...
    package: 'Paquete completo con planos, revisiones y fuentes CAD', source: 'Fuente OpenSCAD editable',
  ...
  ar: {
    ...
    package: 'حزمة كاملة تضم الرسومات والمراجعات ومصادر CAD', source: 'مصدر OpenSCAD قابل للتعديل',
...
async function createArtifactFile(kind: ArtifactKind, cad: ChatShareCadResult, lang: ShareLang): Promise<File> {
  if (kind === 'source') {
    if (!cad.scad) throw new Error('source_unavailable');
    return new File([cad.scad], 'model.scad', { type: 'text/plain' });
  }
  ...
  if (kind === 'ga') {
    ...
    return new File([data.html], 'GA_3D.html', { type: 'text/html' });
  }
  ...
  if (kind === 'step') {
    ...
    return new File([data.step], 'model.step', { type: 'application/step' });
  }
  ...
  if (typeof data.zipBase64 === 'string') {
    ...
    return new File([bytes.buffer], `design_package_${lang}.zip`, { type: 'application/zip' });
  }
  if (Array.isArray(data.files)) {
    ...
    return new File([blob], `design_package_${lang}.zip`, { type: 'application/zip' });
  }
...
export function ChatResultShareTray({ cad, langCode, accent }: { cad: ChatShareCadResult | null; langCode: string; accent: string }) {
  ...
  const cacheRef = useRef(new Map<ArtifactKind, Promise<File>>());
  ...
  const getFile = (kind: ArtifactKind): Promise<File> => {
          ...
          return (
            <div key={row.kind} data-testid={`share-artifact-${row.kind}`} className="nf-share-row" style={{
              ...
              <div className="nf-share-detail" style={{ minWidth: 0 }}>
                ...
                {row.kind === 'ga' && <span style={{ marginInlineStart: 6, padding: '1px 5px', borderRadius: 999, background: `${accent}30`, color: '#bfdbfe', fontSize: 8.5, fontWeight: 900 }}>DEFAULT</span>}
              </div>
              <div style={{ minWidth: 0 }}>
                ...
                <div role="status" style={{ color: notice?.kind === row.kind ? (notice.error ? '#fca5a5' : '#86efac') : (row.enabled ? '#64748b' : '#6b7280'), fontSize: 9.5 }}>
                  {isBusy ? copy.working : notice?.kind === row.kind ? notice.text : state}
                </div>
              </div>
