    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{
          ...
          {[
            { label: lang === 'ko' ? '내 프로젝트' : 'My Projects', icon: '📁', href: `/${lang}/nexyfab/dashboard` },
            { label: lang === 'ko' ? 'Pro 업그레이드' : 'Upgrade to Pro', icon: '⚡', href: `/${lang}/pricing` },
          ].map(item => (
          ...
          {[
            { label: lang === 'ko' ? '견적 요청' : 'RFQ', icon: '💬', href: `/${lang}/nexyfab/rfq` },
            { label: lang === 'ko' ? '주문 추적' : 'Orders', icon: '📦', href: `/${lang}/nexyfab/orders` },
            { label: lang === 'ko' ? '마켓플레이스' : 'Marketplace', icon: '🏭', href: `/${lang}/nexyfab/marketplace` },
          ].map(item => (
            <a key={item.label} href={item.href} style={{
              display: 'flex', alignItems: 'center', gap: 8,
              padding: '9px 14px', color: '#c9d1d9', fontSize: 12,
