...
const FREE_MONTHLY_LIMITS: Record<FreemiumFeature, number> = {
  ...
  supplier_match:     -1,
  rfq_bundle:         -1,
  dfm_autofix:        3,   // 3 free auto-fixes / month; Pro = unlimited
  collaboration_edit: -1,  // Free users are read-only in collab sessions
  export_optimize:    -1,  // Pro-only export optimization (mesh pre-clean, LOD)
  dfm_insights:       5,   // 5 free AI DFM explanations + cost deltas / month
  process_router:     5,   // 5 free AI process-routing queries / month
  ai_supplier_match:  3,   // 3 free AI supplier Top-3 matches / month
  cost_copilot:       5,   // 5 free Design-for-Cost Copilot queries / month
};
...
export function useFreemium() {
  ...
  const stage = (user?.nexyfabStage ?? 'A') as Stage;
  ...
  function check(feature: FreemiumFeature): FreemiumCheckResult {
    ...
    // Hard gate: Pro required (bm-matrix §1.2 #17: Stage D+ 협업 편집 해금 시 예외)
    if (limit === -1) {
      if (feature === 'collaboration_edit' && userMeetsBmMatrixFeatureStage(stage, 17)) {
        return { allowed: true, requiresPro: false, overLimit: false, used: 0, limit: -1 };
      }
  ...
  function getRemainingCount(feature: FreemiumFeature): number {
    ...
    const limit = FREE_MONTHLY_LIMITS[feature];
    if (limit === -1) {
      if (feature === 'collaboration_edit' && userMeetsBmMatrixFeatureStage(stage, 17)) return Infinity;
      return 0;
    }
