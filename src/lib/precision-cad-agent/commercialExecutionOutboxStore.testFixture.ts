...
export async function makeEnqueueFixture(suffix = ''): Promise<EnqueueFixture> {
  if (suffix && !/^-[a-z0-9-]{1,32}$/.test(suffix)) throw new Error('enqueue_fixture_suffix_invalid');
  const scoped = (value: string) => `${value}${suffix}`;
  const tenantId = scoped('tenant-1');
  const projectId = scoped('project-1');
  const workspaceId = scoped('workspace-1');
  const runId = scoped('run-1');
  const jobId = scoped('job-1');
  const approvalBinding: ApprovalBindingInput = { actorId: scoped('owner-1'), role: 'owner', projectId, workspaceId, workspaceRevision: 7, workspaceContentHash: 'a'.repeat(64), tool: 'apply', scope: 'apply', callId: scoped('call-1'), arguments: { feature: 'fillet', radiusMm: 2 } };
  ...
  const command = { domain: 'precision-cad-agent', operation: approvalBinding.tool, arguments: { toolArguments: approvalBinding.arguments, tenantId, projectId: approvalBinding.projectId, workspaceId: approvalBinding.workspaceId, workspaceRevision: approvalBinding.workspaceRevision, modelContentHash: approvalBinding.workspaceContentHash, generationRunId: runId, generationStateRevision: 1, generationProgramSha256, targetSha256: targetHash } };
  const nowIso = new Date(NOW).toISOString(); const engine = new DurableExecutionJournal(undefined, () => nowIso); const planned = engine.plan({ idempotencyKey: scoped('idem-1'), command, workspace: { workspaceId: approvalBinding.workspaceId, projectId: approvalBinding.projectId, revision: approvalBinding.workspaceRevision, contentHash: approvalBinding.workspaceContentHash } }); if (!planned.ok) throw new Error('journal_plan_failed'); const approved = engine.approve(planned.receipt.executionId, { approvalId: `boundary:${challenge.challengeId}`, actorId: approval.actorId, approved: true, approvedAt: nowIso, commandHash: planned.receipt.commandHash, workspaceBindingHash: planned.receipt.workspaceBindingHash, userInitiated: true }); if (!approved.ok || !approved.receipt.approvalHash) throw new Error('journal_approval_failed');
  const job: CommercialExecutionJob = { contractVersion: 'nexyfab.precision-cad-commercial-execution.v3', jobId, tenantId, projectId, executionId: approved.receipt.executionId, generationRunId: runId, generationStateRevision: 1, generationProgramSha256, workspaceId, workspaceRevision: 7, workspaceContentHash: approvalBinding.workspaceContentHash, tool: approvalBinding.tool, scope: approvalBinding.scope, callId: approvalBinding.callId, argumentsHash, commandHash: approved.receipt.commandHash, targetHash, journalVersion: approved.receipt.version, attempt: 1, leaseGeneration: 1, inputArtifact: { artifactId: scoped('input-1'), objectKey: `private/commercial-precision-inputs/${tenantId}/${projectId}/${jobId}/input.json`, contentSha256: 'e'.repeat(64), byteLength: 256, mediaType: 'application/json' } };
