...
class OutboxDb implements DbAdapter {
  ...
  async queryOne<T>(sql: string, ...params: unknown[]): Promise<T | undefined> { if (sql.includes('nf_schema_migrations')) return { version: Number(params[0]), checksum: 'f'.repeat(64) } as T; if (sql.includes('nf_precision_cad_commercial_input_artifacts')) return this.inputRow as T | undefined; return this.row as T | undefined; }
