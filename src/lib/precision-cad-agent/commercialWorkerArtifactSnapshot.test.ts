...
function fixture(): CommercialWorkerReceipt {
  ...
  const receipt = { schema: 'nexyfab.precision-cad-commercial-execution.v3' as const, tenantId: 'tenant-1', projectId: 'project-1', executionId: 'exec-1', generationRunId: 'run-1', generationStateRevision: 11, generationProgramSha256: '8'.repeat(64), workspaceId: 'workspace-1', workspaceRevision: 7, workspaceContentHash: 'a'.repeat(64), journalVersion: 3, leaseGeneration: 1, leaseCapabilityHash: '9'.repeat(64), attempt: 1, jobId: 'job-1', commandHash: 'c'.repeat(64), targetHash: 'd'.repeat(64), inputArtifactSha256: 'e'.repeat(64), workerIdentity: 'worker-1', workerPublicKeyFingerprint: fingerprint, nativeExecutableSha256: '3'.repeat(64), nativeInvocationSha256: '4'.repeat(64), status: 'PASS' as const, startedAt: '2026-08-22T00:00:00.000Z', completedAt: '2026-08-22T00:01:00.000Z', outputArtifacts: values.map(([role, value], index) => { const body = bytes(value); return { artifactId: `${role}-1`, role: role as 'model' | 'report' | 'verification', contentSha256: digest(body), byteLength: body.byteLength, objectKey: `source/${role}-${index}` }; }), failureReasons: [], signatureBase64: '' };
...
describe('commercial worker artifact snapshot', () => {
  it('hashes, copies, and authoritatively rereads all exact outputs', async () => {
    ...
    const result = await snapshotCommercialWorkerArtifacts({ store, receipt, expected: { ...receipt }, trustedWorkers: { 'worker-1': { workerIdentity: 'worker-1', publicKeyPem, fingerprintSha256: fingerprint, nativeExecutableSha256: receipt.nativeExecutableSha256!, nativeInvocationSha256: receipt.nativeInvocationSha256! } }, metadata: receipt.outputArtifacts });
    ...
    const replay = await snapshotCommercialWorkerArtifacts({ store, receipt, expected: { ...receipt }, trustedWorkers: { 'worker-1': { workerIdentity: 'worker-1', publicKeyPem, fingerprintSha256: fingerprint, nativeExecutableSha256: receipt.nativeExecutableSha256!, nativeInvocationSha256: receipt.nativeInvocationSha256! } }, metadata: receipt.outputArtifacts });
