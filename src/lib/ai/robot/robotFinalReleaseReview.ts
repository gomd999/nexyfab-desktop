...
import { verifyRobotVerifiedSystemsAuditAttestation, type RobotVerifiedSystemsReleaseAuditReport, type TrustedRobotVerifiedSystemsAuditKeys } from './robotVerifiedSystemsReleaseAudit';
...
export function verifyRobotFinalReleaseReview(auditBytes: Uint8Array, review: RobotFinalReleaseReview | undefined, trusted: TrustedReviewerKeys, trustedAuditIssuers: TrustedRobotVerifiedSystemsAuditKeys, now = Date.now()): RobotFinalReleaseDecision {
  const errors: string[] = [];
  ...
  // Deliberately reject the legacy v1-v3 audit receipts. They did not bind the
  // full engineering, motion, cable, safety/electrical and physical chain to
  // the exact-CAD/manufacturing target.
  const supportedAudit = audit?.schema === 'nexyfab.robot-verified-systems-release-audit.v2';
  ...
  if (audit && supportedAudit) errors.push(...verifyRobotVerifiedSystemsAuditAttestation(audit as RobotVerifiedSystemsReleaseAuditReport, trustedAuditIssuers, now));
  ...
  for (const signoff of signoffs) {
    ...
    if (typeof audit?.issuedAt === 'string' && Number.isFinite(Date.parse(signoff.reviewedAt)) && Date.parse(signoff.reviewedAt) < Date.parse(audit.issuedAt)) errors.push(`final_release_predates_audit:${signoff.role}`);
