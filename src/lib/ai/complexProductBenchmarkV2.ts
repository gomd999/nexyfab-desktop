...
export interface ComplexGroupMetricV2 { group: string; cases: number; runs: number; minimumRepeats: number; campaigns: number; axes: ComplexAxisMetricV2[]; gatePassRate: number | null; falseVerified: number; falseClear: number; destructivePartMerge: number; minimumRunAccuracy: number | null; runAccuracyStdDev: number | null; eligible: boolean; blockers: string[]; }
...
function groupMetric(group: string, cases: readonly ComplexBenchmarkCaseV2[], runs: readonly ComplexBenchmarkRunV2[], policy: ComplexBenchmarkPolicyV2, requireCaseMinimum: boolean): ComplexGroupMetricV2 {
  ...
  const requiredRuns = policy.repeatsPerCampaign * policy.consecutiveCampaigns; if (minimumRepeats < requiredRuns) blockers.push(`repeats_below_minimum:${minimumRepeats}/${requiredRuns}`); if (campaigns < policy.consecutiveCampaigns) blockers.push(`campaigns_below_minimum:${campaigns}/${policy.consecutiveCampaigns}`);
  ...
  return { group, cases: cases.length, runs: selected.length, minimumRepeats, campaigns, axes, gatePassRate, falseVerified, falseClear, destructivePartMerge, minimumRunAccuracy: runScores.length ? Math.min(...runScores) : null, runAccuracyStdDev: std, eligible: blockers.length === 0, blockers };
}
