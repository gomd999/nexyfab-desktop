/**
 * sketchAssistantLlm — multi-turn conversation memory tests (Phase 6.3).
 *
 * Companion to sketchAssistantLlm.test.ts. Focused on:
 *   - history pass-through to the chatFn message array
 *   - MAX_HISTORY_TURNS pair cap (older messages dropped)
 *   - appendToHistory convenience contract
...
const VALID_RESPONSE = JSON.stringify([
  {
    op: { type: 'delete_last' },
    confidence: 0.7,
    rationale: 'undo most recent',
  },
]);

describe('interpretSketchCommandLlm — multi-turn history', () => {
  it('passes a single prior turn through to the chatFn message array', async () => {
    const { fn, calls } = makeSpyChat(VALID_RESPONSE);
    const history: ChatTurn[] = [
      { role: 'user', content: 'draw a square 30' },
      { role: 'assistant', content: 'Added 30x30 square.' },
    ];
    await interpretSketchCommandLlm(
      { prompt: 'now undo it', state: { lineIds: [] }, history },
      { chatFn: fn },
    );
    ...
    expect(msgs[1]).toEqual({ role: 'user', content: 'draw a square 30' });
    expect(msgs[2]).toEqual({ role: 'assistant', content: 'Added 30x30 square.' });
    expect(msgs[3]!.role).toBe('user');
    expect(msgs[3]!.content).toContain('now undo it');
  });
  ...
  it(`caps history at the last ${MAX_HISTORY_TURNS} user/assistant pairs`, async () => {
    ...
    await interpretSketchCommandLlm(
      { prompt: 'current cmd', state: { lineIds: [] }, history },
      { chatFn: fn },
    );
    const msgs = calls[0]!.messages;
  ...
  it('useStub:true ignores history and still uses the rule-based stub', async () => {
    ...
    const r = await interpretSketchCommandLlm(
      { prompt: 'horizontal line', state: { lineIds: [] }, history },
  ...
  it('empty history works (degrades to current single-turn behavior)', async () => {
    const { fn, calls } = makeSpyChat(VALID_RESPONSE);
    await interpretSketchCommandLlm(
      { prompt: 'undo', state: { lineIds: [] }, history: [] },
      { chatFn: fn },
    );
    const msgs = calls[0]!.messages;
  ...
  it('LLM failure with history → falls back to the stub (history does not break recovery)', async () => {
    const history: ChatTurn[] = [
      { role: 'user', content: 'earlier square please' },
      { role: 'assistant', content: 'made a square' },
    ];
    const failing = async () => {
    ...
    // Stub matches "horizontal line" → at least one add_line suggestion.
    expect(r.matched).toBe(true);
