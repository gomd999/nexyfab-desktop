...
function questionId(kind: string, key: string): string { return `question:${kind}:${key}:${hash(`${kind}:${key}`)}`; }
function choiceId(value: unknown, index: number): string { return `choice:${hash(`${index}:${JSON.stringify(value)}`)}`; }
...
function conflictQuestion(conflict: DesignIntentConflict, node: AiDesignIntentNode | undefined): AiDesignQuestion { const choices: AiDesignQuestionChoice[] = conflict.values.map((value, index) => ({ id: choiceId(value.value, index), label: `${String(value.value)}`, value: value.value, sourceIds: [...value.sourceIds].sort(), sourceHashes: [...value.sourceHashes].sort() })); choices.push({ id: 'choice:provide-value', label: 'Provide another value', value: undefined, sourceIds: [], sourceHashes: [] }); return { id: questionId('conflict', conflict.key), key: conflict.key, kind: 'conflict', priority: 120 + Math.round((node?.impact ?? 0.5) * 20), reason: 'conflicting values require an explicit decision', prompt: conflict.resolutionQuestion, choices, impact: node?.impact ?? 0.5 }; }
