...
export interface IntentInput {
  ...
  facets?: number;
  /** Free-form 2D outline for shapeId === 'sketch' → linear_extrude(polygon).
   *  Closed polygon in mm; params.depth is the extrude height. */
  profile?: Array<{ x: number; y: number }>;
}

/** One part in an assembly: a normal IntentInput placed in world space. */
export interface AssemblyPartInput extends IntentInput {
  name?: string;
  /** World position (mm), default [0,0,0]. */
  position?: [number, number, number];
  /** World rotation (degrees), default [0,0,0]. */
...
/** Apply the supported feature stack (hole/fillet/…) to a base SCAD body. */
function applyScadFeatures(body: string, features: IntentFeature[] | undefined, warnings: string[]): string {
...
/** Free-form 2D outline → extruded solid: linear_extrude(polygon). */
function emitSketchBody(intent: IntentInput): string {
  const profile = intent.profile ?? [];
  if (!Array.isArray(profile) || profile.length < 3) {
    throw new Error('sketch needs a profile of at least 3 {x,y} points');
  }
  const depth = num(intent.params?.depth, 10);
  const pts = profile.map(p => `[${num(p.x, 0)}, ${num(p.y, 0)}]`).join(', ');
  return `linear_extrude(height=${depth}, center=true) polygon(points=[${pts}]);`;
}

/**
 * Assembly → SCAD: each part is emitted via intentToScad, then placed with
 * translate()/rotate() and combined with union(). Heterogeneous parts (box +
 * cylinder + bolt …) become one portable .scad file.
 */
export function assemblyToScad(parts: AssemblyPartInput[]): IntentToScadResult {
  if (!Array.isArray(parts) || parts.length < 2) {
    return { ok: false, reason: 'assembly needs at least 2 parts' };
  }
  const warnings: string[] = [];
  const blocks: string[] = [];
  let usesBosl2 = false;
  for (let i = 0; i < parts.length; i++) {
    const part = parts[i]!;
    const sub = intentToScad(part);
    if (!sub.ok) return { ok: false, reason: `part ${i + 1} (${part.shapeId}): ${sub.reason}` };
    warnings.push(...sub.warnings.map(w => `[part ${i + 1}] ${w}`));
    if (sub.scad.includes('BOSL2/std.scad')) usesBosl2 = true;
    // Strip each part's header/$fn — the assembly carries one shared header.
    const partBody = sub.scad.replace(/^\/\/[^\n]*\n/, '').replace(/^include <BOSL2\/std\.scad>\n/m, '').replace(/^\$fn = \d+;\n*/m, '').trim();
    const [px, py, pz] = part.position ?? [0, 0, 0];
    const [rx, ry, rz] = part.rotation ?? [0, 0, 0];
    const label = part.name ? ` // ${part.name}` : '';
    let placed = partBody;
    if (rx || ry || rz) placed = `rotate([${num(rx, 0)}, ${num(ry, 0)}, ${num(rz, 0)}])\n${indent(placed)}`;
    if (px || py || pz) placed = `translate([${num(px, 0)}, ${num(py, 0)}, ${num(pz, 0)}])\n${indent(placed)}`;
    blocks.push(`${label}\n${placed}`.trim());
  }
  const header = usesBosl2 ? `include <BOSL2/std.scad>\n` : '';
  const scad = `// Generated by NexyFab — assembly of ${parts.length} parts\n${header}$fn = 64;\n\nunion() {\n${indent(blocks.join('\n'), 2)}\n}\n`;
...
export function intentToScad(intent: IntentInput): IntentToScadResult {
  ...
  // Free-form sketch — a distinct capability, not a catalog shape (so it stays
  // out of SUPPORTED_SHAPES + its render test). Build the polygon extrude and
  // apply the same feature stack.
  if (intent.shapeId === 'sketch') {
    const warnings: string[] = [];
    let body: string;
    try {
      body = emitSketchBody(intent);
    } catch (e) {
      return { ok: false, reason: e instanceof Error ? e.message : 'sketch emit failed' };
    }
    body = applyScadFeatures(body, intent.features, warnings);
    const facets = Math.max(8, Math.min(256, num(intent.facets, 64)));
    const scad = `// Generated by NexyFab intentToScad — shape=sketch\n$fn = ${facets};\n\n${body}\n`;
    return { ok: true, scad, warnings };
  }
...
/** The intent shape the NL route returns to the client — a catalog/sketch
 *  single part OR an assembly wrapper. */
export type StoredIntent =
  | { kind: 'assembly'; parts: AssemblyPartInput[] }
  | IntentInput;

/** Unified entry: regenerate SCAD from whichever intent shape the route stored.
 *  Lets the client re-emit SCAD on a slider change WITHOUT another AI call. */
export function scadFromIntent(intent: StoredIntent): IntentToScadResult {
  if (intent && typeof intent === 'object' && 'kind' in intent && intent.kind === 'assembly') {
    return assemblyToScad(intent.parts);
  }
  return intentToScad(intent as IntentInput);
}
