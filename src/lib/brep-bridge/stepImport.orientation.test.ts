...
import { ARC_CHORD_TOL_MM, arcChordPoints, importStep, parseEntities } from './stepImport';
...
function makePlate(opts: {
  ...
  const W = 100, D = 60, H = 10;
  const lines: string[] = [];
  let id = 100;
  const emit = (s: string): number => { lines.push(`#${id}=${s};`); return id++; };
  const pt = (x: number, y: number, z: number): number => emit(`CARTESIAN_POINT('',(${x}.,${y}.,${z}.))`);
  const dir = (x: number, y: number, z: number): number => emit(`DIRECTION('',(${x}.,${y}.,${z}.))`);
  ...
  const face = (
    ...
    const vs = quad.map((q) => emit(`VERTEX_POINT('',#${pt(q[0], q[1], q[2])})`));
    const oes: number[] = [];
    for (let i = 0; i < 4; i++) {
      const a = quad[i]!, b = quad[(i + 1) % 4]!;
      const len = Math.hypot(b[0] - a[0], b[1] - a[1], b[2] - a[2]);
      const d = dir((b[0] - a[0]) / len, (b[1] - a[1]) / len, (b[2] - a[2]) / len);
      const vec = emit(`VECTOR('',#${d},${len}.)`);
      const ln = emit(`LINE('',#${pt(a[0], a[1], a[2])},#${vec})`);
      const ec = emit(`EDGE_CURVE('',#${vs[i]},#${vs[(i + 1) % 4]},#${ln},.T.)`);
    ...
    const ax = emit(`AXIS2_PLACEMENT_3D('',#${org},#${dir(...normal)},#${dir(...refDir)})`);
  ...
  const faces = [
    ...
    face([[0, 0, 0], [W, 0, 0], [W, D, 0], [0, D, 0]], [0, 0, 1], [1, 0, 0], opts.bottomSense, bn),
    face([[0, 0, H], [W, 0, H], [W, D, H], [0, D, H]], [0, 0, 1], [1, 0, 0], 'T', bn, opts.extraBoundOnTop),
    face([[0, 0, 0], [W, 0, 0], [W, 0, H], [0, 0, H]], [0, -1, 0], [1, 0, 0], 'T', bn),
    face([[0, D, 0], [W, D, 0], [W, D, H], [0, D, H]], [0, 1, 0], [1, 0, 0], 'T', bn),
    face([[0, 0, 0], [0, D, 0], [0, D, H], [0, 0, H]], [-1, 0, 0], [0, 1, 0], 'T', bn),
    face([[W, 0, 0], [W, D, 0], [W, D, H], [W, 0, H]], [1, 0, 0], [0, 1, 0], 'T', bn),
  ];
  const shell = emit(`CLOSED_SHELL('',(#${faces.join(',#')}))`);
  emit(`MANIFOLD_SOLID_BREP('',#${shell})`);
  return [
    'ISO-10303-21;', 'HEADER;',
    "FILE_DESCRIPTION((''),'');", "FILE_NAME('t','',(''),(''),'','','');",
...
describe('면 방향 sense 플래그 (ADVANCED_FACE 4번째 인자)', () => {
  ...
  it('★캡 방향이 아니라 **치수**가 맞아야 한다 — 바디 수만 세면 뒤집힌 판도 통과한다', () => {
    const r = importStep(makePlate({ bottomSense: 'F' })) as unknown as {
      tree: { nodes: Array<{ payload: { kind: string; depth?: number; loop?: Array<{ x: number; y: number }> } }> };
    };
    const f = r.tree.nodes[0]!.payload;
    expect(f.kind).toBe('extrude');
    expect(f.depth).toBeCloseTo(10, 6);
    const xs = f.loop!.map((q) => q.x), ys = f.loop!.map((q) => q.y);
...
describe('외곽 경계 선언 (FACE_OUTER_BOUND 부재)', () => {
  it('`FACE_BOUND` 가 하나뿐이면 그것이 외곽이다 — 유일한 해석이라 추정이 아니다', () => {
    expect(bodies(makePlate({ bottomSense: 'F', boundName: 'FACE_BOUND' }))).toBeGreaterThan(0);
  });

  it('★`FACE_BOUND` 가 여럿이면 **거부한다** — 면적 최대를 외곽으로 보는 것은 추정이다', () => {
    const r = importStep(makePlate({ bottomSense: 'F', boundName: 'FACE_BOUND', extraBoundOnTop: true })) as unknown as {
      tree: { nodes: unknown[] }; unsupported?: string[];
    };
    expect(r.tree.nodes.length).toBe(0);
    // 사유가 「무엇을 모르는지」를 말해야 다음에 무엇을 선언할지 알 수 있다.
    expect((r.unsupported ?? []).join(' ')).toMatch(/FACE_BOUND 2개|고를 수 없다/);
  });
});
...
describe('타원 현 근사 (ELLIPSE)', () => {
  ...
  function ellipseChord(a: number, b: number): { points: Array<[number, number, number]>; sagittaMm: number } {
  ...
  it('★고지한 새그가 실제 최대 편차보다 작지 않다 — 과고지 금지', () => {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      for (let k = 1; k < 60; k++) {
        ...
        const u = Math.max(0, Math.min(1, ((px - p0[0]) * dx + (py - p0[1]) * dy) / (dx * dx + dy * dy)));
        worst = Math.max(worst, Math.hypot(px - (p0[0] + u * dx), py - (p0[1] + u * dy)));
      }
    }
