...
describe("importStep — box round-trip", () => {
  it("reads a 10×20×30 box back to a 4-vertex rectangular ExtrudeFeature", () => {
    ...
    expect(result.placements[0]?.sourceEntityId).toEqual(expect.any(Number));
  });
  ...
  it("round-trips a box at non-zero Z origin (writeStepEntities z0 ≠ 0)", () => {
    const out = [
      ...
      writeStepEntities({
        boxes: [
          { name: "lifted", x0: 0, y0: 0, z0: 5, x1: 10, y1: 10, z1: 15 },
        ],
      }),
      "END-ISO-10303-21;\n",
    ].join("");
...
describe("importStep — polygon prism round-trip", () => {
  ...
  it("octagon prism → 8-vertex loop preserved", () => {
    ...
    const result = importStep(out);
    expect((result.tree.nodes[0]!.payload as ExtrudeFeature).loop).toHaveLength(
      8,
    );
  });
});
...
END-ISO-10303-21;`;
    ...
    expect(
      result.warnings.some((w) =>
        w.startsWith("parse:closed_shell_without_manifold_solid_brep"),
      ),
    ).toBe(true);
  });
  ...
  it("healable file without END-ISO marker still imports successfully", () => {
    const out = writeExtrudeAsStep(rectExtrude(2, 2, 2)).replace(
...
function makeRevolvedAreaSolidFile(opts: {
  ...
  for (let i = 0; i < 4; i++) {
    lines.push(
      `#${10 + i}=CARTESIAN_POINT('',(${fmt(p[i]![0])},${fmt(p[i]![1])},${fmt(p[i]![2])}));`,
    );
  }
  for (let i = 0; i < 4; i++) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    lines.push(
      `#${30 + i}=DIRECTION('',(${dx / len}.,${dy / len}.,${dz / len}.));`,
    );
  }
  for (let i = 0; i < 4; i++) {
  ...
  for (let i = 0; i < 4; i++) {
    lines.push(
      `#${50 + i}=EDGE_CURVE('',#${20 + i},#${20 + ((i + 1) % 4)},#${44 + i},.T.);`,
    );
  }
  for (let i = 0; i < 4; i++) {
  ...
  lines.push(
    `#81=DIRECTION('',(${axisDir[0]}.,${axisDir[1]}.,${axisDir[2]}.));`,
  );
...
function makeCylinderBrepFile(opts: {
  ...
  function squareAround(
    ...
    return [
      [
        c[0] - half * perp1[0] - half * perp2[0],
        c[1] - half * perp1[1] - half * perp2[1],
        c[2] - half * perp1[2] - half * perp2[2],
      ],
      [
        c[0] + half * perp1[0] - half * perp2[0],
        c[1] + half * perp1[1] - half * perp2[1],
  ...
  function emitCap(
    ...
    for (let i = 0; i < 4; i++) {
      lines.push(
        `#${pIds[i]}=CARTESIAN_POINT('',(${fmt(quad[i]![0])},${fmt(quad[i]![1])},${fmt(quad[i]![2])}));`,
      );
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const len = Math.hypot(dx, dy, dz) || 1;
      lines.push(
        `#${id}=DIRECTION('',(${dx / len}.,${dy / len}.,${dz / len}.));`,
      );
      dirIds.push(id);
      id++;
    }
    ...
    for (let i = 0; i < 4; i++) {
      lines.push(
        `#${id}=EDGE_CURVE('',#${vIds[i]},#${vIds[(i + 1) % 4]},#${lineIds[i]},.T.);`,
      );
      ecIds.push(id);
      id++;
    }
    ...
    lines.push(
      `#${id}=EDGE_LOOP('',(#${oeIds[0]},#${oeIds[1]},#${oeIds[2]},#${oeIds[3]}));`,
    );
    const loopId = id;
    id++;
    ...
    lines.push(
      `#${id}=CARTESIAN_POINT('',(${fmt(centre[0])},${fmt(centre[1])},${fmt(centre[2])}));`,
    );
    const orgId = id;
    id++;
    lines.push(
      `#${id}=DIRECTION('',(${normalDir[0]}.,${normalDir[1]}.,${normalDir[2]}.));`,
    );
    const ndirId = id;
    id++;
    lines.push(
      `#${id}=DIRECTION('',(${perp1[0]}.,${perp1[1]}.,${perp1[2]}.));`,
    );
    const refDirId = id;
    id++;
    lines.push(
      `#${id}=AXIS2_PLACEMENT_3D('',#${orgId},#${ndirId},#${refDirId});`,
    );
    const axisId = id;
    id++;
  ...
  lines.push(
    `#${nextId}=CLOSED_SHELL('',(#27,#${bottom.faceId},#${top.faceId}));`,
  );
...
#90=REVOLVED_AREA_SOLID('',#44,#82,6.283185307179586);`);
  ...
  it("two cylinders in one file → 2 RevolveFeature nodes", () => {
    ...
    const offset = 1000;
    const c2Block = c2
      .slice(c2DataStart + "DATA;".length, c2DataEnd)
      .replace(/#(\d+)/g, (_, n) => `#${Number(n) + offset}`);
    const merged = c1.slice(0, c1End) + c2Block + c1.slice(c1End);
    const result = importStep(merged);
    expect(result.tree.nodes).toHaveLength(2);
    expect(result.tree.nodes.every((n) => n.payload.kind === "revolve")).toBe(
      true,
    );
...
function makeExtrudedAreaSolidFile(opts: {
  ...
  lines.push(`#72=PLANAR_FACE('',(#71));`);
  lines.push(
    `#80=DIRECTION('',(${axisDir[0]}.,${axisDir[1]}.,${axisDir[2]}.));`,
  );
...
function makeLinearExtrusionBrepFile(opts: {
  ...
  lines.push(
    `#${nextId}=CLOSED_SHELL('',(#25,#${bottom.faceId},#${top.faceId}));`,
  );
  const shellId = nextId++;
  lines.push(`#${nextId}=MANIFOLD_SOLID_BREP('',#${shellId});`);
  return wrapStepFile(lines.join("\n"));
...
describe("importStep — Phase 3 mixed files", () => {
  it("box + sweep (EXTRUDED_AREA_SOLID) → tree has 2 nodes (extrude + sweep)", () => {
    const boxFile = writeExtrudeAsStep(rectExtrude(4, 5, 6));
    const sweepFile = makeExtrudedAreaSolidFile({
      axis: "+z",
      width: 2,
      height: 3,
      depth: 4,
    ...
    const sweepBlock = sweepFile
      ...
      .replace(/#(\d+)/g, (_, n) => `#${Number(n) + offset}`);
    const merged =
      boxFile.slice(0, boxEndsec) + sweepBlock + boxFile.slice(boxEndsec);
    const result = importStep(merged);
    expect(result.tree.nodes).toHaveLength(2);
    const kinds = result.tree.nodes.map((n) => n.payload.kind).sort();
  ...
  it("cylinder + sweep → tree has 2 nodes (revolve + sweep)", () => {
    ...
    const sweepBlock = sweepFile
      ...
      .replace(/#(\d+)/g, (_, n) => `#${Number(n) + offset}`);
    const merged =
      cylFile.slice(0, cylEndsec) + sweepBlock + cylFile.slice(cylEndsec);
    const result = importStep(merged);
    expect(result.tree.nodes).toHaveLength(2);
    const kinds = result.tree.nodes.map((n) => n.payload.kind).sort();
  ...
  it("two extrusions in one file → 2 separate SweepFeature nodes", () => {
    const a = makeExtrudedAreaSolidFile({
      axis: "+z",
      width: 1,
      height: 1,
      depth: 2,
    });
    const b = makeExtrudedAreaSolidFile({
      axis: "+x",
      width: 1,
      height: 1,
      depth: 3,
    ...
    const result = importStep(merged);
    expect(result.tree.nodes).toHaveLength(2);
    expect(result.tree.nodes.every((n) => n.payload.kind === "sweep")).toBe(
      true,
    );
...
#20=SWEPT_DISK_SOLID('',#14,5.,3.,0.,1.);`);
  ...
  it("SWEPT_DISK_SOLID + EXTRUDED_AREA_SOLID in same file → 1 sweep + 1 unsupported", () => {
    const sweep = makeExtrudedAreaSolidFile({
      axis: "+z",
      width: 2,
      height: 2,
      depth: 3,
...
#910=SWEPT_DISK_SOLID('',#903,2.,1.,0.,1.);`;
    ...
    const merged =
      sweep.slice(0, sweepEndsec) + diskBlock + "\n" + sweep.slice(sweepEndsec);
    const result = importStep(merged);
    expect(result.tree.nodes).toHaveLength(1);
    expect(result.tree.nodes[0]!.payload.kind).toBe("sweep");
    expect(result.unsupported).toHaveLength(1);
