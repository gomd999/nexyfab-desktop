 ...
 *  2) 기존 ifcImport.ifcToNexyfabAssembly 라운드트립 — AABB 치수·배치(임포터 방출 0.1mm
 *     반올림 정밀도 기준)·클래스 집계.
 */
...
describe('writeIfcText → 기존 ifcImport 라운드트립', () => {
  it('박스 — proxy 1건·AABB 20×30×40 at(0,0,0)·MILLI 단위(unitScale=1)', () => {
    const w = writeIfcText(BOX, { name: 'rt-box' });
    expect(w.ok).toBe(true);
    if (!w.ok) return;
  ...
  it('L-프리즘 — AABB 60×40×20(임포터=AABB 근사 — 실부피 32000 주장 안 함)', () => {
