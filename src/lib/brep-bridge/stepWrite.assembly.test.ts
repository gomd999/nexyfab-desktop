...
import { writeAssemblyAsStep, type AssemblyStepInput } from "./stepWrite";
...
describe("writeAssemblyAsStep", () => {
  ...
  it("emits multiple native solids inside one part definition without promoting bodies to parts", () => {
    const out = writeAssemblyAsStep({
      assemblyName: "weldment",
      parts: [
        {
          kind: "multi_body",
          id: "frame",
    ...
    expect(out.match(/MANIFOLD_SOLID_BREP/g)).toHaveLength(3);
    const imported = importStepAssembly(out, { maxClassifyParts: 0 });
    expect(
      imported.productStructure?.definitions.filter((item) => !item.container),
    ).toEqual([expect.objectContaining({ name: "Frame", bodyCount: 3 })]);
    expect(imported.productStructure?.occurrences).toHaveLength(1);
  });
