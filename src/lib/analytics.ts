 ...
 *
 * Desktop (Tauri): PostHog “product improvement” traffic respects the first-run
 * wizard optional checkbox (`getTelemetryOptIn`). Web behaviour is unchanged.
 */

import { isTauriApp } from './tauri';
import { getTelemetryOptIn } from './platform';
...
/** PostHog-only gate: desktop users must opt in from the welcome wizard. */
function allowPosthogProductTelemetry(): boolean {
...
function forwardToPostHog(eventName: string, params?: EventParams): void {
  ...
  if (!allowPosthogProductTelemetry()) return;
