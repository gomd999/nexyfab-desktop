...
describe('config module', () => {
  ...
  it('returns empty array when ALLOWED_ORIGINS and NEXT_PUBLIC_SITE_URL are unset', async () => {
  ...
  it('derives allowedOrigins from NEXT_PUBLIC_SITE_URL when ALLOWED_ORIGINS is unset', async () => {
    delete process.env.ALLOWED_ORIGINS;
    process.env.NEXT_PUBLIC_SITE_URL = 'https://app.example.com/path';
    const { config } = await import('./config');
    expect(config.auth.allowedOrigins).toEqual(['https://app.example.com']);
  });
