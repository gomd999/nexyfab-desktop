...
describe('sanitizeText', () => {
  ...
  it('strips decimal numeric entities for angle brackets', () => {
    expect(sanitizeText('&#60;script&#62;alert(1)&#60;/script&#62;')).not.toContain('<script');
  });

  it('strips hex numeric entities (lowercase x)', () => {
    expect(sanitizeText('&#x3C;script&#x3E;')).not.toContain('<script');
    expect(sanitizeText('&#x3c;script&#x3e;')).not.toContain('<script');
  });

  it('strips hex numeric entities (uppercase X)', () => {
    expect(sanitizeText('&#X3C;script&#X3E;')).not.toContain('<script');
  });

  it('strips numeric entities with leading zeros', () => {
    expect(sanitizeText('&#060;script&#062;')).not.toContain('<script');
    expect(sanitizeText('&#x003C;script&#x003E;')).not.toContain('<script');
  });
