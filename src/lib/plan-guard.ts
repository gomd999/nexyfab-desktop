...
export const PLAN_MONTHLY_LIMITS: Record<string, Record<string, number>> = {
  free:       { analyze_step: 5,  rfq: 3,  shape_chat: 20, ai_advisor: 5,  supplier_match: -2, cam_export: -2 },
  pro:        { analyze_step: -1, rfq: -1, shape_chat: -1, ai_advisor: -1, supplier_match: -1, cam_export: -1 },
  team:       { analyze_step: -1, rfq: -1, shape_chat: -1, ai_advisor: -1, supplier_match: -1, cam_export: -1 },
  enterprise: { analyze_step: -1, rfq: -1, shape_chat: -1, ai_advisor: -1, supplier_match: -1, cam_export: -1 },
};
...
export function recordUsageEvent(userId: string, metric: string): void {
  import('./db-adapter').then(({ getDbAdapter }) => {
    const db = getDbAdapter();
    const id = `ue-${Math.random().toString(36).slice(2)}`;
    db.execute(
      `INSERT INTO nf_usage_events (id, user_id, product, metric, quantity, cycle_start, metadata, created_at) VALUES (?, ?, 'nexyfab', ?, 1, 0, NULL, ?)`,
      id, userId, metric, Date.now(),
    ).catch(() => { /* ignore */ });
  }).catch(() => { /* ignore */ });
}
