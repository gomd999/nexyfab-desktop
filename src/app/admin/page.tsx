    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          {[
            { label: '진행 중 계약', value: activeContracts.length, icon: '📦', color: 'text-blue-600' },
            { label: '신규 문의', value: newInquiries.length, icon: '📩', color: 'text-orange-500' },
            { label: '등록 파트너', value: partners.length, icon: '🏭', color: 'text-purple-600' },
            { label: '누적 수수료', value: won(totalCommission), icon: '💰', color: 'text-green-600' },
          ].map((s, i) => (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <a key={i} href={s.href} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-blue-200 hover:shadow-md transition-all group">
              <div className="text-2xl mb-2">{s.icon}</div>
              <div className={`text-xl font-black ${s.color}`}>{s.value}</div>
        ...
        {health && (
          <div className="flex flex-wrap items-center gap-2 mb-6 p-3 bg-white rounded-xl border border-gray-100 shadow-sm text-xs">
            ...
            <a href="/admin/jobs" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg font-semibold transition ${health.jobPending > 0 ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 'bg-gray-50 text-gray-500'}`}>
              ⚡ 대기 Job {health.jobPending}
            </a>
            <a href="/admin/email-logs" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg font-semibold transition ${health.jobFailed > 0 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-gray-50 text-gray-500'}`}>
              {health.jobFailed > 0 ? '❌' : '✅'} 실패 Job {health.jobFailed}
            </a>
            <a href="/admin/sla" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg font-semibold transition ${health.slaOverdue > 0 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-gray-50 text-gray-500'}`}>
              🚨 SLA 위반 {health.slaOverdue}
            </a>
            {health.slaWarning > 0 && (
              <a href="/admin/sla" className="flex items-center gap-1 px-2.5 py-1 rounded-lg font-semibold bg-amber-50 text-amber-600 border border-amber-200 transition">
