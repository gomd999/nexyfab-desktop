...
export default function SLAPage() {
  ...
  const triggerCheck = async () => {
    ...
    try {
      ...
      setTriggerMsg(L(`SLA 체크 완료 — 알림 ${d.alertsFound}건 발견`, `SLA check complete — ${d.alertsFound} alerts found`));
      await load();
    } catch {
      setTriggerMsg(L('오류가 발생했습니다.', 'An error occurred.'));
    } finally {
      setTriggering(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{copy.pageTitles.sla}</h1>
          <p className="text-sm text-gray-500 mt-1">{L('마일스톤 기한 경고 및 연체 현황', 'Milestone deadline warnings and overdue status')}</p>
        </div>
        <div className="flex gap-2 items-center">
          ...
          <button onClick={triggerCheck} disabled={triggering}
            ...
            {triggering ? L('체크 중...', 'Checking...') : L('수동 SLA 체크', 'Run SLA check')}
          </button>
      ...
      {data && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: L('전체 알림', 'All alerts'), value: data.summary.total, color: 'text-gray-900', bg: 'bg-white' },
            { label: L('연체', 'Overdue'), value: data.summary.overdue, color: 'text-red-700', bg: 'bg-red-50' },
            { label: L('경고 (3일 이내)', 'Warning (within 3 days)'), value: data.summary.warning, color: 'text-amber-700', bg: 'bg-amber-50' },
          ].map(c => (
        ...
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
          {(['all', 'overdue', 'warning'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? L('전체', 'All') : f === 'overdue' ? L('연체', 'Overdue') : L('경고', 'Warning')}
            </button>
          ))}
        </div>
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder={L('마일스톤 또는 계약 ID 검색...', 'Search milestone or contract ID...')}
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 text-sm">
            {data?.summary.total === 0
              ? L('모든 마일스톤이 정상입니다.', 'All milestones are on track.')
              : L('검색 결과가 없습니다.', 'No results found.')}
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      ...
                      {alert.status === 'overdue' ? L('연체', 'Overdue') : L('경고', 'Warning')}
                    </span>
                    <span className="text-sm font-bold text-gray-900 truncate">{alert.title}</span>
                  </div>
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500 mt-1">
                    <span>{L('계약:', 'Contract:')} <code className="font-mono text-gray-700 bg-gray-100 px-1 rounded">{alert.contractId}</code></span>
                    <span>{L('기한:', 'Due:')} <strong className={alert.status === 'overdue' ? 'text-red-600' : 'text-amber-600'}>{formatDate(alert.dueDate, locale) ?? alert.dueDate}</strong></span>
                  </div>
                </div>
                <div className="shrink-0 flex flex-col items-end gap-2">
                  <DaysBadge days={alert.daysUntilDue} overdueLabel={L('연체', 'overdue')} />
                  <a href={`/admin/contracts?id=${alert.contractId}`}
                    className="text-xs text-blue-600 hover:underline">{L('계약 보기 →', 'View contract →')}</a>
                </div>
              </div>
