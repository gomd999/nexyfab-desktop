  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-6">
      <div className="bg-gray-800 rounded-2xl border border-gray-700 p-10 max-w-md w-full text-center space-y-4">
        ...
        <h2 className="text-lg font-bold text-white">{L('오류가 발생했습니다', 'Something went wrong', 'エラーが発生しました', '发生错误', 'Algo salió mal', 'حدث خطأ')}</h2>
        <p className="text-sm text-gray-400">{error.message || L('페이지를 불러오는 중 문제가 생겼습니다.', 'There was a problem loading the page.', 'ページの読み込み中に問題が発生しました。', '加载页面时出现问题。', 'Se produjo un problema al cargar la página.', 'حدثت مشكلة أثناء تحميل الصفحة。')}</p>
        <button
          onClick={reset}
