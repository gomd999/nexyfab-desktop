...
export async function POST(req: NextRequest) {
  ...
  const usageCheck = await checkMonthlyLimit(planCheck.userId, planCheck.plan, 'capacity_match', planCheck.orgId);
  if (!usageCheck.ok) {
  ...
  if (!body.partner || !body.partner.processes || body.partner.processes.length === 0) {
    return NextResponse.json({ error: localizedApiMessage(locale, 'badRequest'), code: 'PARTNER_PROCESSES_REQUIRED', outputLanguage: locale.route }, { status: 400 });
  }
  if (!body.partner.idleWindowDays || body.partner.idleWindowDays < 1) {
    return NextResponse.json({ error: localizedApiMessage(locale, 'badRequest'), code: 'IDLE_WINDOW_INVALID', outputLanguage: locale.route }, { status: 400 });
  }
  ...
  const messages: ChatMessage[] = [
    ...
    { role: 'user', content: JSON.stringify({ partner: body.partner, openRfqs: body.openRfqs ?? [], requestedLanguage: locale.languageName }) },
  ];
  ...
  try {
    ...
    if (e instanceof AiNotConfiguredError) {
      recordUsageEvent(planCheck.userId, 'capacity_match', undefined, planCheck.orgId);
      const fallback = ruleBasedResult(body);
      recordAIHistory({ userId: planCheck.userId, orgId: planCheck.orgId, feature: 'capacity_match', title: historyTitle, payload: fallback, context: historyContext, projectId: body.projectId });
  ...
  try {
    ...
    recordUsageEvent(planCheck.userId, 'capacity_match', undefined, planCheck.orgId);
    recordAIHistory({ userId: planCheck.userId, orgId: planCheck.orgId, feature: 'capacity_match', title: historyTitle, payload: result, context: historyContext, projectId: body.projectId });
