...
export async function GET(req: NextRequest) {
  ...
  // Find the factory linked to this partner email
  const factory = await db.queryOne<FactoryRow>(
    "SELECT id, name, region, status FROM nf_factories WHERE partner_email = ? AND status = 'active' LIMIT 1",
    authUser.email,
  ).catch(() => null);
  ...
  const quoteStats = await db.queryOne<QuoteStatsRow>(
    `SELECT
     ...
     WHERE q.factory_name = ? OR q.partner_email = ?`,
    factoryName, authUser.email,
  ).catch(() => null);
  ...
  const contractStats = await db.queryOne<ContractStatsRow>(
    `SELECT
     ...
     FROM nf_contracts
     WHERE partner_email = ? OR factory_name = ?`,
    authUser.email, factoryName,
  ).catch(() => null);
