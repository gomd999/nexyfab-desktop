 ...
 *
 * WHY A ROUTE (honest constraint): the gmsh mesher, the FEA precise path, and the
 * reconstruction fleet (OpenSCAD render + LLM) all need binaries/keys that exist
 * ONLY in the deployed container. This route is the only way to run them THERE and
 * read back two real numbers:
 ...
 * returns; `what=fleet` respects a hard overall time budget and returns PARTIAL
 * results with `timedOut:true` rather than hanging. The route is fully inert
 * (404 on every request) unless SELFTEST_TOKEN is set.
 */
...
// A single precise FEA can take ~25-60s; a fleet run minutes. Allow headroom.
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: `unknown what="${what}" (use fea|fleet)` }, { status: 400 });
  } catch (e) {
