...
export async function POST(req: NextRequest) {
  ...
  try {
    for (let i = 0; i < N; i++) {
      ...
      try {
        const rendered = await executeOpenScad({
          scadSource: layerScad,
          format: 'dxf',
          timeoutMs: 30_000,
          userId: `papercraft:${ip}`,
        });
        if (!rendered.ok) throw new Error(rendered.message);
        const dxf = rendered.buffer.toString('utf8');
  ...
  } finally { /* isolated worker owns and removes all native-process temp files */ }
}
