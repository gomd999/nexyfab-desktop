...
export async function POST(req: NextRequest) {
  ...
  const usageCheck = await checkMonthlyLimit(planCheck.userId, planCheck.plan, 'change_detector', planCheck.orgId);
  if (!usageCheck.ok) {
  ...
  if (!body.prev || !body.next) {
    return NextResponse.json({ error: localizedApiMessage(locale, 'badRequest'), code: 'DESIGN_SPECS_REQUIRED', outputLanguage: locale.route }, { status: 400 });
  }
  ...
  const messages: ChatMessage[] = [
    ...
    { role: 'user', content: JSON.stringify({ prev: body.prev, next: body.next, activeRfqs: body.activeRfqs, requestedLanguage: locale.languageName }) },
  ];
  ...
  try {
    ...
    if (e instanceof AiNotConfiguredError) {
      recordUsageEvent(planCheck.userId, 'change_detector', undefined, planCheck.orgId);
      const fallback = ruleBasedResult(body);
      recordAIHistory({ userId: planCheck.userId, orgId: planCheck.orgId, feature: 'change_detector', title: historyTitle, payload: fallback, context: historyContext, projectId: body.projectId });
  ...
  try {
    ...
    recordUsageEvent(planCheck.userId, 'change_detector', undefined, planCheck.orgId);
    recordAIHistory({ userId: planCheck.userId, orgId: planCheck.orgId, feature: 'change_detector', title: historyTitle, payload: result, context: historyContext, projectId: body.projectId });
