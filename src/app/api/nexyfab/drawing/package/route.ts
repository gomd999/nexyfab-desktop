...
import { buildDesignArtifactManifest } from '@/lib/designArtifactBinding';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const body = await readBoundedJson<{ assembly?: Assembly; options?: Record<string, unknown>; templateId?: string; domain?: string; params?: Record<string, unknown>; verifyParams?: Record<string, unknown> }>(req, MAX_BODY_BYTES);
    ...
    if (boundedJsonError(error)?.code === 'PAYLOAD_TOO_LARGE') return NextResponse.json({ ok: false, error: '패키지 입력이 너무 큽니다.' }, { status: 413 });
  ...
  try {
    files.push({
      ...
      content: mods.pkg.ga2dDrawing(assembly, {
        ...
        // 모든 요청 언어를 생성기에 전달한다. 현재 생성기 본문 번역 범위가
        // 제한된 언어는 한국어 본문으로 남지만, 언어 메타데이터·부분 번역
        // 경로를 조용히 버리지 않고 이후 번역 확장을 가능하게 한다.
...
`,
  ...
  const known: Record<string, string> = {
    ...
    'artifact-manifest.json': '동일 REV 산출물의 실제 바이트 SHA-256 목록 — 제조 승인 증명은 아니다.',
    'generator.mjs': '이 패키지를 다시 만드는 스크립트(Node 18+).',
  };
  const rows = files.map((f) => li(f.name, known[f.name] ?? '산출물')).join('')
    + li('artifact-manifest.json', known['artifact-manifest.json']);
  files.push({
  ...
  // Exact-byte manifest: STEP, BOM, drawings and reports all bind to the same
  // canonical design revision. It is co-revision evidence, not manufacturing
  // approval; the latter remains false until round-trip gates and review pass.
  const artifactBinding = buildDesignArtifactManifest({
    revisionId: basis.rev,
    // Every request input that can alter generated bytes belongs to the
    // revision. Otherwise language/title, FEA or verification changes could
    // produce a different package while retaining the same revision hash.
    revisionValue: { assembly, options, verifyParams, template: tpl, domain },
    artifacts: files.map((file) => ({
      name: file.name,
      mime: file.mime,
      bytes: file.name.endsWith('.dxf')
        ? iconv.encode(file.content, 'cp949')
        : file.b64
  ...
  files.push({
    ...
    content: JSON.stringify(artifactBinding.manifest, null, 2),
  });
