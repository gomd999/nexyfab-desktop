 ...
 *
 * caller: { assembly, views? } → { ok, views: { front?, top? }, dims }.
 */
...
type HlrModule = {
  hlrDrawingWithDims: (asm: unknown, opts?: { views?: string[] }) => Promise<{
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  let body: { assembly?: { parts?: unknown[] }; views?: string[] };
  try {
    body = (await req.json()) as typeof body;
  } catch {
    return NextResponse.json({ ok: false, error: 'invalid json' }, { status: 400 });
  }
  if (!body.assembly || !Array.isArray(body.assembly.parts) || body.assembly.parts.length === 0) {
  ...
  try {
    ...
    const result = await mod.hlrDrawingWithDims(body.assembly, { views });
    if (!result.ok) {
      return NextResponse.json({ ok: false, error: 'assembly gate 실패', gateErrors: result.gateErrors ?? [] }, { status: 422 });
    }
