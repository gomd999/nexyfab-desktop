...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Authorize: owner or has a quote for this RFQ
  let authorized = rfq.user_id === authUser.userId;
  if (!authorized) {
    const quote = await db.queryOne<{ id: string }>(
      `SELECT id FROM nf_quotes WHERE rfq_id = ? AND partner_email = ?`,
      rfqId, authUser.email,
    ).catch(() => null);
    authorized = !!quote;
  }
