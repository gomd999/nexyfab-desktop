...
const SUBTITLES: Record<string, Record<string, string>> = {
  home: { kr: '브라우저에서 3D 모델링부터 AI 견적까지. 28만+ 공장 DB', en: 'From 3D modeling to AI quoting — all in your browser. 286K+ factories', ja: 'ブラウザで3DモデリングからAI見積もりまで。28万以上の工場', cn: '浏览器3D建模到AI报价。28万+工厂数据库', es: 'Modelado 3D a cotización IA en tu navegador. 286K+ fábricas', ar: 'من النمذجة إلى التسعير في متصفحك. 286 ألف+ مصنع' },
  ...
  'how-it-works': { kr: 'DB 검색 → AI 분석 → 적합 파트너', en: 'DB Search → AI Analysis → Matched Partner', ja: 'DB検索 → AI分析 → 適合パートナー', cn: 'DB搜索 → AI分析 → 匹配合作伙伴', es: 'Búsqueda DB → Análisis IA → Socio Adecuado', ar: 'بحث DB → تحليل AI → شريك مناسب' },
