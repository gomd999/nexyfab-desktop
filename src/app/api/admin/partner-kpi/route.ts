...
export async function GET(req: NextRequest) {
  ...
  // 1. All factories
  type FactoryRow = { id: string; name: string; partner_email: string | null };
  const factories = await db.queryAll<FactoryRow>(
    'SELECT id, name, partner_email FROM nf_factories ORDER BY name ASC',
  ).catch((): FactoryRow[] => []);
  ...
  for (const f of factories) {
    ...
    const contractStats = await db.queryOne<ContractStats>(
      `SELECT
       ...
       FROM nf_contracts
       WHERE partner_email = ? OR factory_name = (SELECT name FROM nf_factories WHERE id = ? LIMIT 1)`,
      f.partner_email, f.id,
    ).catch(() => null);
    ...
    const overdueStats = await db.queryOne<OverdueRow>(
      ...
      nowIso, nowIso, f.partner_email, f.id,
    ).catch(() => null);
