...
export async function POST(req: NextRequest) {
  ...
  const factories = await db.queryAll<FactoryRow>(
    `SELECT id, name, partner_email, processes, rating, price_level
     FROM nf_factories
     WHERE status = 'active'`,
  );
  ...
  if (best.partner_email) {
    try {
      ...
      await sendEmail(best.partner_email, '새 견적 요청이 배정되었습니다', html);
    } catch (err) {
  ...
  const partnerKey = best.partner_email
    ? `partner:${best.partner_email}`
    : `factory:${best.id}`;
