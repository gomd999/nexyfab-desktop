...
export async function POST(req: NextRequest) {
  ...
  const factory = await db.queryOne<{
    id: string; name: string; name_ko: string | null; contact_email: string | null;
  }>('SELECT id, name, name_ko, contact_email FROM nf_factories WHERE id = ?', body.factoryId);
  ...
  if (factory.contact_email) {
    ...
    await enqueueJob('send_email', {
      ...
      subject: `[NexyFab] 새 견적 요청 — ${rfq.shape_name || rfq.id}`,
      html,
    });
  }

  // 파트너 인앱 알림 (공장 이메일 기준 partner:email 키 사용)
  if (factory.contact_email) {
    createNotification(
      `partner:${factory.contact_email}`,
      'quote_received',
      '새 견적 요청 배정',
