 ...
 * PATCH  /api/admin/users  — 회원 정보 수정 { userId, plan?, role?, locked? }
...
export async function GET(req: NextRequest) {
  ...
  const [users, countRow, stats] = await Promise.all([
    ...
    db.queryAll<{ plan: string; count: number }>(
      `SELECT plan, COUNT(*) AS count FROM nf_users GROUP BY plan`,
    ),
  ]);

  // 각 유저의 제품별 역할 조회
  const userIds = users.map(u => u.id);
  let rolesMap: Record<string, { product: string; role: string }[]> = {};
  if (userIds.length > 0) {
  ...
  // 각 유저의 조직 정보 조회
  let orgsMap: Record<string, { org_id: string; org_name: string; role: string }[]> = {};
  if (userIds.length > 0) {
    const placeholders = userIds.map(() => '?').join(',');
