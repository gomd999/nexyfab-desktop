...
export async function POST(req: NextRequest) {
  ...
  if (action === 'quote_expiry_remind') {
    ...
    const origin = req.headers.get('origin') ?? process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:3000';
  ...
  if (action === 'db_backup') {
    const secret = process.env.CRON_SECRET ?? '';
    const origin = req.headers.get('origin') ?? (process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:3000');
    const res = await fetch(`${origin}/api/jobs/db-backup`, {
      method: 'POST',
      headers: { 'x-cron-secret': secret },
    }).catch(() => null);
    const data = res ? await res.json().catch(() => ({})) : {};
