...
import { buildCurrentCanonicalMechanicalArtifactBundleV2 } from '@/app/[lang]/shape-generator/drawing/currentCanonicalMechanicalBundleV2';
...
/**
 * Authenticated, read/compute-only endpoint for the explicit FeatureTree v2
 * exact sequence. It never accepts caller revision data, creates a revision,
 * or grants manufacturing release.
 */
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!auth) {
      return response({
        ok: false, status: 'HOLD', code: 'UNAUTHORIZED',
    ...
    if (!SAFE_ID.test(id) || !SAFE_ID.test(documentId)) {
      return response({
        ok: false, status: 'HOLD', code: 'INVALID_PATH',
        release: 'HOLD', releaseReady: false,
      }, 400);
    }
    const rate = await rateLimitAsync(
      `cad-v2-mechanical-exact-feature-tree-v2:${auth.userId}:${getTrustedClientIp(request.headers)}`,
      RATE_LIMIT_MAXIMUM,
      60_000,
    ...
    if (!access) {
      return response({
        ok: false, status: 'HOLD', code: 'NOT_FOUND',
        release: 'HOLD', releaseReady: false,
      }, 404);
    }
    ...
    if (result.status !== 'EXACT_BUNDLE_V2_PASS') {
