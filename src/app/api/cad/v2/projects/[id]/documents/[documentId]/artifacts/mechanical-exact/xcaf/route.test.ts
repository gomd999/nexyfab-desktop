...
vi.mock('@/lib/rate-limit', () => ({ rateLimitAsync: mocks.rate, rateLimitHeaders: mocks.rateHeaders }));
vi.mock('@/lib/occt/xcafHttpInspectionClient', () => ({ loadXcafHttpInspectorFromEnvironment: mocks.loadInspector }));
vi.mock('@/app/[lang]/shape-generator/drawing/currentCanonicalMechanicalXcafService', () => ({ readCurrentCanonicalMechanicalXcaf: mocks.read }));

import { dynamic, GET, maxDuration, runtime } from './route';

const URL = 'https://nexyfab.com/api/cad/v2/projects/project-1/documents/document-1/artifacts/mechanical-exact/xcaf';
...
const envelope = {
  ...
  status: 'PASS_NATIVE_INVOCATION_BOUND', authority: 'SERVER_CURRENT_CANONICAL_HEAD',
  verification: 'XCAF_OCCURRENCE_IDENTITY_ONLY', geometryIdentity: 'NOT_EXPOSED_BY_BINDING', release: 'HOLD',
};
...
describe('current canonical mechanical XCAF route', () => {
  it('reads only the authenticated current head through the configured worker and remains release HOLD', async () => {
    ...
    await expect(result.json()).resolves.toMatchObject({
      ok: true, status: 'PASS_NATIVE_INVOCATION_BOUND',
      geometryIdentity: 'NOT_EXPOSED_BY_BINDING', release: 'HOLD', releaseReady: false, envelope,
    });
    expect(mocks.read).toHaveBeenCalledWith({
      db: mocks.db, projectId: 'project-1', documentId: 'document-1', inspector: mocks.inspector,
    });
  });
  ...
  it('fails before native work for auth, path, rate, access, and worker configuration failures', async () => {
    ...
    expect((await GET(request(), { params: Promise.resolve({ id: '../bad', documentId: 'document-1' }) })).status).toBe(400);
  ...
  it('does not expose worker, database, or canonical blocker details', async () => {
    mocks.read.mockResolvedValueOnce({
      schema: envelope.schema, status: 'HOLD', release: 'HOLD', blockers: ['CURRENT_XCAF_SERVICE_HOLD'],
    });
    const held = await GET(request(), context());
    expect(held.status).toBe(422);
    await expect(held.json()).resolves.toEqual({
      ok: false, status: 'HOLD', code: 'CURRENT_CANONICAL_XCAF_HOLD', release: 'HOLD',
    });
  });
});
