...
import { readCurrentCanonicalMechanicalXcafV2 } from '@/app/[lang]/shape-generator/drawing/currentCanonicalMechanicalXcafV2Service';
...
/** Authenticated current-head-only XCAF inspection for FeatureTree v2. */
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    const auth = await getAuthUser(request);
    if (!auth) {
      return response({ ok: false, status: 'HOLD', code: 'UNAUTHORIZED', release: 'HOLD' }, 401);
    }
    ...
    const rate = await rateLimitAsync(
      `cad-v2-mechanical-xcaf-feature-tree-v2:${auth.userId}:${getTrustedClientIp(request.headers)}`,
      RATE_LIMIT_MAXIMUM,
      60_000,
    ...
    if (!access) {
      return response({ ok: false, status: 'HOLD', code: 'NOT_FOUND', release: 'HOLD' }, 404);
    }
    const configured = loadXcafHttpInspectorFromEnvironment();
    if (!configured.ok) {
      return response({
        ok: false, status: 'HOLD', code: 'XCAF_SERVICE_UNAVAILABLE', release: 'HOLD',
      }, 503);
    }
    ...
    if (result.status !== 'PASS_NATIVE_INVOCATION_BOUND') {
      return response({
        ok: false, status: 'HOLD', code: 'CURRENT_CANONICAL_XCAF_V2_HOLD', release: 'HOLD',
      }, 422);
    }
    return response({
      ok: true,
    ...
    return response({
      ok: false, status: 'HOLD', code: 'INTERNAL_ERROR', release: 'HOLD',
    }, 500);
  }
}
