...
describe('IFC semantic roundtrip route', () => {
  it('returns release evidence without echoing source', async () => {
    ...
    expect(response.status).toBe(200); expect(json).toMatchObject({ ok: true, roundtripPassed: true, releaseReady: false, sourceReturned: false, quoteOrRfqSideEffects: false });
