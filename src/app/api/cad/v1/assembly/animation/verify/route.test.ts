...
describe('CAD v1 animation verification recovery contract', () => {
  ...
  it('uses mesh separation and motion bounds to clear an AABB-overlapping rotational interval',async()=>{
    ...
    const lBody={id:'l-body',name:'l-body',dependencies:[],payload:{kind:'extrude',loop:[{x:0,y:0},{x:30,y:0},{x:30,y:10},{x:10,y:10},{x:10,y:30},{x:0,y:30}],depth:10,direction:'one_sided',mode:'add'}};
    ...
    const body={state,animation,frameStep:10,rotationalMaxDepth:0,localBoxes:{l:{min:{x:0,y:0,z:0},max:{x:30,y:30,z:10}},pin:{min:{x:0,y:0,z:0},max:{x:10,y:10,z:10}}},featureTrees:{l:{nodes:[lBody]},pin:{nodes:[extrude('pin-body',0,10,0,10)]}}};
  ...
  it('removes an exact swept-AABB false positive only after continuous mesh-distance proof',async()=>{
    ...
    const body={state,animation,frameStep:10,jointEvidence:untrustedNativeJointEvidence,localBoxes:{l:{min:{x:0,y:0,z:0},max:{x:30,y:30,z:10}},pin:{min:{x:0,y:0,z:0},max:{x:10,y:10,z:10}}},featureTrees:{l:{nodes:[lBody]},pin:{nodes:[extrude('pin-body',0,10,0,10)]}}};
  ...
  it('derives motion broad-phase bounds from exact OCCT solids instead of undersized caller boxes', async () => {
    const tree=(id:string)=>({nodes:[{id,name:id,dependencies:[],payload:{kind:'extrude',loop:[{x:0,y:0},{x:10,y:0},{x:10,y:10},{x:0,y:10}],depth:10,direction:'one_sided',mode:'add'}}]});
    const state={parts:[
      {id:'a',name:'a',partTemplateId:'a',position:{x:0,y:0,z:0},orientation:IDENTITY_QUAT},
      {id:'b',name:'b',partTemplateId:'b',position:{x:5,y:0,z:0},orientation:IDENTITY_QUAT},
    ],mates:[]};
    const animation={version:1,name:'static collision',fps:30,startFrame:0,endFrame:1,tracks:[]};
    const tiny={min:{x:0,y:0,z:0},max:{x:1,y:1,z:1}};
    const response=await POST(new NextRequest('http://localhost/api/cad/v1/assembly/animation/verify',{method:'POST',headers:{'content-type':'application/json','x-forwarded-for':'animation-derived-box-test'},body:JSON.stringify({state,animation,localBoxes:{a:tiny,b:tiny},featureTrees:{a:tree('a-body'),b:tree('b-body')}})}));
    const json=await response.json();
    expect(json.broad.frames.some((frame:{pairs:unknown[]})=>frame.pairs.length>0)).toBe(true);
    expect(json.precise).toMatchObject({status:'completed',collisionFree:false,geometryErrors:[]});
    expect(Object.keys(json.precise.exactCadEvidence)).toEqual(['a','b']);
    expect(json.releaseReady).toBe(false);
  });

  it('requires no caller mesh boxes when exact FeatureTrees are present and returns the review target hash', async () => {
    const tree={nodes:[{id:'body',name:'body',dependencies:[],payload:{kind:'extrude',loop:[{x:0,y:0},{x:10,y:0},{x:10,y:10},{x:0,y:10}],depth:10,direction:'one_sided',mode:'add'}}]};
    const state={parts:[{id:'part',name:'part',partTemplateId:'part',position:{x:0,y:0,z:0},orientation:IDENTITY_QUAT}],mates:[]};
    const animation={version:1,name:'exact motion',fps:30,startFrame:0,endFrame:10,tracks:[{id:'track',targetPartId:'part',keyframes:[{frame:0,position:{x:0,y:0,z:0}},{frame:10,position:{x:2,y:0,z:0}}]}]};
    const response=await POST(new NextRequest('http://localhost/api/cad/v1/assembly/animation/verify',{method:'POST',headers:{'content-type':'application/json','x-forwarded-for':'animation-no-caller-box-test'},body:JSON.stringify({state,animation,featureTrees:{part:tree}})}));
    const json=await response.json();
    expect(response.status).toBe(200);
    expect(json).toMatchObject({ok:true,releaseReady:false,precise:{status:'completed',collisionFree:true,geometryErrors:[]}});
    expect(json.verificationInputHash).toMatch(/^[a-f0-9]{64}$/);
    expect(Object.keys(json.precise.exactCadEvidence)).toEqual(['part']);
  });
});
