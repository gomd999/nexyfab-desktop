...
const MAX_ARCHITECTURE_VERIFY_BODY_BYTES = 16 * 1024 * 1024;
...
export async function POST(req: NextRequest) {
  ...
  type Body = { architecture?: ArchitectureDocument; circulation?: ArchitecturalCirculationModel; rules?: CirculationRules; topologyToleranceMm?: number };
  let body: Body | null;
  try { body = await readBoundedJson<Body>(req, MAX_ARCHITECTURE_VERIFY_BODY_BYTES); }
  ...
  const verificationPassed = topology.releaseReady && circulation.status === 'passed';
  return NextResponse.json({
    ok: true,
    verificationPassed,
    // Geometry/topology checks are local verification evidence; no commercial receipt is present here.
    releaseReady: false,
