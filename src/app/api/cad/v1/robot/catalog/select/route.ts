...
export async function POST(req: NextRequest) {
  ...
  const multipart = await readBoundedMultipartForm(req, 257_000_000); if (multipart.tooLarge) return NextResponse.json({ ok: false, code: 'TOO_LARGE' }, { status: 413 });
  const form = multipart.form; const requirements = form?.get('requirements'); const manifest = form?.get('manifest'); const artifacts = form?.getAll('artifact') ?? [];
