...
export async function POST(req: NextRequest) {
  ...
  const form = await req.formData().catch(() => null); const program = form?.get('program'), revision = form?.get('revision'), packetFile = form?.get('packet'), reviewFile = form?.get('review'), housing = form?.get('housing'), requirements = form?.get('requirements'), manifest = form?.get('manifest'), artifacts = form?.getAll('artifact') ?? [];
  ...
  try {
    ...
    const zip = zipSync({ [result.manifest.programArtifact]: result.programBytes, [`revision-${result.manifest.lineageId}-r${result.manifest.revision}-${result.manifest.programHash}.json`]: result.manifestBytes, [`integration-application-${result.targetHash}.json`]: strToU8(`${JSON.stringify(receipt, null, 2)}\n`) }, { level: 6 });
    return new NextResponse(Buffer.from(zip), { status: 200, headers: { 'content-type': 'application/zip', 'content-disposition': `attachment; filename="robot-cad-r${result.manifest.revision}-${result.manifest.programHash}.zip"`, 'x-nexyfab-target-hash': result.targetHash, 'x-nexyfab-program-hash': result.manifest.programHash, 'x-nexyfab-revision': String(result.manifest.revision), 'x-nexyfab-cad-applied-workspace': 'false', 'x-nexyfab-release-ready': 'false' } });
