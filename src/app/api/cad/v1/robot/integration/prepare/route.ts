...
export async function POST(req: NextRequest) {
  ...
  const multipart = await readBoundedMultipartForm(req, 310_000_000); if (multipart.tooLarge) return NextResponse.json({ ok: false, code: 'TOO_LARGE' }, { status: 413 });
  const form = multipart.form; const program = form?.get('program'); const revision = form?.get('revision'); const housing = form?.get('housing'); const requirements = form?.get('requirements'); const manifest = form?.get('manifest'); const artifacts = form?.getAll('artifact') ?? [];
