...
const MAX_MEP_ROUTE_BODY_BYTES = 4 * 1024 * 1024;

type Body = { request?: MepRouteRequest; drainRules?: { minimumSlopePercent: number; maximumSlopePercent: number }; fitting?: { diameterMm: number; bendRadiusMm: number }; penetration?: { outsideDiameterMm: number; radialClearanceMm: number; hosts: PenetrationHost[]; sleeveWallThicknessMm?: number; firestopAnnulusMm?: number } };
...
export async function POST(req: NextRequest) {
  ...
  try {
    body = await readBoundedJson<Body>(req, MAX_MEP_ROUTE_BODY_BYTES);
  } catch (error) {
  ...
  try {
    ...
    const routeVerificationPassed = route.status === 'routed' && (!body.fitting || fittings.status === 'passed') && (!body.penetration || penetrations.status === 'passed');
    const releaseGate = unrunPrecisionManufacturingRelease('piping');
    return NextResponse.json({ ok: true, routeVerificationPassed, releaseReady: releaseGate.releaseReady, releaseGate, route, fittings, penetrations, serviceOpenings, quoteOrRfqSideEffects: false });
