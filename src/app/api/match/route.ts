...
export async function GET(req: NextRequest) {
  ...
  if (apiKey && matches.length > 1) {
    try {
      ...
      const prompt = `You are a manufacturing partner matching expert.

RFQ: "${inquiryForMatch.request_field || '(no description)'}"
Budget: "${inquiryForMatch.budget_range || 'not specified'}"

Candidates (already keyword-scored):
${JSON.stringify(candidateList, null, 2)}

Re-rank these candidates by semantic fit to the RFQ. Consider:
- Manufacturing capability alignment
- Budget fit
- Experience and track record

Return ONLY a JSON array of objects in new rank order:
[{ "id": "partner_id", "semanticScore": 0-100, "reason": "one sentence why" }]
