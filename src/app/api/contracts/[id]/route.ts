...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (partnerEmail !== undefined) {
    setClauses.push('partner_email = ?');
    if (partnerEmail === null || partnerEmail === '') setVals.push(null);
    else {
      const pe = normPartnerEmail(partnerEmail);
      setVals.push(pe.length ? pe : null);
    }
  }
...
<p>계약 ID: ${id} / 프로젝트: ${updated!.project_name} / 최종 수수료: ${(updated!.final_charge ?? 0).toLocaleString('ko-KR')}원</p>`,
  ...
  if (status && status !== prevStatus) {
    const customerEmail = updated!.customer_email;
    const partnerEmail  = updated!.partner_email;
    const projectName   = updated!.project_name;
    const portalLink    = `${SITE_URL}/portal?contractId=${id}`;
