...
export async function POST(req: NextRequest) {
  ...
  const existing = await db.queryOne<{
    id: string; services: string; password_hash: string | null; email: string; plan: string; stage: string | null;
  }>(
    'SELECT id, services, password_hash, email, plan, stage FROM nf_users WHERE email = ?', email,
  );
    ...
    const accessToken = await signJWT(
      {
        ...
        emailVerified: true,
        service,
        nexyfabStage: parseUserStageColumn(existing.stage),
      },
      15 * 60,
    );
  ...
  const accessToken = await signJWT(
    { sub: userId, email, plan: 'free', emailVerified: false, service, nexyfabStage: 'A' },
    15 * 60,
  );

  return NextResponse.json({
    user: { id: userId, email, name: displayName, plan: 'free', emailVerified: false, nexyfabStage: 'A' },
    accessToken,
