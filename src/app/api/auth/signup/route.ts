...
export async function POST(req: NextRequest) {
  ...
  if (!AUTH_URL) {
    ...
    const existingUser = await db.queryOne<{ id: string; services: string; password_hash: string | null }>(
      'SELECT id, services, password_hash FROM nf_users WHERE email = ?', email,
    );
    ...
    if (existingUser) {
      ...
      const token = await signJWT({ sub: existingUser.id, email, plan: 'free', emailVerified: true, service: SERVICE_NAME });
      const linkedResponse = NextResponse.json({
        user: { id: existingUser.id, email, name: displayName, plan: 'free', projectCount: 0, emailVerified: true },
        linked: true,
    ...
    const token = await signJWT({ sub: id, email, plan: 'free', emailVerified: false, service: SERVICE_NAME });
