...
export async function GET() {
    ...
    // API 키가 없거나 두 데이터 모두 실패 → 스텁 반환 (가짜 데이터 금지)
    if (aluminum.length === 0 && copper.length === 0) {
        return NextResponse.json({
            ...
            cached: false,
            stale: true,
            reason: apiKey ? 'external_api_failure' : 'no_api_key',
        });
    }

    // 한쪽만 실패한 경우 실패한 쪽은 빈 배열
    ...
    writeCache(result);
    return NextResponse.json({ ...result, cached: false, stale: false });
}
