...
function buildUnsubUrl(email: string): string {
  const secret = process.env.UNSUBSCRIBE_SECRET || process.env.JWT_SECRET || 'nexyfab-unsub-secret';
  ...
  for (const user of d1Users) {
    ...
    try {
      await enqueueJob('send_email', {
        to: user.email,
        subject,
        html: dripD1Html(name, lang, buildUnsubUrl(user.email)),
      });
      await db.execute(
  ...
  for (const user of d7Users) {
    ...
    try {
      await enqueueJob('send_email', {
        to: user.email,
        subject,
        html: dripD7Html(name, lang, buildUnsubUrl(user.email)),
      });
      await db.execute(
