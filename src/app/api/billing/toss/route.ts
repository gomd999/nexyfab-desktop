...
export async function PUT(req: NextRequest) {
  ...
  if (
    ...
    // ── Subscription invoice (nf_aw_invoices) ────────────────────────────────
    await db.execute(
  ...
  // Handle cancellation / failure for manufacturing orders
  if (
    event.eventType === 'PAYMENT_STATUS_CHANGED' &&
    event.data.status &&
    ['CANCELED', 'ABORTED', 'EXPIRED'].includes(event.data.status) &&
    event.data.orderId?.startsWith('NF-')
  ) {
    await db.execute(
      "UPDATE nf_orders SET payment_status = 'failed' WHERE toss_order_id = ? AND payment_status IN ('pending', 'processing')",
      event.data.orderId,
    ).catch(() => {});
  }
