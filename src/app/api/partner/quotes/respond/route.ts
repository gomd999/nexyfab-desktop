...
export async function POST(req: NextRequest) {
  ...
  const quote = await db.queryOne<{
    ...
    partner_email: string | null; status: string;
  }>('SELECT id, inquiry_id, project_name, partner_email, status FROM nf_quotes WHERE id = ?', quoteId);

  if (!quote) {
    return NextResponse.json({ error: '견적을 찾을 수 없습니다.' }, { status: 404 });
  }
  if (quote.partner_email !== partner.email) {
