...
export async function POST(req: NextRequest) {
  ...
  const contractRow = await db.queryOne<{ id: string; attachments: string | null }>(
    'SELECT id, attachments FROM nf_contracts WHERE id = ? AND partner_email = ?',
    contractId, partner.email,
  );
  if (!contractRow) {
  ...
  let attachments: any[];
  try { attachments = JSON.parse(contractRow.attachments || '[]'); } catch { attachments = []; }
  ...
  if (version > 1) {
    const prev = [...attachments]
      .filter((a: any) => a.originalName === originalName)
  ...
  // Async virus scan (non-blocking)
  import('@/lib/virus-scan').then(({ scanBuffer }) =>
    scanBuffer(buffer, originalName).then(result => {
      if (!result.skipped && !result.clean) {
        console.error(`[virus-scan] INFECTED: ${originalName} uploaded by ${partner.email}, positives: ${result.positives}`);
      }
    })
  ).catch(() => {});
    ...
    try {
      ...
      await db.execute(
        `INSERT INTO nf_files (id, user_id, storage_key, filename, mime_type, size_bytes, category, ref_type, ref_id, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        randomUUID(), partner.partnerId, fileKey, originalName,
...
export async function DELETE(req: NextRequest) {
  ...
  let attachments: any[];
  try { attachments = JSON.parse(contractRow.attachments || '[]'); } catch { attachments = []; }
  const attIdx = attachments.findIndex((a: any) => a.id === attachmentId);
  ...
  const newAttachments = attachments.filter((_: any, i: number) => i !== attIdx);
  await db.execute(
    'UPDATE nf_contracts SET attachments = ?, updated_at = ? WHERE id = ?',
