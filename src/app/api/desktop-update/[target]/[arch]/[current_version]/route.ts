...
export async function GET(
  ...
  const downloadUrl = release
    ...
    : `${FALLBACK_BASE}/${latestVersion}/${target.includes('windows') ? `NexyFab_${latestVersion}_x64_ko-KR.msi` : target.includes('darwin') ? `NexyFab_${latestVersion}_${target.includes('aarch64') ? 'aarch64' : 'x64'}.dmg` : `nexyfab_${latestVersion}_amd64.AppImage`}`;
