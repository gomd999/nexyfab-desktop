    ...
    try {
      ...
      for (let attempt = 1; !r.ok && r.raw && attempt <= 2; attempt++) {
        ...
        const fixPrompt = tooLarge
          ...
          : `The program failed to render in OpenSCAD with this error:\n${r.raw.slice(0, 800)}\nRewrite the model from scratch using ONLY plain OpenSCAD primitives (cube, cylinder, sphere, translate, rotate, scale, hull, difference, union, for). Do NOT use BOSL2 (no cuboid/cyl/rounding=/attach/anchor).${escalate ? ' Keep it simple — basic boxes and cylinders only, no rounding.' : ''} Keep the same Customizer parameter variables, comments and groups so the sliders still work.`;
        try {
