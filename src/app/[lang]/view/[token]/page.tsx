...
export default function ViewTokenPage({ params }: { params: Promise<{ lang: string; token: string }> }) {
  ...
  const setupScene = useCallback(async (canvas: HTMLCanvasElement, data: ShareData) => {
    ...
    try {
      ...
      const meshData = JSON.parse(json) as { positions: number[]; normals?: number[]; indices?: number[] };
      geometry = new THREE.BufferGeometry();
      geometry.setAttribute('position', new THREE.Float32BufferAttribute(meshData.positions, 3));
      if (meshData.normals?.length) geometry.setAttribute('normal', new THREE.Float32BufferAttribute(meshData.normals, 3));
      if (meshData.indices?.length) geometry.setIndex(meshData.indices);
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <a href={`/${lang}/shape-generator`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 0 }}>
