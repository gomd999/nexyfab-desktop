...
export default function MarketplacePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  let filtered = manufacturers.filter(m => {
    if (q && !m.name.toLowerCase().includes(q) && !m.nameKo.includes(q) &&
        !m.description.toLowerCase().includes(q) && !m.descriptionKo.includes(q)) return false;
    ...
    if (certFilter.length > 0 && !certFilter.every(c => m.certifications.includes(c))) return false;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <a href={`/${lang}/shape-generator`} style={{ fontSize: 18, fontWeight: 800, color: '#e6edf3', textDecoration: 'none' }}>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              color: '#e6edf3', fontSize: 13, width: 200,
            }}
          />
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px', display: 'flex', gap: 24 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!loading && !error && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {paginated.map(m => (
                <ManufacturerCard key={m.id} manufacturer={m} isKo={isKo} lang={lang} />
              ))}
            </div>
          )}
...
function ManufacturerCard({ manufacturer: m, isKo, lang }: {
  manufacturer: Manufacturer; isKo: boolean; lang: string;
}) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <a
          href={`/${lang}/nexyfab/manufacturers/${m.id}`}
          style={{
          ...
          onMouseEnter={e => { (e.target as HTMLAnchorElement).style.borderColor = '#8b949e'; (e.target as HTMLAnchorElement).style.color = '#e6edf3'; }}
          onMouseLeave={e => { (e.target as HTMLAnchorElement).style.borderColor = '#30363d'; (e.target as HTMLAnchorElement).style.color = '#8b949e'; }}
        >
