...
export function AiDesignWorkspaceSurface({
  ...
  const workspaceCopy = getAiDesignWorkspaceCopy(lang);
  const text = workspaceCopy.surface;
  ...
  return (
    <main className={styles.workspace} data-testid="ai-design-v10-workspace" dir={langDir(lang)}>
      <header className={styles.header}>
        ...
        <div className={styles.revision} aria-label={text.serverRevision}>
      ...
      <section className={styles.chatRail} aria-label={text.conversationAria}>
        ...
        <div className={styles.authority}>
          ...
          <span>{text.exactCadBoundary}</span>
        </div>
      ...
      <section className={styles.canvasArea} aria-label={text.linkedCanvas}>
        <div className={styles.canvasToolbar}>
          <div role="group" aria-label={text.canvasModeAria}>
        ...
        <div className={`${styles.canvasStage} ${mode === 'split' ? styles.split : ''}`}>
          {(mode === '2d' || mode === 'split') && (
            <div className={styles.viewPane} data-testid="concept-2d-view">
              <span className={styles.viewLabel}>2D · {text.conceptView}</span>
              <svg viewBox="0 0 720 420" role="img" aria-label={text.linked2dAria}>
          ...
          {(mode === '3d' || mode === 'split') && (
            <div className={styles.viewPane} data-testid="concept-3d-view">
              <span className={styles.viewLabel}>3D · {text.conceptView}</span>
              {renderThree
                ? <ConceptCanvas3d nodes={nodes} selectedId={selection?.id ?? null} onSelect={onSelect} />
                : <div className={styles.canvasLoading}>{text.rendererTestBoundary}</div>}
            </div>
          )}
          {!nodes.length && <div className={styles.emptyCanvas}>{text.emptyCanvas}</div>}
        </div>
        <div className={styles.syncBar}>
          <span>{text.linkedSelection}: {selectedNode?.label ?? '—'}</span>
          <span>{mappingNeedsInput ? text.precisionMappingRequired : text.stableIdMappingReady}</span>
          <span>{text.preview}: {preview ? text.previewNotRun : controller.preview.status}</span>
        </div>
      </section>
      ...
      <aside id="ai-design-inspector" className={`${styles.inspector} ${inspectorOpen ? styles.inspectorOpen : ''}`} aria-label={text.inspector}>
        ...
        <section>
          <h3>{text.model}</h3>
          <dl><dt>{text.identifier}</dt><dd>{workspace.model.publicModelId ?? text.notSelected}</dd><dt>{text.status}</dt><dd>{workspace.model.selectionStatus}</dd></dl>
        ...
        <section>
          ...
          <div className={styles.segmented} role="group" aria-label={text.gaugeStepAria}>
            <button type="button" aria-pressed={gaugeMode === 'fine'} onClick={() => onGaugeMode('fine')}>{text.fine}</button>
            <button type="button" aria-pressed={gaugeMode === 'coarse'} onClick={() => onGaugeMode('coarse')}>{text.coarse}</button>
        ...
        <section>
          ...
          <dl><dt>{text.geometry}</dt><dd>{workspaceCopy.launcher.notRun}</dd><dt>{text.precision}</dt><dd>{precision.status}</dd><dt>{text.requests}</dt><dd>{precision.requestIds.length}</dd><dt>{text.release}</dt><dd>{text.no}</dd></dl>
        </section>
      </aside>
    </main>
  );
