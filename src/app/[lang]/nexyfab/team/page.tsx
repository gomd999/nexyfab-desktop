...
function TeamDashboardInner({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      if (res.ok) { setInviteMsg(data.message ?? (isKo ? '초대 이메일을 발송했습니다.' : 'Invite sent.')); setInviteEmail(''); await fetchMembers(selectedTeam.id); }
      else setInviteMsg(data.error ?? 'Error');
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      } else setTransferMsg(data.error ?? 'Error');
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {isOwner && (
                          <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 10, padding: '16px 20px' }}>
                            ...
                            {inviteMsg && <p style={{ margin: '8px 0 0', fontSize: 12, color: inviteMsg.includes('error') || inviteMsg.includes('Error') ? '#f85149' : '#3fb950' }}>{inviteMsg}</p>}
                          </div>
                        )}
                        ...
                        <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 10, padding: '20px 24px' }}>
                          ...
                          {transferableMembers.length === 0 ? (
                            ...
                            <>
                              ...
                              {transferMsg && <p style={{ margin: '8px 0 0', fontSize: 12, color: transferMsg.includes('Error') || transferMsg.includes('error') ? '#f85149' : '#3fb950' }}>{transferMsg}</p>}
                            </>
                          )}
                        </div>
