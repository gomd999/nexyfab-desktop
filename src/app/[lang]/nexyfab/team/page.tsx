    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--nx-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--nx-text)' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--nx-text-3)' }}>{isKo ? '로그인이 필요합니다' : 'Sign in required'}</p>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--nx-bg)', color: 'var(--nx-text)', padding: '60px 32px', textAlign: 'center' }}>
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          <p style={{ marginTop: 16, color: 'var(--nx-text-3)', fontSize: 12 }}>
            {isKo ? '현재 플랜: ' : 'Current plan: '}<strong style={{ color: 'var(--nx-text-2)' }}>{user.plan.toUpperCase()}</strong>
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--nx-bg)', fontFamily: 'system-ui, -apple-system, sans-serif', color: 'var(--nx-text)' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--nx-panel-2)', padding: '16px 32px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, background: 'var(--nx-bg)', zIndex: 10 }}>
        ...
        <span style={{ fontSize: 14, color: 'var(--nx-text-3)' }}>{isKo ? '팀 대시보드' : 'Team Dashboard'}</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px' }}>
        {loadingTeams ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--nx-text-3)' }}>{isKo ? '불러오는 중...' : 'Loading...'}</div>
        ) : (
          <>
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div className="team-sidebar" style={{ width: 220, flexShrink: 0 }}>
              <div style={{ background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, overflow: 'hidden' }}>
                ...
                {allTeams.length === 0 ? (
                  <div style={{ padding: '16px 14px', fontSize: 12, color: 'var(--nx-text-3)', textAlign: 'center' }}>{isKo ? '팀이 없습니다' : 'No teams yet'}</div>
                ) : allTeams.map(team => {
            ...
            {selectedTeam ? (
              <div className="team-main" style={{ flex: 1, minWidth: 0 }}>
                ...
                {loadingContent ? (
                  <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--nx-text-3)' }}>{isKo ? '불러오는 중...' : 'Loading...'}</div>
                ) : (
                  <>
                    {/* ═══ OVERVIEW ═══════════════════════════════════════ */}
                    {activeTab === 'overview' && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
                        ...
                        <div style={{ gridColumn: '1 / -1', background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, padding: '16px 20px' }}>
                          <h3 style={{ margin: '0 0 12px', fontSize: 12, color: 'var(--nx-text-3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{isKo ? '최근 팀 활동' : 'Recent Team Activity'}</h3>
                          {activityLogs.slice(0, 6).length === 0 ? (
                            <p style={{ color: 'var(--nx-text-3)', fontSize: 13, margin: 0 }}>{isKo ? '아직 활동 기록이 없습니다.' : 'No activity yet.'}</p>
                            ...
                            return (
                              <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid var(--nx-panel-2)' }}>
                                ...
                                <span style={{ fontSize: 11, color: 'var(--nx-text-3)', whiteSpace: 'nowrap' }}>{timeAgo(log.createdAt, isKo)}</span>
                              </div>
                            );
                          })}
                    ...
                    {activeTab === 'members' && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div style={{ background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, overflow: 'hidden' }}>
                          ...
                          {members.length === 0 ? (
                            <div style={{ padding: '28px 20px', textAlign: 'center', color: 'var(--nx-text-3)', fontSize: 13 }}>{isKo ? '아직 팀원이 없습니다.' : 'No members yet.'}</div>
                          ) : members.map(member => (
                            <div key={member.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', borderBottom: '1px solid var(--nx-panel-2)' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {member.joined_at && <p style={{ margin: 0, fontSize: 11, color: 'var(--nx-text-3)' }}>{isKo ? '가입: ' : 'Joined: '}{new Date(member.joined_at).toLocaleDateString(isKo ? 'ko-KR' : 'en-US')}</p>}
                              </div>
                            ...
                            {pendingInvites.map(inv => (
                              <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', borderBottom: '1px solid var(--nx-panel-2)' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <p style={{ margin: 0, fontSize: 11, color: 'var(--nx-text-3)' }}>{isKo ? '만료: ' : 'Exp: '}{new Date(inv.expires_at).toLocaleDateString(isKo ? 'ko-KR' : 'en-US')}</p>
                                </div>
                                {isOwner && (
                      ...
                      <div>
                        ...
                        {teamProjects.length === 0 ? (
                          <div style={{ textAlign: 'center', padding: '80px 0', background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10 }}>
                            ...
                            <p style={{ color: 'var(--nx-text-3)', fontSize: 13 }}>{isKo ? '아직 팀 프로젝트가 없습니다.' : 'No team projects yet.'}</p>
                          </div>
                        ) : (
                            ...
                            {teamProjects.map(p => (
                              ...
                              >
                                ...
                                <div style={{ padding: '10px 12px' }}>
                                  ...
                                  <p style={{ margin: '2px 0 0', fontSize: 10, color: 'var(--nx-text-3)' }}>{new Date(p.updatedAt).toLocaleDateString(isKo ? 'ko-KR' : 'en-US')}</p>
                                </div>
                              </div>
                            ))}
                          </div>
                    ...
                    {activeTab === 'activity' && (
                      <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 12, color: 'var(--nx-text-3)' }}>{isKo ? '필터:' : 'Filter:'}</span>
                        ...
                        <div style={{ background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, overflow: 'hidden' }}>
                          ...
                          {filteredActivity.length === 0 ? (
                            <div style={{ padding: '48px 20px', textAlign: 'center', color: 'var(--nx-text-3)', fontSize: 13 }}>{isKo ? '활동 기록이 없습니다.' : 'No activity recorded.'}</div>
                            ...
                            return (
                              <div key={log.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 20px', borderBottom: idx < filteredActivity.length - 1 ? '1px solid var(--nx-panel-2)' : 'none' }}>
                                ...
                                <span style={{ fontSize: 11, color: 'var(--nx-text-3)', whiteSpace: 'nowrap', paddingTop: 2 }}>{timeAgo(log.createdAt, isKo)}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {activeTab === 'settings' && isOwner && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div style={{ background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, padding: '20px 24px' }}>
                          ...
                          <p style={{ margin: '0 0 14px', fontSize: 12, color: 'var(--nx-text-3)' }}>{isKo ? '팀의 표시 이름을 변경합니다.' : 'Change how your team is displayed.'}</p>
                        ...
                        <div style={{ background: 'var(--nx-panel)', border: '1px solid var(--nx-border)', borderRadius: 10, padding: '20px 24px' }}>
                          ...
                          <p style={{ margin: '0 0 14px', fontSize: 12, color: 'var(--nx-text-3)' }}>{isKo ? '소유권을 다른 팀원에게 이전합니다. 이전 후 본인은 매니저로 유지됩니다.' : 'Transfer ownership to another member. You will remain as manager.'}</p>
                          {transferableMembers.length === 0 ? (
                            <p style={{ margin: 0, fontSize: 13, color: 'var(--nx-text-3)' }}>{isKo ? '이전 가능한 팀원이 없습니다.' : 'No eligible members to transfer to.'}</p>
                          ) : (
                            <>
                              <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <div style={{ background: 'var(--nx-panel)', border: '1px solid #6e2020', borderRadius: 10, padding: '20px 24px' }}>
                          ...
                          <p style={{ margin: '0 0 16px', fontSize: 12, color: 'var(--nx-text-2)' }}>{isKo ? '팀을 삭제하면 모든 팀원이 제거되고 복구할 수 없습니다.' : 'Deleting a team removes all members and cannot be undone.'}</p>
                          {!deleteConfirm ? (
