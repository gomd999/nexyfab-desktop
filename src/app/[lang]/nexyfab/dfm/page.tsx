  ...
  async function runCheck() {
    ...
    try {
      ...
      if (Object.keys(numericParams).length === 0) {
        setError(copy('최소 1개 이상의 파라미터를 입력하세요.', 'Enter at least one parameter.'));
        return;
      }
  ...
  return (
    <main style={{ maxWidth: 880, margin: '0 auto', padding: '32px 20px', color: '#0f172a' }}>
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>
        {copy('제조 적합성 자동 검증 (DFM)', 'DFM Auto-Check')}
      </h1>
      ...
      <section style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 20, marginBottom: 20 }}>
          ...
          {FIELDS.map(f => (
            <label key={f.key} style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 13 }}>
              <span style={{ fontWeight: 600, color: '#334155' }}>
                {copy(f.labelKo, f.labelEn)} <span style={{ color: '#94a3b8', fontWeight: 400 }}>({f.unit})</span>
              </span>
              <input
                type="number"
                step={f.step}
        ...
        <div style={{ marginTop: 14 }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 13 }}>
            <span style={{ fontWeight: 600, color: '#334155' }}>
              {copy('CAD 파일 ID (선택)', 'CAD File ID (optional)')}
            </span>
            <input
              type="text"
              value={fileId}
              onChange={e => setFileId(e.target.value)}
              placeholder={copy('파일 함에서 복사한 ID', 'ID copied from your files')}
              style={{ padding: '8px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 14 }}
            />
          </label>
        </div>
        ...
        >
          {loading ? copy('검증 중…', 'Checking…') : copy('DFM 검증 실행', 'Run DFM Check')}
        </button>
        {error && (
      ...
      {result && (
        <section>
          <div style={{ display: 'flex', gap: 16, marginBottom: 16, fontSize: 14 }}>
            <Stat label={copy('오류', 'Errors')}   value={result.issues}   color="#dc2626" />
            <Stat label={copy('경고', 'Warnings')} value={result.warnings} color="#d97706" />
            {result.id && (
              <span style={{ marginLeft: 'auto', color: '#64748b', fontSize: 12 }}>
                {copy('저장됨: ', 'Saved as ')}<code>{result.id}</code>
              </span>
            )}
          </div>
          ...
          {result.items.length === 0 ? (
            <div style={{ padding: 20, background: '#dcfce7', color: '#166534', borderRadius: 10, fontSize: 14 }}>
              {copy('✓ 감지된 이슈 없음 — 제조 적합성 양호합니다.', '✓ No issues found — design looks manufacturable.')}
            </div>
          ) : (
              ...
              {result.items.map((it, i) => {
                ...
                return (
                  <li key={i} style={{
                    ...
                    <strong style={{ minWidth: 48 }}>{levelLabel(it.level, lang)}</strong>
...
function ActionPanel({ lang, issues, warnings, pending, done, onChoose }: ActionPanelProps) {
  const copy = (ko: string, en: string) => createCommercialLocalizer(lang)(ko, en);
  // 결과 등급에 따라 버튼 구성을 분기.
  ...
  if (grade === 'pass') {
    choices.push({
      ...
      label: copy('매칭 의뢰 →', 'Proceed to Matching →'),
    });
  } else if (grade === 'warn') {
    choices.push({
      action: 'proceed_to_match', primary: true,
      label: copy('그대로 매칭 의뢰 →', 'Match as-is →'),
    });
    choices.push({
      action: 'revise', primary: false,
      label: copy('수정 후 재검증', 'Revise & re-check'),
    });
  } else {
    choices.push({
      action: 'request_expert', primary: true,
      label: copy('전문가에게 수정 요청', 'Request expert review'),
    });
    choices.push({
      action: 'revise', primary: false,
      label: copy('직접 수정 후 재검증', 'Revise myself'),
    });
  }
  ...
  if (done && done !== 'proceed_to_match') {
    return (
      <div style={{ marginTop: 16, padding: 14, background: '#ecfdf5', color: '#065f46', borderRadius: 10, fontSize: 14 }}>
        {done === 'request_expert'
          ? copy('✓ 전담자가 24시간 내 연락드립니다.', '✓ Our expert team will contact you within 24h.')
          : copy('✓ 의사가 기록되었습니다. 도면 수정 후 다시 검증해 주세요.', '✓ Noted. Revise your design and re-run the check.')}
      </div>
    );
  }
  ...
  return (
    <div style={{ marginTop: 20, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      {choices.map(c => (
        ...
        >
          {pending === c.action ? copy('처리 중…', 'Working…') : c.label}
        </button>
      ))}
    </div>
  );
