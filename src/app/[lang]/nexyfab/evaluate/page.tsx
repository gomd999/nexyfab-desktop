...
export default function EvaluatePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const evaluate = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/nexyfab/evaluate-report', {
        ...
        body: JSON.stringify({ metrics: { ...metrics, mass_g: massG }, material, process, filename, lang }),
      });
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto text-[var(--nx-text,#e6edf3)]">
      ...
      {err && <div className="rounded-lg border border-red-500/40 bg-red-500/10 text-red-300 text-sm p-3 mb-6">{err}</div>}
