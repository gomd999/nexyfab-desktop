...
function won(n: number, lang: string) { return formatMoney(n, lang, 'KRW') ?? ''; }
function fmtDate(ts: number, lang: string) { return formatDate(ts, lang, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) ?? ''; }
...
export default function AdminDashboard({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 403) { setError(L('관리자 권한이 필요합니다.', 'Admin access required.')); return; }
      const d = await r.json() as AdminStats;
      setStats(d);
    } catch {
      setError(L('데이터를 불러오지 못했습니다.', 'Failed to load data.'));
    } finally {
      setLoading(false);
    }
  }, [token, L]);
    ...
    try {
      ...
      if (d.ok) {
        setAppMsg(action === 'approve'
          ? L('✅ 승인 완료. 파트너에게 로그인 코드가 발송됐습니다.', '✅ Approved. Login code sent to partner.')
          : L('✅ 거절 처리됐습니다.', '✅ Rejected.'));
        loadApplications();
      } else {
        setAppMsg(`❌ ${d.error ?? L('처리 실패', 'Action failed')}`);
      }
    } catch {
      setAppMsg(L('❌ 오류가 발생했습니다.', '❌ An error occurred.'));
    }
  }

  const tabs = [
    { id: 'overview' as const, label: L('📊 개요', '📊 Overview') },
    { id: 'applications' as const, label: `📋 ${L('파트너 신청', 'Applications')}${stats?.partners.pendingApplications ? ` (${stats.partners.pendingApplications})` : ''}` },
    { id: 'rfqs' as const, label: L('📝 최근 RFQ', '📝 Recent RFQs') },
    { id: 'orders' as const, label: L('📦 최근 주문', '📦 Recent Orders') },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: '14px 32px', display: 'flex', alignItems: 'center', gap: 14, position: 'sticky', top: 0, background: C.bg, zIndex: 10 }}>
        ...
        <span style={{ fontSize: 14, color: C.muted }}>{L('어드민 대시보드', 'Admin Dashboard')}</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: C.muted }}>{L('데이터 로딩 중...', 'Loading...')}</div>
        )}
        ...
        {!loading && stats && (
          <>
            ...
            {activeTab === 'overview' && (
              <>
                <h2 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: C.muted }}>{L('플랫폼 현황', 'Platform Overview')}</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 14, marginBottom: 32 }}>
                  <StatCard label={L('전체 RFQ', 'Total RFQs')} value={stats.rfq.total} sub={L(`대기 ${stats.rfq.pending} · 견적완료 ${stats.rfq.quoted}`, `Pending ${stats.rfq.pending} · Quoted ${stats.rfq.quoted}`)} color={C.accent} />
                  <StatCard label={L('전체 주문', 'Total Orders')} value={stats.orders.total} sub={L(`진행 중 ${stats.orders.active} · 납품완료 ${stats.orders.delivered}`, `Active ${stats.orders.active} · Delivered ${stats.orders.delivered}`)} color={C.orange} />
                  <StatCard label={L('이번달 매출', 'Monthly Revenue')} value={won(stats.revenue.month, lang)} sub={L(`이번주 ${won(stats.revenue.week, lang)}`, `This week ${won(stats.revenue.week, lang)}`)} color={C.green} />
                  <StatCard label={L('활성 파트너', 'Active Partners')} value={stats.partners.active} sub={L(`전체 제조사 ${stats.partners.total}`, `Total manufacturers ${stats.partners.total}`)} color={C.purple} />
                  <StatCard label={L('전체 회원', 'Total Users')} value={stats.users.total} sub={L(`이번주 신규 +${stats.users.newThisWeek}`, `New this week +${stats.users.newThisWeek}`)} color={C.yellow} />
                  {stats.partners.pendingApplications > 0 && (
                    <StatCard label={L('대기 중 신청', 'Pending Applications')} value={stats.partners.pendingApplications} sub={L('파트너 승인 필요', 'Partner approval required')} color={C.red} />
                  )}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '18px 20px' }}>
                    <h3 style={{ margin: '0 0 14px', fontSize: 13, fontWeight: 700, color: C.muted }}>{L('RFQ 상태 분포', 'RFQ Status')}</h3>
                    {[
                      { label: L('대기(pending)', 'Pending'), val: stats.rfq.pending, color: C.yellow },
                      { label: L('견적완료(quoted)', 'Quoted'), val: stats.rfq.quoted, color: C.accent },
                      { label: L('수락(accepted)', 'Accepted'), val: stats.rfq.accepted, color: C.green },
                      { label: L('기타', 'Other'), val: Math.max(0, stats.rfq.total - stats.rfq.pending - stats.rfq.quoted - stats.rfq.accepted), color: C.dim },
                    ].map(item => (
                  ...
                  <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: '18px 20px' }}>
                    <h3 style={{ margin: '0 0 14px', fontSize: 13, fontWeight: 700, color: C.muted }}>{L('주문 상태 분포', 'Order Status')}</h3>
                    {[
                      { label: L('접수(placed)', 'Placed'), val: stats.orders.placed, color: C.accent },
                      { label: L('진행 중', 'Active'), val: stats.orders.active, color: C.orange },
                      { label: L('납품완료', 'Delivered'), val: stats.orders.delivered, color: C.green },
                    ].map(item => (
                      <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: item.color, flexShrink: 0 }} />
                        <span style={{ fontSize: 12, color: C.muted, flex: 1 }}>{item.label}</span>
                ...
                <div style={{ marginTop: 24, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {[
                    { label: L('📧 이메일 로그', '📧 Email Logs'), href: `/${lang}/nexyfab/admin/email-logs` },
                    { label: L('📄 이메일 템플릿', '📄 Email Templates'), href: `/${lang}/nexyfab/admin/email-templates` },
                    { label: L('🔗 RFQ 매칭', '🔗 RFQ Matching'), href: `/${lang}/nexyfab/admin/rfq-matching` },
                    { label: L('📥 실견적 입력', '📥 Real quotes'), href: `/${lang}/nexyfab/admin/quotes` },
                  ].map(link => (
            ...
            {activeTab === 'applications' && (
              <div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                  {['pending', 'approved', 'rejected'].map(s => (
                    <button key={s} onClick={() => loadApplications(s)} style={{
                      ...
                      {s === 'pending' ? L('대기 중', 'Pending') : s === 'approved' ? L('승인됨', 'Approved') : L('거절됨', 'Rejected')}
                    </button>
                  ))}
                </div>
                {appMsg && (
                ...
                {appLoading && <div style={{ color: C.dim, textAlign: 'center', padding: 40 }}>{L('로딩 중...', 'Loading...')}</div>}
                {!appLoading && apps.length === 0 && (
                  <div style={{ textAlign: 'center', padding: 60, color: C.dim, fontSize: 13 }}>{L('신청 내역이 없습니다.', 'No applications found.')}</div>
                )}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          <span style={{
                            ...
                            {app.status === 'pending' ? L('대기', 'Pending') : app.status === 'approved' ? L('승인', 'Approved') : L('거절', 'Rejected')}
                          </span>
                        </div>
                        ...
                        <div style={{ fontSize: 10, color: C.dim, marginTop: 4 }}>{fmtDate(app.createdAt, lang)}</div>
                      </div>
                      {app.status === 'pending' && (
            ...
            {activeTab === 'rfqs' && (
              <div>
                <h2 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: C.muted }}>{L(`최근 RFQ (${stats.recentRfqs.length}건)`, `Recent RFQs (${stats.recentRfqs.length})`)}</h2>
                {stats.recentRfqs.length === 0 && <div style={{ color: C.dim, textAlign: 'center', padding: 40 }}>{L('RFQ 없음', 'No RFQs')}</div>}
                {stats.recentRfqs.map(rfq => (
                  <div key={rfq.id} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: '12px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.dim }}>{rfq.user_email ?? '—'} · {fmtDate(rfq.created_at, lang)}</div>
                    </div>
            ...
            {activeTab === 'orders' && (
              <div>
                <h2 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: C.muted }}>{L(`최근 주문 (${stats.recentOrders.length}건)`, `Recent Orders (${stats.recentOrders.length})`)}</h2>
                {stats.recentOrders.length === 0 && <div style={{ color: C.dim, textAlign: 'center', padding: 40 }}>{L('주문 없음', 'No orders')}</div>}
                {stats.recentOrders.map(order => (
                  <div key={order.id} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: '12px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.dim }}>{order.manufacturer_name} · {fmtDate(order.created_at, lang)}</div>
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 700, color: C.green }}>{won(order.total_price_krw, lang)}</span>
