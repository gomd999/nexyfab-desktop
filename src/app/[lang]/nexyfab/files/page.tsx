...
export default function FilesPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/nexyfab/files?${qs}`);
  ...
  async function handleDelete(fileId: string) {
    ...
    setDeleteError('');
    try {
      const res = await fetch(`/api/nexyfab/files?id=${fileId}`, { method: 'DELETE' });
      if (!res.ok) {
    ...
    <div style={{ padding: '32px', maxWidth: 960, margin: '0 auto', color: '#c9d1d9' }}>
      ...
      {deleteError && (
        <div style={{
          marginBottom: 16, padding: '10px 14px', background: 'rgba(248,81,73,0.1)',
          border: '1px solid rgba(248,81,73,0.3)', borderRadius: 8,
          fontSize: 13, color: '#f85149', display: 'flex', alignItems: 'center', gap: 8,
        }}>
          <span style={{ flex: 1 }}>{deleteError}</span>
          <button onClick={() => setDeleteError('')} style={{ background: 'none', border: 'none', color: '#f85149', cursor: 'pointer', fontSize: 14 }}>✕</button>
        </div>
      )}
