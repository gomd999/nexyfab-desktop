...
import { PRECISION_CAD_AGENT_COPY, PRECISION_CAD_LIFECYCLE_COPY, PRECISION_CAD_RESULT_COPY } from './PrecisionCadAgentPanel';
...
describe('Precision CAD agent i18n', () => {
  it('covers every supported locale for controls, status, artifacts, and exact-promotion truth', () => {
    ...
    expect(Object.keys(PRECISION_CAD_LIFECYCLE_COPY).sort()).toEqual([...locales].sort());
    for (const locale of locales) {
      ...
      expect(PRECISION_CAD_LIFECYCLE_COPY[locale].phases).toHaveLength(6);
      expect(PRECISION_CAD_LIFECYCLE_COPY[locale].recovery.length).toBeGreaterThan(20);
    }
  });
});
