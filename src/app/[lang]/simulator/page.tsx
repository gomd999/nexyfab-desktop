...
function SimulatorPageInner() {
    ...
    const [vizTab2, setVizTab2] = useState(0);
    const [spInput, setSpInput] = useState(0);
    ...
    useEffect(() => {
        setSpInput(Math.round(simData.finalPrice * 1.3 / 1000) * 1000);
    }, [simData.finalPrice]);
    ...
    const exportToExcel = () => {
        ...
        const csv = [
            '# Summary Overview',
            rowsToCsv(summaryData),
            '',
            '# Cost Analysis',
            rowsToCsv(analysisData),
            '',
            '# BOM Details',
            rowsToCsv(currentComponentsReport),
            '',
            '# Market Config',
            rowsToCsv(marketData),
        ].join('\r\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `Global_Manufacturing_Cost_Report_${timestamp}.csv`;
        a.click();
...
`}} />
            ...
            <style precedence="default" href="simulator-chart">{`
