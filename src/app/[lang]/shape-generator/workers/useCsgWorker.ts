...
export function useCsgWorker() {
  ...
  useEffect(() => {
    try {
      ...
      // Worker creation can fail in SSR or unsupported environments — that is
      // fine; performCSG will fall back to synchronous execution.
  ...
  /** Cancel any in-flight CSG work and terminate the worker. */
