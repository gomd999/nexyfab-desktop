...
describe('immutable manufacturing package v3', () => {
  it('hashes every pre-manifest artifact and binds release evidence', async () => {
    ...
    await exportManufacturingZipBundle(new THREE.BoxGeometry(10, 10, 10), 'part', {
      ...
      releaseEvidence: { revisionManifestSha256: sha, kernelStackIdentitySha256: sha, kernelEvidenceSha256: sha, drawingStatus: 'pass', pmiStatus: 'verified', workflowStatus: 'manufacturing_or_construction_approved' },
  ...
  it('labels a handoff without release evidence as blocked instead of manufacturing-ready', async () => {
    downloaded.length = 0;
    await exportManufacturingZipBundle(new THREE.BoxGeometry(1, 1, 1), 'legacy', {
    ...
    const zip = unzipSync(new Uint8Array(await downloaded[0]!.blob.arrayBuffer()));
    const manifest = JSON.parse(strFromU8(zip['legacy-manufacturing.json']!));
    expect(manifest.releaseDecision.status).toBe('blocked');
    expect(manifest.releaseDecision.blockers).toContain('release-evidence-missing-or-invalid');
  });
