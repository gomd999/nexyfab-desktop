...
export function hex8ResponseSpectrum(grid: TopologyGrid, opts: ResponseSpectrumOptions): ResponseSpectrumResult {
  ...
  const empty: ResponseSpectrumResult = {
    perMode: [], baseShearSRSS: 0, baseShearSum: 0, peakDispByFreeDof: new Float64Array(0),
  ...
  for (const m of modes) {
    ...
    // peak modal displacement u_i = Γ·φ_i·S_d, accumulated in SRSS (sum of squares).
    const scale = gamma * Sd;
    for (let j = 0; j < nFree; j++) { const ui = scale * m.vector[j]!; peakDisp[j] += ui * ui; }
  }
  for (let j = 0; j < nFree; j++) peakDisp[j] = Math.sqrt(peakDisp[j]);
