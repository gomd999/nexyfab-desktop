 ...
 * primitives, run boolean ops, tessellate back to THREE.BufferGeometry.
 *
 * Design notes
 * ─────────────
...
export async function ensureOcctReady(): Promise<void> {
  ...
  initPromise = (async () => {
    ...
    const factoryOpts = typeof window !== 'undefined'
      ? { locateFile: (p: string) => (p.endsWith('.wasm') ? '/replicad_single.wasm' : p) }
      : undefined;
...
export function occtBoxBooleanWithPrimitive(
  ...
  // Convert to THREE.BufferGeometry. mesh.vertices is flat [x,y,z,...],
