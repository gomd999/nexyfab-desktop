 ...
 *
 *   (featureType + params + sketchData + upstreamGeoId)
 *
 * so that a parameter slider tick on a late feature doesn't force re-running
 * every upstream feature from scratch. Each BufferGeometry we hand out gets a
...
export function featureCacheKey(
  ...
  return `${upstreamGeoId}::${f.type}::${hashParams(f.params)}::${hashSketchData(f)}`;
}
