...
export const moldToolsFeature: FeatureDefinition = {
  ...
  apply(geometry, params) {
    ...
    try {
      ...
      if (pullAxis === 1) {
        const yOffset = operation === 1 ? splitVal + half : splitVal - half;
        cutBox.translate(0, yOffset, 0);
      } else if (pullAxis === 0) {
        const xOffset = operation === 1 ? splitVal + half : splitVal - half;
        cutBox.translate(xOffset, 0, 0);
      } else {
        const zOffset = operation === 1 ? splitVal + half : splitVal - half;
        cutBox.translate(0, 0, zOffset);
      }
