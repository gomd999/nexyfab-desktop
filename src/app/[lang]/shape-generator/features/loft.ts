...
/** One closed cross-section ring (SEGS points, CCW) for a loft section.
 *  shapeType: 0 circle, 1 square, 2 triangle. `rot` applies the twist. */
...
function loftSections(params: Record<string, number>) {
  ...
  for (let s = 0; s <= sections; s++) {
    ...
    rings.push({ points: getRingPoints(shapeType, size, rot), y });
  }
  return { rings, height };
}

function applyLoftMesh(params: Record<string, number>): THREE.BufferGeometry {
  const { rings, height } = loftSections(params);
  const verts: number[] = [];
  const idx: number[] = [];

  for (const ring of rings) {
    for (const [px, pz] of ring.points) verts.push(px, ring.y, pz);
  }

  const sections = rings.length - 1;
  for (let s = 0; s < sections; s++) {
    for (let i = 0; i < SEGS; i++) {
      const a = s * SEGS + i;
