...
export interface ConstraintSolveResult {
  ...
  /** Final residual norm */
  residual?: number;
  /** Constraints whose removal would not change rank — safe to drop */
  redundant?: string[];
}
...
function buildResiduals(
  ...
  for (const c of constraints) {
    switch (c.type) {
      ...
      case 'distance': {
        // Variant A: point-to-point (two point ids)
        // Variant B: point-to-line  (point id + line segment id)
        const a = c.entityIds[0];
        const b = c.entityIds[1];
        if (!a || !b) break;
        const target = c.value ?? 0;
        const segB = segById.get(b);
        if (segB && segB.type === 'line') {
          const le = lineEnds(segB.id);
          if (!le) break;
          add(c.id, x => {
            const P = readPt(a, x, vars);
            const L0 = readPt(le.aId, x, vars);
            const L1 = readPt(le.bId, x, vars);
            if (!P || !L0 || !L1) return 0;
            ...
            const signed = ((P.x - L0.x) * dy - (P.y - L0.y) * dx) / len;
  ...
  // Resolve parametric expressions (e.g. "D2 = 2*D1 + 10") once per solve.
  const dimTargets = resolveDimensionTargets(dimensions);

  // Driving dimensions (locked only)
  for (const d of dimensions) {
    if (!d.locked) continue;
    const resolved = dimTargets.get(d.id) ?? d.value;
    switch (d.type) {
      case 'linear': {
      ...
      case 'diameter': {
        ...
        if (!ci) break;
        const target = d.type === 'diameter' ? resolved / 2 : resolved;
        add(d.id, x => {
      ...
      case 'angular': {
        // Angle between two line segments, value in degrees.
        const s1 = d.entityIds[0] ? segById.get(d.entityIds[0]) : undefined;
        const s2 = d.entityIds[1] ? segById.get(d.entityIds[1]) : undefined;
        if (!s1 || !s2 || s1.type !== 'line' || s2.type !== 'line') break;
        const e1 = lineEnds(s1.id);
        const e2 = lineEnds(s2.id);
        if (!e1 || !e2) break;
        const target = (resolved * Math.PI) / 180;
        const cosT = Math.cos(target);
        add(d.id, x => {
          const A0 = readPt(e1.aId, x, vars);
          const A1 = readPt(e1.bId, x, vars);
...
/**
 * Identify redundant residual rows. A row is "redundant" if removing it does
 * not change the row rank of J — i.e. the constraint is already implied by
 * others. Uses leave-one-out rank probing. O(m² · n²); cheap for typical
 * sketches (<100 constraints).
 *
 * Groups rows by constraint id so a multi-row constraint (e.g. coincident
 * contributes 2 rows) is reported only if ALL its rows are collectively
 * redundant.
 */
function findRedundantConstraints(
  J: Float64Array,
  m: number,
  n: number,
  sourceIds: string[],
  ...
  if (m === 0 || n === 0) return [];
  const fullRank = estimateRank(J, m, n);
  if (fullRank >= m) return []; // no redundancy

  // Group row indices by constraint id.
  ...
  for (const [id, rows] of groups) {
    // Build J' without `rows` and compare rank.
    const mPrime = m - rows.length;
    if (mPrime === 0) continue;
    const skip = new Set(rows);
    const Jp = new Float64Array(mPrime * n);
    let w = 0;
    for (let i = 0; i < m; i++) {
      if (skip.has(i)) continue;
      for (let j = 0; j < n; j++) Jp[w * n + j] = J[i * n + j];
      w++;
    }
    const r = estimateRank(Jp, mPrime, n);
    if (r >= fullRank) redundant.push(id);
  }
  return redundant;
}
...
export function solveConstraints(
  ...
  if (lastJ && n > 0) {
    ...
    dof = n - rank;
    if (m > rank) redundant = findRedundantConstraints(lastJ, m, n, sourceIds);
  }
  ...
  if (err <= tolerance && unsatisfied.length === 0) {
    if (redundant.length > 0) {
      status = 'over-defined';
      message = `${redundant.length} constraint(s) are redundant — removal would not break the sketch`;
    } else {
  ...
  return {
    ...
    solveResult: {
      ...
      dof,
      residual: err,
      redundant: redundant.length > 0 ? redundant : undefined,
      conflicts: status === 'over-defined' && unsatisfied.length > 0
        ? unsatisfied.map(id => `constraint ${id}`)
        : undefined,
    },
  };
}
