...
const dict = {
  ko: {
    ...
    toolTrim: '자르기', toolSelect: '선택', toolDimension: '치수', toolConstraint: '구속',
    filletApplied: (r: number) => `Fillet r=${r}mm 적용`,
    mirrorPivotSet: '미러 기준점 설정. 다시 클릭해 X축 / Shift+클릭해 Y축 미러',
    mirroredAxis: (axis: string) => `${axis}축 미러 완료`,
    toConstructionNormal: '보조선 → 일반선으로 변환',
    toConstruction: '보조선으로 변환 (형상 생성 시 제외)',
    axisHint: '클릭=X축 / Shift+클릭=Y축',
    angleLocked: '각도고정',
    ...
    points: '점',
    esc: 'ESC 취소',
    clickToStart: '클릭하여 스케치 시작',
    selectToolThenClick: '도구를 선택하고 캔버스를 클릭하세요',
    qtLine: '/ 선', qtCircle: '○ 원', qtRect: '□ 사각', qtPolygon: '⬡ 다각',
    orSelect3D: '또는 좌측 패널에서 3D 형상 선택',
  },
  en: {
    ...
    filletApplied: (r: number) => `Fillet r=${r}mm applied`,
    mirrorPivotSet: 'Mirror pivot set. Click again for X-mirror, Shift+Click for Y-mirror',
    mirroredAxis: (axis: string) => `Mirrored about ${axis} axis`,
    toConstructionNormal: 'Construction → Normal line',
    toConstruction: 'Set as construction line (excluded from shape)',
    axisHint: 'Click=X / Shift+Click=Y',
    angleLocked: 'angle lock',
    ...
    points: 'pt',
    esc: 'ESC to cancel',
    clickToStart: 'Click to start sketching',
    selectToolThenClick: 'Select a tool and click the canvas',
    qtLine: '/ Line', qtCircle: '○ Circle', qtRect: '□ Rect', qtPolygon: '⬡ Poly',
    orSelect3D: 'Or pick a 3D shape from the left panel',
  },
  ja: {
    ...
    filletApplied: (r: number) => `Fillet r=${r}mm 適用`,
    mirrorPivotSet: 'ミラー基準点設定。再クリックでX軸 / Shift+クリックでY軸ミラー',
    mirroredAxis: (axis: string) => `${axis}軸ミラー完了`,
    toConstructionNormal: '補助線 → 通常線に変換',
    toConstruction: '補助線に変換 (形状生成から除外)',
    axisHint: 'クリック=X軸 / Shift+クリック=Y軸',
    angleLocked: '角度固定',
    ...
    points: '点',
    esc: 'ESCでキャンセル',
    clickToStart: 'クリックしてスケッチを開始',
    selectToolThenClick: 'ツールを選択してキャンバスをクリック',
    qtLine: '/ 線', qtCircle: '○ 円', qtRect: '□ 矩形', qtPolygon: '⬡ 多角',
    orSelect3D: 'または左パネルから3D形状を選択',
  },
  zh: {
    ...
    filletApplied: (r: number) => `Fillet r=${r}mm 已应用`,
    mirrorPivotSet: '已设镜像基准点。再次点击=X轴镜像, Shift+点击=Y轴镜像',
    mirroredAxis: (axis: string) => `已沿${axis}轴镜像`,
    toConstructionNormal: '辅助线 → 普通线',
    toConstruction: '设为辅助线 (几何生成时排除)',
    axisHint: '点击=X轴 / Shift+点击=Y轴',
    angleLocked: '角度锁定',
    ...
    esc: 'ESC 取消',
    clickToStart: '点击开始绘制',
    selectToolThenClick: '选择工具然后点击画布',
    qtLine: '/ 线', qtCircle: '○ 圆', qtRect: '□ 矩形', qtPolygon: '⬡ 多边',
    orSelect3D: '或从左侧面板选择3D形状',
  },
  es: {
    ...
    filletApplied: (r: number) => `Fillet r=${r}mm aplicado`,
    mirrorPivotSet: 'Pivote de espejo fijado. Clic otra vez para X, Shift+Clic para Y',
    mirroredAxis: (axis: string) => `Reflejado sobre eje ${axis}`,
    toConstructionNormal: 'Auxiliar → Línea normal',
    toConstruction: 'Convertir a auxiliar (excluido de la geometría)',
    axisHint: 'Clic=X / Shift+Clic=Y',
    angleLocked: 'ángulo fijo',
    ...
    points: 'pt',
    esc: 'ESC cancelar',
    clickToStart: 'Clic para empezar el boceto',
    selectToolThenClick: 'Selecciona una herramienta y haz clic en el lienzo',
    qtLine: '/ Línea', qtCircle: '○ Círc', qtRect: '□ Rect', qtPolygon: '⬡ Pol',
    orSelect3D: 'O escoge una forma 3D en el panel izquierdo',
  },
  ar: {
    ...
    filletApplied: (r: number) => `تم تطبيق Fillet r=${r}mm`,
    mirrorPivotSet: 'تم تعيين محور المرآة. انقر مرة أخرى للمحور X / Shift+نقر للمحور Y',
    mirroredAxis: (axis: string) => `تم الانعكاس حول المحور ${axis}`,
    toConstructionNormal: 'مساعد → خط عادي',
    toConstruction: 'تحويل إلى خط مساعد (مستبعد من الشكل)',
    axisHint: 'نقر=X / Shift+نقر=Y',
    angleLocked: 'قفل الزاوية',
    ...
    points: 'نقطة',
    esc: 'ESC للإلغاء',
    clickToStart: 'انقر لبدء الرسم',
    selectToolThenClick: 'اختر أداة ثم انقر على اللوحة',
    qtLine: '/ خط', qtCircle: '○ دائرة', qtRect: '□ مستطيل', qtPolygon: '⬡ مضلع',
    orSelect3D: 'أو اختر شكل 3D من اللوحة اليسرى',
  },
} as const;
...
const SKETCH_MSG = {
  ...
  /** Toast when a second closed loop is auto-routed to a new hole profile (avoids bridge glitch). */
  autoHoleProfile: {
    ko: '안쪽 윤곽은 새 프로파일(홀)로 추가했습니다. 외곽+원을 한 프로파일에 넣으면 잘못 이어질 수 있어요.',
    en: 'Inner loop added as a new profile (hole). Don’t stack two closed loops in one profile — they would connect with a bad edge.',
    ja: '内側の輪郭を新しいプロファイル(穴)に追加しました。外周+円を1つのプロファイルに重ねると誤接続します。',
    cn: '已将内圈添加为新轮廓（孔）。不要把两个闭合圈放在同一轮廓里，否则会出现错误连线。',
    es: 'Bucle interior añadido como perfil nuevo (agujero). No apiles dos contornos cerrados en un solo perfil.',
    ar: 'تمت إضافة الحلقة الداخلية كملف جديد (ثقب). لا تضع حلقتين مغلقتين في ملف واحد.',
  },
  otherProfileLegend: {
    ko: '회색 점선 = 다른 프로파일(윤곽을 클릭하면 그 프로파일로 전환). 왼쪽 탭으로도 전환. 파란 선 = 지금 편집 중.',
    en: 'Gray dashed = other profiles (click an outline to switch to it, or use the left tabs). Blue = active profile.',
    ja: '灰色の破線 = 他プロファイル（輪郭クリックで切替・左タブでも可）。青 = 編集中。',
    cn: '灰色虚线 = 其他轮廓（点击轮廓切换，或用左侧标签）。蓝色 = 当前编辑。',
...
function mergeOrStartHoleProfile(
  profile: SketchProfile,
  newLoopSegs: SketchSegment[],
  onProfileChange: (p: SketchProfile) => void,
  onAddClosedLoopAsNewProfile?: (p: SketchProfile) => void,
  onHoled?: () => void,
): void {
  if (profile.closed && profile.segments.length > 0 && onAddClosedLoopAsNewProfile) {
    onAddClosedLoopAsNewProfile({ segments: newLoopSegs, closed: true });
    onHoled?.();
    return;
  }
  onProfileChange({ segments: [...profile.segments, ...newLoopSegs], closed: true });
}
...
interface SketchCanvasProps {
  ...
  /** When the active profile is already closed, a new circle/rect/etc. is sent here instead of concatenating segments (prevents bogus bridges between loops). */
  onAddClosedLoopAsNewProfile?: (profile: SketchProfile) => void;
  /** Same length as `otherProfiles`: sketch index of each reference outline (for click-to-switch). */
  otherProfileSketchIndices?: number[];
  /** When the click is nearer to another profile's edge than the active one, switch to that profile. */
  onSelectSketchProfileIndex?: (sketchIndex: number) => void;
  onToolChange?: (tool: SketchTool) => void;
  // Grid visibility
  showGrid?: boolean;
  /** When false (default), hide center Line/Circle/Rect/Poly shortcuts — left & top toolbars already expose them. */
  showFloatingSketchShortcuts?: boolean;
  /** Optional parent control (e.g. floating sketch palette). */
  paletteGridVisible?: boolean;
  onPaletteGridChange?: (v: boolean) => void;
  paletteSnapEnabled?: boolean;
  onPaletteSnapChange?: (v: boolean) => void;
  hideFloatingControlBar?: boolean;
  showConstraintOverlay?: boolean;
  showDimensionOverlay?: boolean;
  /** Underlay image (sketch tracing). Object URL from parent; pointer-events none. */
  referenceImage?: {
    href: string;
    opacity: number;
    scale?: number;
  ...
  /** When true, draw a vertical slice-plane guide (2D preview only). */
  sliceGuideActive?: boolean;
  /** World X (mm) for vertical slice guide; default 60. */
  sliceGuidePlaneMm?: number;
  /** New line segments use construction geometry when not normal. */
  sketchLineStyle?: 'normal' | 'construction' | 'centerline';
  /** Increment to trigger fit-to-view (Look at sketch plane). */
  lookAtNonce?: number;
...
function SketchCanvas({
  ...
  onAddConstraint, onRemoveConstraint, onAddDimension,
  selectedConstraintType = 'horizontal',
  otherProfiles = [],
  onAddClosedLoopAsNewProfile,
  otherProfileSketchIndices,
  onSelectSketchProfileIndex,
  onToolChange,
  showGrid = true,
  showFloatingSketchShortcuts = false,
  paletteGridVisible,
  onPaletteGridChange,
  paletteSnapEnabled,
  onPaletteSnapChange,
  hideFloatingControlBar = false,
  showConstraintOverlay = true,
  showDimensionOverlay = true,
  referenceImage = null,
  sliceGuideActive = false,
  sliceGuidePlaneMm = 60,
  ...
  const pathSeg = pathname?.split('/').filter(Boolean)[0] ?? 'en';
  ...
  // Canvas controls: grid visibility + snap (internal or palette-controlled)
  const [gridVisibleInternal, setGridVisibleInternal] = useState(showGrid);
  const [snapEnabledInternal, setSnapEnabledInternal] = useState(true);
  const gridVisible = paletteGridVisible !== undefined ? paletteGridVisible : gridVisibleInternal;
  const snapEnabled = paletteSnapEnabled !== undefined ? paletteSnapEnabled : snapEnabledInternal;

  const toggleGridVisible = useCallback(() => {
    if (paletteGridVisible !== undefined) onPaletteGridChange?.(!paletteGridVisible);
    else setGridVisibleInternal(v => !v);
  }, [paletteGridVisible, onPaletteGridChange]);

  const toggleSnapEnabled = useCallback(() => {
    if (paletteSnapEnabled !== undefined) onPaletteSnapChange?.(!paletteSnapEnabled);
    else setSnapEnabledInternal(v => !v);
  }, [paletteSnapEnabled, onPaletteSnapChange]);
  ...
  useEffect(() => {
    if (lookAtNonce <= 0) return;
    fitToView();
  }, [lookAtNonce, fitToView]);

  const lineConstructionFlag = sketchLineStyle !== 'normal';
  ...
  const closeProfile = useCallback(() => {
    ...
    if (dist(first, last) > 0.01) {
      const newSegments = [...profile.segments, {
        type: 'line' as const,
        points: [last, first],
        id: genId('seg'),
        ...(lineConstructionFlag ? { construction: true as const } : {}),
      }];
    ...
    setTempPoints([]);
  }, [getFirstPoint, getLastPoint, profile, onProfileChange, lineConstructionFlag]);
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    // Multi-profile: click nearer to another profile's edge → switch (avoids hunting the left tabs)
    if (
      onSelectSketchProfileIndex
      && otherProfiles.length > 0
      && otherProfileSketchIndices
      && otherProfileSketchIndices.length === otherProfiles.length
    ) {
      const thresh = 18 / zoom;
      const activeNear = findNearestSegment(profile.segments, pt, thresh);
      let bestSketchIdx = -1;
      let bestDist = Infinity;
      for (let i = 0; i < otherProfiles.length; i++) {
        const op = otherProfiles[i];
        if (op.segments.length === 0) continue;
        const n = findNearestSegment(op.segments, pt, thresh);
        if (!n) continue;
        const si = otherProfileSketchIndices[i];
      ...
      if (bestSketchIdx >= 0 && (!activeNear || bestDist < activeNear.distance)) {
        onSelectSketchProfileIndex(bestSketchIdx);
        return;
      }
    }
    ...
    if (activeTool === 'line') {
      ...
      if (tempPoints.length === 0) {
        const last = getLastPoint();
        if (last) {
          const newSeg: SketchSegment = {
            type: 'line',
            points: [last, { ...pt, id: genId('p') }],
            id: genId('seg'),
            ...(lineConstructionFlag ? { construction: true } : {}),
          };
    ...
    if (activeTool === 'rect') {
      if (tempPoints.length === 0) {
        ...
        mergeOrStartHoleProfile(profile, rectSegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
        setTempPoints([]);
      }
      return;
    }
    ...
    if (activeTool === 'polygon') {
      if (tempPoints.length === 0) {
        ...
        mergeOrStartHoleProfile(profile, polySegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
        setTempPoints([]);
      }
      return;
    }
    ...
    if (activeTool === 'ellipse') {
      if (tempPoints.length === 0) {
        ...
        mergeOrStartHoleProfile(profile, ellipseSegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
        setTempPoints([]);
      }
      return;
    }
    ...
    if (activeTool === 'slot') {
      if (tempPoints.length === 0) {
        ...
        mergeOrStartHoleProfile(profile, slotSegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
        setTempPoints([]);
      }
      return;
    }
    ...
    if (activeTool === 'fillet') {
      ...
      if (bestPt) {
        ...
        if (newSegs !== profile.segments) {
          ...
          showToast(t.filletApplied(filletRadius));
        }
      }
      return;
    }
    ...
    if (activeTool === 'mirror') {
      if (tempPoints.length === 0) {
        ...
        setTempPoints([]);
        showToast(t.mirroredAxis(axisIsY ? 'Y' : 'X'));
      }
      return;
    }
    ...
    if (activeTool === 'construction') {
      ...
      if (nearest) {
        ...
        showToast(seg.construction ? t.toConstructionNormal : t.toConstruction);
      }
      return;
    }
      ...
      if (nearest) {
        ...
        if (tempPoints.length === 0) {
          if (seg.type === 'circle' || seg.type === 'arc') {
            // Immediate radial/diameter dimension
            const r = dist(seg.points[0], seg.points[1]);
            const rounded = Math.round(r * 10) / 10;
            setPendingDim({
              entityIds: [seg.id || ''],
              type: seg.type === 'circle' ? 'diameter' : 'radial',
              value: seg.type === 'circle' ? rounded * 2 : rounded,
              editValue: String(seg.type === 'circle' ? rounded * 2 : rounded),
              position: { x: pt.x + 10 / zoom, y: pt.y + 10 / zoom },
            });
          ...
          const firstSeg = profile.segments[hoverSegIdx];
          if (firstSeg && firstSeg.type === 'line' && seg.type === 'line') {
            if (firstSeg.id === seg.id) {
              // Clicked same line -> linear
              const len = dist(seg.points[0], seg.points[1]);
              const rounded = Math.round(len * 10) / 10;
              setPendingDim({
                entityIds: [seg.id || ''],
                type: 'linear',
                value: rounded, editValue: String(rounded),
                position: { x: pt.x, y: pt.y },
              });
            } else {
              // Clicked two lines -> angular
              const dx1 = firstSeg.points[1].x - firstSeg.points[0].x;
              const dy1 = firstSeg.points[1].y - firstSeg.points[0].y;
              const dx2 = seg.points[1].x - seg.points[0].x;
              const dy2 = seg.points[1].y - seg.points[0].y;
              const ang1 = Math.atan2(dy1, dx1);
              const ang2 = Math.atan2(dy2, dx2);
              let diff = Math.abs((ang2 - ang1) * 180 / Math.PI);
              if (diff > 180) diff = 360 - diff;
              if (diff > 90) diff = 180 - diff; // Acute angle
              const rounded = Math.round(diff * 10) / 10;
              setPendingDim({
                entityIds: [firstSeg.id || '', seg.id || ''],
                type: 'angular',
                value: rounded, editValue: String(rounded),
                position: { x: pt.x + 10 / zoom, y: pt.y + 10 / zoom },
              });
        ...
        // Clicked empty space while 1 line is selected -> Linear dimension placed at click pos
        const firstSeg = profile.segments[hoverSegIdx];
        if (firstSeg && firstSeg.type === 'line') {
          const len = dist(firstSeg.points[0], firstSeg.points[1]);
          const rounded = Math.round(len * 10) / 10;
          setPendingDim({
            entityIds: [firstSeg.id || ''],
            type: 'linear',
            value: rounded, editValue: String(rounded),
            position: { x: pt.x, y: pt.y },
          });
    ...
    isPanning, profile, activeTool, smartSnap, zoom, tempPoints, pickFilter,
  ...
  }, [panX, panY, activeTool, profile.segments, screenToMm, zoom, pickFilter]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // 숫자 인라인 입력 (line/circle/rect/polygon, 첫 점 찍은 상태)
      const dimToolsActive = (activeTool === 'line' || activeTool === 'circle' || activeTool === 'rect' || activeTool === 'polygon') && tempPoints.length === 1;
      if (dimToolsActive && !e.ctrlKey && !e.altKey && !e.metaKey) {
        if (/^[0-9]$/.test(e.key)) {
          e.preventDefault();
      ...
      if (e.key === 'Enter') {
        ...
        // Circle: typed value = radius (mm)
        if (activeTool === 'circle' && tempPoints.length === 1 && dimInput) {
          const r = parseFloat(dimInput);
          if (r > 0 && isFinite(r)) {
            e.preventDefault();
            const center = tempPoints[0];
        ...
        // Rectangle: typed value = side length (square) — direction follows cursor quadrant
        if (activeTool === 'rect' && tempPoints.length === 1 && dimInput) {
          const side = parseFloat(dimInput);
          if (side > 0 && isFinite(side)) {
            e.preventDefault();
            const c1 = tempPoints[0];
            const sx = mousePos.x >= c1.x ? 1 : -1;
            const sy = mousePos.y >= c1.y ? 1 : -1;
            const c2: SketchPoint = { x: c1.x + sx * side, y: c1.y + sy * side, id: genId('p') };
            const rectSegs = generateRectSegments(c1, c2);
            mergeOrStartHoleProfile(profile, rectSegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
            setTempPoints([]);
            setDimInput('');
        ...
        // Polygon: typed value = circumradius (mm)
        if (activeTool === 'polygon' && tempPoints.length === 1 && dimInput) {
          const r = parseFloat(dimInput);
          if (r > 0 && isFinite(r)) {
            e.preventDefault();
            const center = tempPoints[0];
            const radiusPt: SketchPoint = { x: center.x + r, y: center.y, id: genId('p') };
            const polySegs = generatePolygonSegments(center, radiusPt, polygonSides);
            mergeOrStartHoleProfile(profile, polySegs, onProfileChange, onAddClosedLoopAsNewProfile, () => showToast(L('autoHoleProfile')));
            setTempPoints([]);
            setDimInput('');
            return;
  ...
  }, [tempPoints, tempPoints.length, splinePoints.length, splinePoints, activeTool, onToolChange, showToast, selectedSegIdx, profile, onProfileChange, onAddClosedLoopAsNewProfile, closeProfile, L, dimInput, mousePos, pendingDim, polygonSides]);
  ...
  const sliceX = sliceGuidePlaneMm;
  const sliceGuideOverlay = sliceGuideActive ? (
    <g pointerEvents="none">
      ...
      >
        Slice (preview) · X={sliceGuidePlaneMm.toFixed(0)}
      </text>
    </g>
  ) : null;

  const refScale = referenceImage?.scale ?? 1;
  const refOx = referenceImage?.offsetX ?? 0;
  const refOy = referenceImage?.offsetY ?? 0;
  const cx = vbX + viewBoxW / 2;
  const cy = vbY + viewBoxH / 2;
  const referenceUnderlay = referenceImage?.href ? (
    <g
      pointerEvents="none"
      transform={`translate(${refOx}, ${-refOy})`}
    >
      <g transform={`translate(${cx}, ${cy}) scale(${refScale}) translate(${-cx}, ${-cy})`}>
        <image
  ...
  if (profile.segments.length > 0) {
    ...
    for (const seg of profile.segments) {
      if (seg.type === 'line') {
        ...
        const pts = sampleNurbsSegment(seg, 32);
        for (let i = 1; i < pts.length; i++) pathD += ` L ${pts[i].x} ${-pts[i].y}`;
        for (const cp of seg.points) allPoints.push(cp);
      }
    }
  ...
  // ─── Constraint icons ──────────────────────────────────────────────────
  // Render a small glyph badge per (constraint, segment) pair so multi-entity
  // constraints (parallel/perp/equal/concentric/…) are shown on every segment
  // they apply to. Badges on the same segment are stacked so they don't
  // overlap, and pairs are linked by a faint dashed connector. Shift+click a
  // badge to remove the constraint.
  ...
  const iconMap: Record<string, string> = {
    ...
    symmetric: '⇔', midpoint: 'M', fixed: '⊗', angle: '∠',
  };
  const constraintLabel: Record<string, string> = {
    horizontal: 'Horizontal', vertical: 'Vertical', perpendicular: 'Perpendicular',
    parallel: 'Parallel', tangent: 'Tangent', coincident: 'Coincident',
    concentric: 'Concentric', equal: 'Equal', symmetric: 'Symmetric',
    midpoint: 'Midpoint', fixed: 'Fixed', angle: 'Angle',
  };

  // Track how many badges already placed on each segment so we can stack them.
  const badgeStack = new Map<string, number>();

  const segMid = (seg: { points: { x: number; y: number }[] }) => {
    if (seg.points.length === 2) {
      return { x: (seg.points[0].x + seg.points[1].x) / 2, y: (seg.points[0].y + seg.points[1].y) / 2 };
    }
    // Fallback: centroid of all points
    const sx = seg.points.reduce((s, p) => s + p.x, 0) / seg.points.length;
    const sy = seg.points.reduce((s, p) => s + p.y, 0) / seg.points.length;
    return { x: sx, y: sy };
  };

  for (const c of constraints) {
    const segs = c.entityIds
      .map(id => profile.segments.find(s => s.id === id))
      .filter((s): s is typeof profile.segments[0] => !!s && s.points.length > 0);
    if (segs.length === 0) continue;

    const positions = segs.map(segMid);

    // Draw dashed connector between first two segment midpoints (for pair-wise
    // constraints, this shows visually what belongs together).
    ...
    segs.forEach((seg, i) => {
      const { x: mx, y: my } = positions[i];
      const stackKey = seg.id ?? `idx${i}`;
      const stackIdx = badgeStack.get(stackKey) ?? 0;
      badgeStack.set(stackKey, stackIdx + 1);
      const dy = 14 / zoom + stackIdx * (14 / zoom);
      const glyph = iconMap[c.type] || '?';
      const title = `${constraintLabel[c.type] ?? c.type}${c.satisfied ? '' : ' (unsatisfied)'}`;
      constraintIcons.push(
        <g
          key={`${c.id}-${i}`}
          style={{ cursor: onRemoveConstraint ? 'pointer' : 'default' }}
          onClick={(e) => {
            if (e.shiftKey && onRemoveConstraint) {
              e.stopPropagation();
              onRemoveConstraint(c.id);
            }
          }}
        >
          <title>{title}{onRemoveConstraint ? ' — shift+click to remove' : ''}</title>
          <rect
            x={mx - 6 / zoom} y={-my - dy}
  ...
  const otherProfilePaths: React.ReactNode[] = otherProfiles.map((op, opIdx) => {
    ...
    return (
      <g key={`other_${opIdx}`}>
        {/* Halo so dashed outlines stay visible on dark grid / overlaps */}
        <path
          d={d}
          fill="none"
          stroke="rgba(148,163,184,0.45)"
          strokeWidth={sw + 3 / zoom}
        ...
        <path
          d={d}
          fill={op.closed ? 'rgba(129,140,248,0.1)' : 'none'}
          stroke="#b4bcc8"
          strokeWidth={sw}
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0d1117' }}>
      ...
      >
        ...
        {/* Dimension annotations */}
        <g style={{ opacity: showDimensionOverlay ? 1 : 0.12, pointerEvents: showDimensionOverlay ? 'auto' : 'none' as const }}>
        ...
        {/* Dimension inline preview: circle (radius) */}
        {activeTool === 'circle' && tempPoints.length === 1 && dimInput && (() => {
          const r = parseFloat(dimInput);
          if (!(r > 0) || !isFinite(r)) return null;
          const c = tempPoints[0];
          return (
            <g>
              <circle cx={c.x} cy={-c.y} r={r} fill="none" stroke="#e3b341"
                strokeWidth={2 / zoom} strokeDasharray={`${6 / zoom} ${3 / zoom}`} />
              <line x1={c.x} y1={-c.y} x2={c.x + r} y2={-c.y}
                stroke="#e3b341" strokeWidth={1 / zoom} />
            </g>
          );
        })()}

        {/* Dimension inline preview: rect (square side) */}
        {activeTool === 'rect' && tempPoints.length === 1 && dimInput && (() => {
          const s = parseFloat(dimInput);
          if (!(s > 0) || !isFinite(s)) return null;
          const c1 = tempPoints[0];
          const sx = mousePos.x >= c1.x ? 1 : -1;
          const sy = mousePos.y >= c1.y ? 1 : -1;
          const x = sx > 0 ? c1.x : c1.x - s;
          const y = sy > 0 ? -c1.y - s : -c1.y;
          return (
        ...
        {/* Dimension inline preview: polygon (circumradius) */}
        {activeTool === 'polygon' && tempPoints.length === 1 && dimInput && (() => {
          const r = parseFloat(dimInput);
          if (!(r > 0) || !isFinite(r)) return null;
          const c = tempPoints[0];
              ...
              if (e.key === 'Enter') {
                ...
                if (isFinite(v) && v > 0 && onAddDimension) {
                  onAddDimension({
                    ...
                    entityIds: pendingDim.entityIds,
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              onClick={() => {
                ...
                if (isFinite(v) && v > 0 && onAddDimension) {
                  onAddDimension({
                    id: genId('dim'), type: pendingDim.type,
                    entityIds: pendingDim.entityIds,
                    value: Math.round(v * 10) / 10,
      ...
      {profile.segments.length === 0 && tempPoints.length === 0 && !profile.closed && (
        <div style={{
          ...
          {/* Quick tool buttons (optional — hidden by default to reduce overlap with left/top sketch UI) */}
