    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--nx-bg)', color: 'var(--nx-text)', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--nx-panel-2)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        <span style={{
          ...
          background: utilization > 0.7 ? 'var(--nx-ok)22' : '#e3b34122',
          color: utilization > 0.7 ? 'var(--nx-ok)' : '#e3b341',
          border: `1px solid ${utilization > 0.7 ? 'var(--nx-ok)44' : '#e3b34144'}`,
        }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', gap: 0 }}>
        ...
        <div style={{ width: 220, flexShrink: 0, borderRight: '1px solid var(--nx-panel-2)', display: 'flex', flexDirection: 'column' }}>
          {/* Sheet settings */}
          <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--nx-panel-2)' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--nx-text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
              {t.sheetSettings}
            </div>
            {[
              ...
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                ...
                <input
                  ...
                  style={{
                    width: 64, padding: '3px 6px', background: 'var(--nx-panel)', border: '1px solid var(--nx-border)',
                    borderRadius: 4, color: 'var(--nx-text)', fontSize: 11, outline: 'none',
                  }}
                />
              </div>
            ))}
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: 'var(--nx-text-2)', cursor: 'pointer' }}>
              <input
                type="checkbox"
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            <div style={{ padding: '8px 12px 4px', fontSize: 10, fontWeight: 700, color: 'var(--nx-text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'flex', justifyContent: 'space-between' }}>
              <span>{t.parts} ({totalInstances})</span>
              <button onClick={addPart} style={{ background: 'none', border: 'none', color: 'var(--nx-accent)', fontSize: 12, cursor: 'pointer', padding: 0 }}>+ {t.add}</button>
            </div>
            {parts.map((p, pi) => (
              <div key={p.id} style={{ padding: '5px 12px', borderBottom: '1px solid var(--nx-panel-2)', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 9, color: 'var(--nx-text-3)' }}>{p.width}×{p.height}mm ×{p.quantity}</div>
                </div>
                <button onClick={() => setEditingPart({ ...p })} style={{ background: 'none', border: 'none', color: 'var(--nx-text-3)', cursor: 'pointer', fontSize: 11, padding: 0 }}>✏</button>
                <button onClick={() => removePart(p.id)} style={{ background: 'none', border: 'none', color: 'var(--nx-error)', cursor: 'pointer', fontSize: 11, padding: 0 }}>×</button>
              </div>
            ))}
          </div>

          {/* Stats */}
          <div style={{ padding: '8px 12px', borderTop: '1px solid var(--nx-panel-2)', fontSize: 10, color: 'var(--nx-text-3)' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 16, display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ marginTop: 6, fontSize: 10, color: 'var(--nx-text-3)', textAlign: 'center' }}>
              {sheetW} × {sheetH} mm
            </div>
          </div>
        </div>
          ...
          <div style={{
            ...
            {([
              ...
              <div key={key} style={{ marginBottom: 10 }}>
                <label style={{ fontSize: 11, color: 'var(--nx-text-2)', display: 'block', marginBottom: 4 }}>{label}</label>
                <input
                  type={type}
                  ...
                  style={{
                    width: '100%', padding: '5px 8px', background: 'var(--nx-panel)', border: '1px solid var(--nx-border)',
                    borderRadius: 5, color: 'var(--nx-text)', fontSize: 12, outline: 'none', boxSizing: 'border-box',
                  }}
                />
              </div>
            ))}
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              <button
                ...
                style={{ flex: 1, padding: '7px 0', borderRadius: 6, border: 'none', background: 'var(--nx-accent)', color: 'var(--nx-text)', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}
              >
                {t.save}
              </button>
              <button
                onClick={() => setEditingPart(null)}
                style={{ flex: 1, padding: '7px 0', borderRadius: 6, border: '1px solid var(--nx-border)', background: 'transparent', color: 'var(--nx-text-2)', fontSize: 12, cursor: 'pointer' }}
              >
                {t.cancel}
              </button>
            </div>
