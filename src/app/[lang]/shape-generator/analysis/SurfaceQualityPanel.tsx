...
const dict = {
  ...
  es: {
    title: 'Análisis de Curvatura',
    gaussian: 'Gaussiana',
    mean: 'Media',
    maxPrincipal: 'Principal Máx',
    minPrincipal: 'Principal Mín',
    zebra: 'Rayas Cebra',
    draft: 'Análisis de Desmoldeo',
    analyze: 'Analizar',
    results: 'Resultados',
    minCurv: 'Curvatura Mín',
    maxCurv: 'Curvatura Máx',
    flatPct: '% Plano',
    sharpEdges: 'Aristas Vivas',
    stripes: 'Núm. Rayas',
    pullDir: 'Dirección de Tiro',
    minAngle: 'Ángulo Mín',
    close: 'Cerrar',
  },
  ar: {
...
export default function SurfaceQualityPanel({
  ...
  const modeLabel = useCallback(
    (m: CurvatureMode) => {
      const keyMap: Record<CurvatureMode, keyof typeof t> = {
        gaussian: 'gaussian',
        mean: 'mean',
      ...
      <div style={{ padding: '10px 14px' }}>
        ...
        {stats && (
          <div>
            ...
            >
              <span style={{ fontWeight: 600, fontSize: 13 }}>{t.results}</span>
              {qualityScore !== null && (
                <span
                  style={{
                    padding: '2px 10px',
            ...
            >
              {[
                { label: t.minCurv + ' (G)', value: fmt(stats.minGaussian) },
                { label: t.maxCurv + ' (G)', value: fmt(stats.maxGaussian) },
                { label: t.minCurv + ' (M)', value: fmt(stats.minMean) },
                { label: t.maxCurv + ' (M)', value: fmt(stats.maxMean) },
                { label: t.flatPct, value: stats.flatFacePercent.toFixed(1) + '%' },
                { label: t.sharpEdges, value: String(stats.sharpEdgeCount) },
              ].map((s, i) => (
                <div
                  key={i}
                  style={{
