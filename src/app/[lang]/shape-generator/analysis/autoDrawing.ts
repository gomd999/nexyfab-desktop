...
/**
 * 동일 메시에서 도면이 3D와 맞는지 비교할 때 쓰는 가벼운 지문(해시 아님 — UI “도면이 오래됐을 수 있음” 힌트용).
 * 같은 `BufferGeometry`면 안정적으로 동일 문자열을 반환한다.
 */
export function computeDrawingGeometryFingerprint(geometry: THREE.BufferGeometry): string {
  ...
  const iCount = ix ? ix.count : vCount;
  let acc = 0;
  if (pos && vCount > 0) {
    const step = Math.max(1, Math.floor(vCount / 48));
    for (let i = 0; i < vCount; i += step) {
      acc += pos.getX(i) * 1000 + pos.getY(i) * 1001 + pos.getZ(i) * 1002;
    }
  }
  const bbKey = [
    Math.round(bb.min.x * 1000),
    Math.round(bb.min.y * 1000),
    Math.round(bb.min.z * 1000),
    Math.round(bb.max.x * 1000),
    Math.round(bb.max.y * 1000),
    Math.round(bb.max.z * 1000),
  ].join(',');
  return `i:${iCount},v:${vCount},bb:${bbKey},s:${Math.round(acc * 1e6)}`;
}
