/**
 * iso286.ts — ISO 286 표준공차등급(IT)·축 기본편차의 **산식 구현** (260801e).
 *
 * ## 왜 표를 옮겨 적지 않는가
...
const SIZE_BANDS: ReadonlyArray<{ lo: number; hi: number }> = [
...
const IT_MULTIPLIER: Readonly<Record<number, number>> = {
  5: 7, 6: 10, 7: 16, 8: 25, 9: 40, 10: 64, 11: 100,
};

/** 호칭치수가 속한 구간의 기하평균 D. 구간 밖이면 null. */
function bandMean(nominalMm: number): number | null {
...
export function toleranceUnit(nominalMm: number): number | null {
 ...
 * (0 은 「공차 없음」으로 읽힌다).
 */
export function itTolerance(nominalMm: number, grade: number): number | null {
...
export type ShaftDevSymbol = 'e' | 'f' | 'g' | 'h' | 'k' | 'n';
export const FORMULA_SHAFT_SYMBOLS: ReadonlyArray<ShaftDevSymbol> = ['e', 'f', 'g', 'h', 'k', 'n'];
 ...
 */
export function holeFundamentalDeviation(sym: 'H'): { kind: 'EI'; value: number } | null {
