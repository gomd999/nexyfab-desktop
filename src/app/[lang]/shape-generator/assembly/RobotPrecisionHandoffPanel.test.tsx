...
describe('RobotPrecisionHandoffPanel', () => {
  ...
  it('downloads an immutable integration review target without exposing CAD apply', async () => {
    ...
    expect(view.getByTestId('robot-integration-packet').textContent).toContain('J6:motor:old / J6:reducer:old / J6:bearing:old → M / R / B'); expect(view.queryByTestId('robot-integration-apply')).toBeNull();
  ...
  it('shows six-axis selection margins without applying CAD changes or release', async () => {
    ...
    await waitFor(() => expect(view.getByTestId('robot-catalog-selection-report').textContent).toContain('Six-axis selection passed'));
    expect(view.getByTestId('robot-catalog-selection-report').textContent).toContain('J6: M1 + R1 + B1'); expect(view.getByTestId('robot-catalog-selection-report').textContent).toContain('CAD integration not_run · release false');
  ...
  it('validates catalog artifact bytes without activating or persisting the catalog', async () => {
    ...
    expect(view.getByTestId('robot-catalog-report').textContent).toContain('component selection against six-axis requirements has not run');
