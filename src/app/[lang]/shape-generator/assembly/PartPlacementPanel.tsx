...
export default function PartPlacementPanel({ parts, onChange, isKo, currentShapeId, currentParams }: Props) {
  ...
  const handleDuplicate = useCallback((part: PlacedPart) => {
    const dup: PlacedPart = { ...part, id: genId(), name: part.name + ' (복사)', position: [part.position[0] + 10, part.position[1], part.position[2]] };
    onChange([...parts, dup]);
  }, [parts, onChange]);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url; a.download = 'assembly_bom.csv'; a.click();
  ...
  parts.forEach(part => {
    for (let q = 0; q < part.qty; q++) {
      try {
        ...
        if (!result) continue;
        const offset = q * 20; // slight offset for multiple quantities
        const deg2rad = Math.PI / 180;
        results.push({
          name: part.qty > 1 ? `${part.name} #${q + 1}` : part.name,
          result,
          position: [
            (part.position[0] + offset) / 1000,  // mm → scene units (÷1000 not needed, Three.js uses mm scale)
            part.position[1] / 1,
            part.position[2] / 1,
          ],
          rotation: [
