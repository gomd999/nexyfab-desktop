...
{selectionEditPreview&&<div data-testid="solver-assembly-selection-edit-preview" style={{padding:6,background:'var(--nx-panel-2)',borderRadius:4,fontSize:11}}><div><strong>{loc(lang, { ko: '계획', en: 'Plan', ja: '計画', zh: '计划', es: 'Plan', ar: 'الخطة' })}:</strong> {selectionEditPreview.summary}</div><div>{loc(lang, { ko: '작업', en: 'Operations', ja: '操作', zh: '操作', es: 'Operaciones', ar: 'العمليات' })}: {selectionEditPreview.transaction.operations.map(operation=>operation.kind).join(', ')}</div><div>{loc(lang, { ko: '리비전', en: 'Revision', ja: 'リビジョン', zh: '修订版', es: 'Revisión', ar: 'المراجعة' })}: {selectionEditPreview.transaction.baseRevision}</div>{selectionEditPreview.transaction.selection.topology.some(ref=>ref.referenceQuality!=='persistent')&&<label><input type="checkbox" checked={confirmDerivedSelection} onChange={event=>setConfirmDerivedSelection(event.target.checked)}/> {loc(lang, { ko: '파생 위상 참조 확인', en: 'Confirm derived topology reference', ja: '派生トポロジ参照を確認', zh: '确认派生拓扑引用', es: 'Confirmar referencia topológica derivada', ar: 'تأكيد مرجع الطوبولوجيا المشتق' })}</label>}<div style={{display:'flex',gap:5,marginTop:4}}><button type="button" onClick={applySelectionEdit}>{loc(lang, { ko: '원자적으로 적용', en: 'Apply atomically', ja: 'アトミックに適用', zh: '原子应用', es: 'Aplicar atómicamente', ar: 'تطبيق ذري' })}</button><button type="button" onClick={()=>{setSelectionEditPreview(null);setSelectionEditError(null);}}>{loc(lang, { ko: '취소', en: 'Cancel', ja: 'キャンセル', zh: '取消', es: 'Cancelar', ar: 'إلغاء' })}</button></div></div>}
...
{selectionEditUndo&&<button type="button" onClick={undoSelectionEdit}>{loc(lang, { ko: '마지막 선택 편집 실행 취소', en: 'Undo last selection edit', ja: '最後の選択編集を元に戻す', zh: '撤销上次选择编辑', es: 'Deshacer la última edición de selección', ar: 'التراجع عن آخر تحرير للتحديد' })}</button>}
...
<button type="button" data-testid="solver-assembly-add-keyframe" disabled={!selectedPartId} onClick={addSelectedPoseKeyframe}>◆ {loc(lang, { ko: '키프레임', en: 'Keyframe', ja: 'キーフレーム', zh: '关键帧', es: 'Fotograma clave', ar: 'إطار رئيسي' })}</button>
...
<button type="button" onClick={exportAnimation}>{loc(lang, { ko: 'JSON 내보내기', en: 'Export JSON', ja: 'JSON をエクスポート', zh: '导出 JSON', es: 'Exportar JSON', ar: 'تصدير JSON' })}</button><button type="button" onClick={exportAnimationHtml}>{loc(lang, { ko: 'HTML 내보내기', en: 'Export HTML', ja: 'HTML をエクスポート', zh: '导出 HTML', es: 'Exportar HTML', ar: 'تصدير HTML' })}</button><button type="button" onClick={()=>void exportAnimationGlb()}>{loc(lang, { ko: 'GLB 내보내기', en: 'Export GLB', ja: 'GLB をエクスポート', zh: '导出 GLB', es: 'Exportar GLB', ar: 'تصدير GLB' })}</button><button type="button" onClick={()=>animationImportRef.current?.click()}>{loc(lang, { ko: 'JSON 가져오기', en: 'Import JSON', ja: 'JSON をインポート', zh: '导入 JSON', es: 'Importar JSON', ar: 'استيراد JSON' })}</button><input ref={animationImportRef} type="file" accept="application/json,.json" hidden onChange={event=>{const file=event.target.files?.[0];if(file)void importAnimation(file);event.target.value='';}}/>
...
<button type="button" onClick={()=>void applyGroupTransform()}>{loc(lang, { ko: '적용', en: 'Apply', ja: '適用', zh: '应用', es: 'Aplicar', ar: 'تطبيق' })}</button>
<button type="button" disabled={selection.length!==2} onClick={snapSelectedReferences}>{loc(lang, { ko: '참조 스냅', en: 'Snap references', ja: '参照をスナップ', zh: '捕捉引用', es: 'Ajustar referencias', ar: 'محاذاة المراجع' })}</button>
                  </div>
