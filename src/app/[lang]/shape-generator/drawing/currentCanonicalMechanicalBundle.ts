...
export const CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA =
  'nexyfab.precision-cad.current-canonical-mechanical-artifact-bundle.v1' as const;

export interface CurrentCanonicalMechanicalArtifactBundle {
  schema: typeof CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA;
  status: 'EXACT_BUNDLE_PASS';
...
export type CurrentCanonicalMechanicalBundleResult =
  | CurrentCanonicalMechanicalArtifactBundle
  | {
      schema: typeof CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA;
      status: 'HOLD';
      authority: 'SERVER_CURRENT_CANONICAL_HEAD';
...
export interface BuildCurrentCanonicalMechanicalBundleInput {
  db: DbAdapter;
  projectId: string;
  documentId: string;
  now?: Date;
...
export type CurrentCanonicalMechanicalBundleValidation =
  | { ok: true; bundle: CurrentCanonicalMechanicalArtifactBundle }
...
const SOURCE_KEYS = [
  'projectId', 'documentId', 'revision', 'partId', 'featureTreeSha256',
...
function hold(...blockers: string[]): CurrentCanonicalMechanicalBundleResult {
  return {
...
/** Recomputes every bundle-level binding and delegates immutable handoff verification. */
export async function validateCurrentCanonicalMechanicalArtifactBundle(
  value: unknown,
): Promise<CurrentCanonicalMechanicalBundleValidation> {
  try {
    if (!safeRecord(value, BUNDLE_KEYS)
      ...
      return { ok: false, reason: 'CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA_INVALID' };
    }
    const bundle = value as unknown as CurrentCanonicalMechanicalArtifactBundle;
    const revision = bundle.source.revision;
    if (bundle.schema !== CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA
      || bundle.status !== 'EXACT_BUNDLE_PASS'
      ...
      || !SHA256.test(bundle.source.rightsReceiptSha256)
      || Object.values(bundle.artifacts).some(value => !SHA256.test(value))
      || !SHA256.test(bundle.artifactManifestSha256)) {
      return { ok: false, reason: 'CURRENT_CANONICAL_MECHANICAL_BUNDLE_VALUE_INVALID' };
    }
    ...
    if (!validation.ok || !exact || !canonical
      ...
      return { ok: false, reason: 'CURRENT_CANONICAL_MECHANICAL_BUNDLE_BINDING_INVALID' };
    }
    ...
    return manifest === bundle.artifactManifestSha256
      ? { ok: true, bundle }
      : { ok: false, reason: 'CURRENT_CANONICAL_MECHANICAL_BUNDLE_HASH_MISMATCH' };
  } catch {
    return { ok: false, reason: 'CURRENT_CANONICAL_MECHANICAL_BUNDLE_MALFORMED' };
  }
}

/**
 * Current-head-only vertical: canonical FeatureTree -> native OCCT solid ->
 * STEP round-trip -> HLR drawing -> dimension/BOM receipts. It never commits
 * and never upgrades manufacturing release.
 */
export async function buildCurrentCanonicalMechanicalArtifactBundle(
  input: BuildCurrentCanonicalMechanicalBundleInput,
): Promise<CurrentCanonicalMechanicalBundleResult> {
  const current = await readMechanicalCurrentHead(input.db, input.projectId, input.documentId);
  ...
  try {
    handoff = await buildAssemblyDrawingHandoff({
      ...
      state: {
        parts: [{
          id: bound.partId,
          name: bound.name,
          partTemplateId: `canonical:${bound.documentId}:${bound.partId}`,
          fixed: true,
    ...
    return hold('CANONICAL_HANDOFF_BUILD_FAILED');
  }
  const exactResult = await enrichServerDrawingHandoffWithExactSinglePart(handoff);
  if (exactResult.status !== 'PASS') {
    return hold(`EXACT_ENRICHMENT_${exactResult.status}`, exactResult.reason);
  }
  ...
  if (!validation.ok || !exact || !exact.source.canonicalRevision
    ...
    return hold('EXACT_HANDOFF_REVISION_BINDING_INVALID');
  }
  const source = {
    projectId: bound.projectId,
    documentId: bound.documentId,
  ...
  const bundle: CurrentCanonicalMechanicalArtifactBundle = {
    schema: CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA,
    status: 'EXACT_BUNDLE_PASS',
    authority: 'SERVER_CURRENT_CANONICAL_HEAD',
    verification: 'NATIVE_OCCT_STEP_ROUNDTRIP_AND_HLR',
    ...
    source,
    artifacts,
    artifactManifestSha256: sha256({ schema: CURRENT_CANONICAL_MECHANICAL_BUNDLE_SCHEMA, source, artifacts }),
    handoff: validation.handoff,
  };
  const bundleValidation = await validateCurrentCanonicalMechanicalArtifactBundle(bundle);
