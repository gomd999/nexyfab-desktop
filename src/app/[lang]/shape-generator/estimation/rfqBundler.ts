...
/**
 * Build an RFQ zip bundle and trigger a browser download.
 * Returns the Blob for optional further use.
 */
export function downloadRFQBundle(data: RFQBundleData): Blob {
  const enc = new TextEncoder();
  ...
  a.download = `rfq_${safeName}_${new Date().toISOString().slice(0, 10)}.zip`;
  a.click();
