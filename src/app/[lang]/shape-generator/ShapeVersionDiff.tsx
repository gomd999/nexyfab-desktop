...
export default function ShapeVersionDiff({
  ...
  const lA = labelA ?? t(lang, 'before');
  const lB = labelB ?? t(lang, 'after');
  ...
  const { mA, mB, changedCount, volumePct, summaryText } = useMemo(() => {
    ...
    if (Math.abs(volumePct) < 0.05) {
      volText = t(lang, 'volumeUnchanged');
    } else if (volumePct > 0) {
      const byStr = t(lang, 'by');
      volText = `${t(lang, 'volumeIncreased')}${byStr ? ' ' + byStr : ''} ${Math.abs(volumePct).toFixed(1)}%`;
    } else {
      const byStr = t(lang, 'by');
      volText = `${t(lang, 'volumeDecreased')}${byStr ? ' ' + byStr : ''} ${Math.abs(volumePct).toFixed(1)}%`;
    }

    const summaryText = `${changedCount} ${t(lang, 'summary')}, ${volText}`;

    return { mA, mB, changedCount, volumePct, summaryText };
  }, [geometryA, geometryB, lang]);
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#8b949e', marginBottom: 4 }}>{t(lang, 'overlap')}</div>
        ...
        <div style={{ padding: '4px 14px 14px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              <DiffRow label={`${t(lang, 'width')} (${t(lang, 'mm')})`} valA={mA.w} valB={mB.w} unit={t(lang, 'mm')} />
              <DiffRow label={`${t(lang, 'height')} (${t(lang, 'mm')})`} valA={mA.h} valB={mB.h} unit={t(lang, 'mm')} />
              <DiffRow label={`${t(lang, 'depth')} (${t(lang, 'mm')})`} valA={mA.d} valB={mB.d} unit={t(lang, 'mm')} />
              <DiffRow label={`${t(lang, 'volume')} (${t(lang, 'cm3')})`} valA={mA.volume} valB={mB.volume} unit={t(lang, 'cm3')} />
              <DiffRow label={`${t(lang, 'surfaceArea')} (${t(lang, 'cm2')})`} valA={mA.surface} valB={mB.surface} unit={t(lang, 'cm2')} />
              <tr>
        ...
        {Math.abs(volumePct) >= 0.05 && (
          ...
          >
            <div style={{ fontSize: 11, color: '#8b949e', marginBottom: 4 }}>
              {t(lang, 'volume')} Δ {pct(mA.volume, mB.volume)}%
            </div>
            <div
              style={{
                height: 6,
