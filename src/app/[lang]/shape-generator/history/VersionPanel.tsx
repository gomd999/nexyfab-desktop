...
export default function VersionPanel({
  ...
  const isKo = lang === 'ko';
  const labels = {
    title: isKo ? '버전 히스토리' : 'Version History',
    saveSnapshot: isKo ? '스냅샷 저장' : 'Save Snapshot',
    restore: isKo ? '복원' : 'Restore',
    confirmRestore: isKo ? '이 버전으로 복원하시겠습니까?' : 'Restore this version?',
    yes: isKo ? '예' : 'Yes',
    cancel: isKo ? '취소' : 'Cancel',
    delete: isKo ? '삭제' : 'Delete',
    rename: isKo ? '이름 변경' : 'Rename',
    noVersions: isKo ? '저장된 버전이 없습니다' : 'No saved versions',
    params: isKo ? '파라미터 변경' : 'Parameter changes',
    featureChanges: isKo ? '피처 변경' : 'Feature changes',
    features: isKo ? '피처' : 'features',
    compare: isKo ? '비교' : 'Compare',
    compareMode: isKo ? '버전 비교' : 'Compare Versions',
    compareModeHint: isKo ? '비교할 버전 2개 선택' : 'Select 2 versions to compare',
    compareExit: isKo ? '비교 종료' : 'Exit Compare',
    compareSelected: isKo ? '선택됨' : 'selected',
    paramDiff: isKo ? '파라미터 비교' : 'Parameter diff',
    featureDiff: isKo ? '피처 비교' : 'Feature diff',
    newer: isKo ? '새 버전' : 'Newer',
    older: isKo ? '이전 버전' : 'Older',
    noDiff: isKo ? '변경 없음' : 'No changes',
  };
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: theme.textMuted, fontWeight: 600 }}>
            {filteredVersions.length} {isKo ? '개 버전' : `version${filteredVersions.length !== 1 ? 's' : ''}`}
          </div>
        </div>
        <button
          onClick={onClose}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {(diff.paramChanges.length > 0 || diff.featureChanges.length > 0) && (
                        ...
                        >
                          {isExpanded ? '▾ Hide diff' : `▸ ${diff.paramChanges.length + diff.featureChanges.length} changes`}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {compareMode && compareIds.length === 2 && (() => {
        ...
        return (
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 800, color: '#f0883e', marginBottom: 8 }}>⇄ {isKo ? '버전 비교 결과' : 'Version Comparison'}</div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              {[vA, vB].map((v, i) => (
                <div key={v.id} style={{ flex: 1, background: theme.cardBg, borderRadius: 6, padding: '6px 8px', border: `1px solid ${i === 0 ? '#f0883e55' : '#388bfd55'}` }}>
                  ...
                  <div style={{ fontSize: 9, color: theme.textMuted, marginTop: 3 }}>{relativeTime(v.timestamp, lang)}</div>
                </div>
              ))}
            </div>
            {/* Param diff */}
