...
/**
 * Scan a paramExpressions map and return the keys whose expressions reference
 * a name that is not in the available variables list. Used to detect "broken"
 * dependencies after a feature is removed (its params disappear from scope).
 *
 * Returns: { key: string; missing: string[] }[]
 */
export function findBrokenExpressions(
  paramExpressions: Record<string, string>,
  availableVarNames: string[],
): Array<{ key: string; expression: string; missing: string[] }> {
  const availableSet = new Set(availableVarNames);
  const broken: Array<{ key: string; expression: string; missing: string[] }> = [];
  for (const [key, expr] of Object.entries(paramExpressions)) {
    if (!isExpression(expr)) continue;
    const idents = extractIdentifiers(expr);
    const missing = idents.filter(id => !availableSet.has(id));
    if (missing.length > 0) {
      broken.push({ key, expression: expr, missing: Array.from(new Set(missing)) });
    }
  }
  return broken;
}
...
/**
 * Find which paramExpressions keys reference a given target name. Used before
 * removing a feature: if any expressions reference its params, warn the user.
 */
export function findDependentExpressions(
  paramExpressions: Record<string, string>,
  targetNames: string[],
): Array<{ key: string; expression: string; references: string[] }> {
  const targetSet = new Set(targetNames);
  const dependents: Array<{ key: string; expression: string; references: string[] }> = [];
  for (const [key, expr] of Object.entries(paramExpressions)) {
    if (!isExpression(expr)) continue;
    const idents = extractIdentifiers(expr);
    const refs = idents.filter(id => targetSet.has(id));
    if (refs.length > 0) {
      dependents.push({ key, expression: expr, references: Array.from(new Set(refs)) });
    }
  }
  return dependents;
}
