...
1. ~~배포 전환 최종 확인~~ **✅ 완료(01:55)** — 셸 청크 지문 `4B5E6951…` →
   `A4FDD769…` 전환 확인 + smoke 3면 재통과. Wave 4+5 정본(-107)이 라이브.
   (⚠️ `railway redeploy`는 GitHub 롤백 함정 — 금지 유지.)
1b. **G3 직행 검토** — 사용자 질의(260721 "모든 분야에서 쓸 수 있나") 후속:
   기계설계 대체 선언(G2+G3)의 최단 경로 = W5-F mate 실구현 심화 + R5 참조
   재지정 UX. Wave 5 2차분과 우선순위 조율.
