...
1. ~~배포 -111 전환 확인~~ **✅ 완료(13:47)** — 지문 `60DF…`→`1ED8…`, smoke 3면
   통과, `POST /api/nexyfab/design-brief` 미인증 401(라우트+인증 게이트 라이브)·
   GET 405. **design-brief 3면이 프로덕션 라이브.** (인증 실호출·자유텍스트 LLM
   경로는 토큰/프로덕션 AI 설정 필요 — 표면·게이트는 실증.)
