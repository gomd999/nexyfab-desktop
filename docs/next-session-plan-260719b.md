...
## 4. ★서비스 목표와 도메인 커버리지 (사용자 확정 260719b)

**목표: 일반인과 전문가 모두가 기계·건축·토목·조경·인테리어 5개 분야에서 쓰는 설계 서비스.**
→ 앞으로 모든 기능은 **「5도메인 × 2사용자층」 매트릭스**로 갭을 점검한다.

### 도메인 자산 현황(260719b 시점)
| 분야 | 템플릿 | 전문가 산출물 | 판정 |
|---|---|---|---|
| 기계 | 16종+어휘 30종 | 실시도서·구멍표·GD&T·검도 M1~M6·STEP·체결자동 | 최강 |
| 토목(+교량) | 2+5종 | 선형 시트팩·LandXML·IFC4.3·간이구조검토·KDS | 강 |
| 건축 | 7종 | 축선 구조평면·IFC·RC 배근·계산기 | 중상 |
| 인테리어 | 5종 | MEP 급배수 라우팅·단위세대 평면 | 중 |
| 조경 | 2종 | 배치도·등고·관수 체인 | **최약** |

### 일반인 사용자층 갭(전 분야 공통)
1. **진입이 전문가형** — 템플릿 파라미터(“베이 수”·“저판 두께”)를 곧바로 마주함.
   일반인은 “마당에 6×3m 데크” 수준에서 출발해야 함 → **진입 위저드** 필요
2. **산출물 해석 불가** — 도면·검도 리포트뿐, “이걸 들고 뭘 하는지”가 없음
   → **쉬운 요약 1페이지**(자재·주의점·다음 단계) 필요
3. **용어 장벽** — 검도 게이트·GD&T·BOQ 에 쉬운 설명 레이어 부재

### 실행 순서(A→B→C→D)
- **A 조경·인테리어 템플릿 확충**: 조경 5종(펜스·화단옹벽·주차포장·정자·식재)
  + 인테리어 4종(붙박이장·카운터·파티션·천장그리드) — 분야 균형의 실체
- **B 일반인 진입 위저드**: 분야→무엇→크기→확인 4단계, 기존 preset API·생성 경로 재사용
- **C 일반인용 요약 레이어**: 도면집에 `쉬운요약.html` 동봉(이게 뭔지·뭘 사는지·주의점·다음 단계·비법정 고지)
- **D 전문가 심화(기존 큐)**: HLR GA 편입 · mate 선언 · 조립도 판독 트랙
