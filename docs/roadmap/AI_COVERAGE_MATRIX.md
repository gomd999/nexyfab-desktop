...
| 열 | A | 부분 | 불가 | A 비율 |
|---|---|---|---|---|
| **AI 계획 가능?** | ①②④⑤⑦⑧ = **6** | ③⑥ = 2 | (없음) = 0 | **6/8 A · 8/8 ≥부분** *(260722 WB-7: ⑥ 부분승격, 불가 소멸)* |
| **게이트 검증 가능?** | ①②④⑤⑦⑧ = **6** | ③⑥ = 2 | (없음) = 0 | **6/8 A = 75% · 8/8 ≥부분** *(WB-1/4/2/3/8/7 승격, 불가 0)* |
| **zero-touch 실측치** | 0 | 0 | 0 | **n=0 (전 카테고리 미측정)** |

> **대체 수준 판정 (260722 WB-7 갱신)**: 현행 드라이버는 **각기둥형 기계부품
> (브래킷·플레이트)·회전체(축/샤프트)·판금 전개품·용접 프레임(컷리스트)·표준 체결
> (ISO 나사)·그 mate 어셈블리(정밀 간섭 포함)**를 A로, **하우징(곡면 쉘/필렛)·기어
> (인벌류트 치형)를 부분**으로 실행 검증한다 — **8/8 전 카테고리가 최소 '부분',
> 그 중 6/8이 완전 A(검증 A 12.5%→75%). '불가' 카테고리는 소멸.** 남은 '부분'
> 경계는 ③ OCCT 곡면(WB-6)·⑥ 인벌류트 치형(별도 대공사). 자동화율(zero-touch)은
