...
| 열 | A | 부분 | 불가 | A 비율 |
|---|---|---|---|---|
| **AI 계획 가능?** | ①②④⑤⑧ = **5** | ③ = 1 | ⑥⑦ = 2 | **5/8 = 62.5%** *(260722 WB-3: ⑤승격)* |
| **게이트 검증 가능?** | ①②④⑤⑧ = **5** | ③ = 1 | ⑥⑦ = 2 | **5/8 = 62.5%** *(WB-1 ②·WB-4 ⑧·WB-2 ④·WB-3 ⑤ 승격, 12.5%→62.5%)* |
| **zero-touch 실측치** | 0 | 0 | 0 | **n=0 (전 카테고리 미측정)** |

> **대체 수준 판정 (260722 WB-3 갱신)**: 현행 드라이버는 **각기둥형 기계부품
> (브래킷·플레이트)·회전체(축/샤프트)·판금 전개품·용접 프레임(컷리스트)·그 mate
> 어셈블리(정밀 간섭 포함)**에 대해 계획→실측 게이트→패키지 전 경로가 실행으로
> 검증된다(검증 A 12.5%→**62.5%**). **하우징은 부분(단순형상 한정), 기어·체결은
