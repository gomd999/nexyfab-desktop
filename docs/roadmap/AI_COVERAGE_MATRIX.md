...
| **⑧ 복합 어셈블리** | **A** — 2부품+mate(concentric/coincident)+BOM. `pinBlockAssemblyPlan()` 실행 → 패키지. llmPlanner가 assembly 스펙(solveMates 입력형) 코어스 통과 | **A** *(260722 WB-4 승격)* — mate 수렴 A + **간섭 게이트 추가**(비메이트 부품 관통=fail, 메이트 쌍=화이트리스트). 비메이트 겹침 관통 10mm 주입→패키지 미산출 실증. *(잔여: AABB 보수 근사 — 정밀 BRep 교차는 후속)* | **미측정 (n=0)** |

### A 비율 산술 (실행 근거 기반)

| 열 | A | 부분 | 불가 | A 비율 |
|---|---|---|---|---|
| **AI 계획 가능?** | ①②⑧ = **3** | ③ = 1 | ④⑤⑥⑦ = 4 | **3/8 = 37.5%** *(260722 WB-1: ②승격)* |
| **게이트 검증 가능?** | ①②⑧ = **3** | ③ = 1 | ④⑤⑥⑦ = 4 | **3/8 = 37.5%** *(WB-1 ②·WB-4 ⑧ 승격, 12.5%→37.5%)* |
| **zero-touch 실측치** | 0 | 0 | 0 | **n=0 (전 카테고리 미측정)** |

> **대체 수준 판정 (260722 WB-1/WB-4 갱신)**: 현행 드라이버는 **각기둥형 기계부품
> (브래킷·플레이트)·회전체(축/샤프트)·그 mate 어셈블리(간섭 검사 포함)**에 대해
> 계획→실측 게이트→패키지 전 경로가 실행으로 검증된다(검증 A 12.5%→**37.5%**).
> **하우징은 부분(단순형상 한정), 판금·웰드먼트·기어·체결은 계획 어휘 자체가 부재.**
> 자동화율(zero-touch)은 **전 카테고리 n=0**이므로 "대체 수준" 정량 판정은 **GA3
> 첫 실측 전까지 유보**한다(측정 없는 목표는 날조 — 불변 원칙).
...
| ~~⑧ 간섭 게이트 미포함~~ **✅ WB-4 완료(260722)** — 간섭 게이트 표준 승격 | ~~WB-4~~ (정밀 BRep 교차는 잔여) |
