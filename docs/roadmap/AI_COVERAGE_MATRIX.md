...
이어서 도면 치수 병목까지 해소한 뒤 3차 측정:

| 측정 | 구멍 편입 전 | WB-9 후 | **뷰 좌표계 수정 후** |
|---|---|---|---|
| 전 게이트 완주 | 0/12 | 0/12 | **8/12** → **9/12**(사각 컷아웃) → **10/12 = 83.3%**(대각·블라인드·일람표) |
| 계획 단계 통과 | 10/12 | 9/12 | **12/12** |
| `measured 0 mm` | — | 지배적 | **0건** |
| `holes` 실사용 | (표현 불가) | 4/12 | **7/12** |

도면 치수 실패의 원인은 값 오류가 아니라 **뷰 좌표계 오해**였다(front는 Y를 투영으로
날리는데 모델이 거기서 loop의 Y를 재려 했고, 두께를 두 수직 엣지 사이로 재려 했다 —
둘 다 그 뷰에서 분리량이 구조적으로 0). 프롬프트에 뷰별 가시 축을 명시하고 프리플라이트에
"그 뷰에서 잴 수 없는 선형 치수"를 신설해 해소.

이어서 **사각 컷아웃**(`HoleSpec.shape:'rect'`)을 편입해 마지막 엔진 갭을 닫았다 —
공구 루프만 정n각형→사각형이고 커널 경로는 그대로다. 사각 공구는 테셀레이션이 없어
**정확**(50×50×120 − 42×42×120 = 88320.000 mm³ 실측 일치). 각파이프·창 컷아웃이 열렸다.

**최종 남은 3건은 엔진 갭이 아니라 모델의 기하 추론 오류**이고 게이트가 제 일을 한 것이다:
실측 부피 vs 모델이 스스로 적은 기대 부피의 불일치 2건(모델이 그린 루프가 의도한 형상이
아니라는 뜻 — 여기서 expectedVolume을 생략하라고 가르치면 그 불일치가 숨는다) · 빗변을
`linear`로 재려 한 대각 치수 1건.
상세·다음 행동은 `docs/next-session-plan-260727.md` §6.
