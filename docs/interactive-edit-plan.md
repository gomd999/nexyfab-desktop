...
## 개선 1~7 일괄(완료 260719 — 사용자 승인 전체 실행)
#1 언두(스튜디오 스택≤10·챗 푸시풀 ↩≤5) · #2 치수 직접 입력(face-drag targetMm — faceDimOf) ·
#3 회전 배치 면 명명·푸시풀(일반 회전행렬 R/Rᵀ — 벽 rz90 실증, 사면=정직 거부 유지) ·
#4 드래그 5mm 스냅 · #5 다중 선택(Ctrl+클릭)+복제/삭제(part-op 결정론, 그룹 이동=API 有·UI 후속) ·
#6 수정 후 재빌드가 도메인 게이트 자동 재실행(간섭·지지·구조 — 계산기 카드는 기존 ↻) ·
#7 부품 필렛(part.filletMm→STEP B-rep 반영·표시 뷰어=무필렛 명시·실패=드롭 보고)+교체 드롭다운(edit-part 지시 자동).
E2E: 스튜디오=픽→edit→치수200 적용→복제(5→6)→언두, 챗=면 드래그 radial. STEP 필렛 CLI 반영 확인.
