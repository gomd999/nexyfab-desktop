...
필요) 1 · **사각 컷아웃 미지원 1**(각파이프 = 사각 단면 파내기, 현재 구멍은 원형만).

## 6. 다음 작업 (우선순위 순)

0. **★사각 컷아웃** — 12브리프 중 유일하게 남은 *진짜 엔진 갭*. `holeGate`의 공구는 이미
   임의 루프의 extrude이므로 원형 n각형 대신 사각 루프를 넣으면 되는 정도(면적도 정확 —
   테셀레이션 근사가 없다). 각파이프·창 컷아웃이 한 번에 열린다.
1. ~~**★도면 치수 병목**~~ → §5-1에서 해소(0/12→8/12). 잔여는 아래 세부만: — 이제 단일 최대 병목. `d_thickness`(front 뷰)가 `measured 0 mm`로
