...
**260808 원격 조사 실측(🤖 — 자동 취득 불가 확인)**:
- CALSPIA 자료실: 다운로드가 JS 팝업(`sub_pop()`) 뒤라 직접 URL 없음, 페이지에
  공공누리 표기 **없음**(푸터 "All Rights Reserved") → 이 페이지만으로는 권리 확정 불가.
- 국가기록원 기록물(도로옹벽표준도): **공공누리 배너 존재하나 유형 미상**,
  공개구분 "부분공개", 원문은 웹뷰어(archWebViewer) 전용 — 다운로드 URL 없음.
- 👤 실행 경로: ①국가기록원 원문보기 접속→공공누리 **유형 번호** 확인(제1유형이면
  변경금지·상업이용 조건 확인) ②또는 국토교통부/국가기록원에 벤치마킹 목적 이용
  서면 문의 ③또는 licensed-customer/expert-authored 경로(실무 도면 제공자와 계약).
