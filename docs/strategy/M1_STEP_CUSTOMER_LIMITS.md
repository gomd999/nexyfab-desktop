...
| 박스 보내기 (AP214 B-rep) | OCCT 라운드트립 검증됨 | `BoxGeometry`만 B-rep, 나머지는 AP242 테셀레이션 |
| 일반 메시 보내기 (AP242) | 메시 → STEP 테셀레이션 | B-rep 재구성 불가, 수입 측이 메시로 받음 |
| 어셈블리 STEP 보내기 (멀티바디 단일 파일) | **미지원** | 파트별 개별 STEP로 분리 다운로드 권장 |
| 비-박스 solid B-rep 보내기 | **미지원** | 메시 경로(AP242)만; B-rep export는 후속 로드맵 |
...
| 증상 | 의심 원인 | 권장 조치 |
|---|---|---|
| 비-박스 객체가 STEP에 안 보임 | AP214 B-rep는 박스만 | AP242 테셀레이션 경로로 보내면 메시 형태로 export |
...
- **비-박스 solid B-rep export** — OCCT writer 확장 (M1_EXCHANGE.md §보내기 강화)
- **어셈블리 단일 STEP** — 멀티바디 묶음 출력 (Phase C)
- **AP242 BIM 풀 호환** — entity 화이트리스트 확장 (Phase D)
- **STEP 라운드트립 가시화** — import → re-export → diff 리포트 (Phase C)
