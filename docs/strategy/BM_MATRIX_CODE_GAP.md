...
**코드 정렬 점검:** 2026-05-11 — `stage-overview` 데모 sentinel 제외, `stage-worker` CRON 헤더, [bm-cron-runbook.md](../bm-cron-runbook.md) 추가. **Phase 3 문서:** [stage-e-business-account-policy.md](./stage-e-business-account-policy.md), [bm-logging-coverage.md](./bm-logging-coverage.md), [bm-ui-screen-map.md](./bm-ui-screen-map.md).
...
|----|------------------|------------|------|
| G-S1 | Stage **E**: 누적 ≥ 1억 + (법인 **또는** 팀원 ≥2) | `computeStage`·`evaluateStage`·`nf_stage_event.trigger_value` 모두 `isBusinessAccount` 동기화; DB 컬럼은 `nf_users.account_type`·`business_reg_number` | **부분** — 운영 경계: [stage-e-business-account-policy.md](./stage-e-business-account-policy.md) |
...
| ID | 스펙 요지 | 코드 현황 | 상태 |
|----|-----------|------------|------|
| G-U1 | Stage별 UI 매트릭스 | §1.2 표 **32개 기능 번호**는 `bm-matrix-stage-ui.ts`와 lockstep(Vitest 고정). `mergePlanLimitsWithBmStage`·`useFreemiumGate`·`nexyfabStage`로 게이트. 화면 매핑 롤링: [bm-ui-screen-map.md](./bm-ui-screen-map.md) | **부분** (매트릭스·게이트 구현됨; 행별 QA·카피는 롤링) |
...
| G-M2 | §4.4 로깅 의무 (결함·환불·CAD 접근 등) | 코드 스냅샷: [bm-logging-coverage.md](./bm-logging-coverage.md) — 번들 INSERT·환불 테이블 명칭 등 남음 | 롤링 |
...
| 날짜 | 내용 |
|------|------|
| 2026-05-11 | Phase 3 — G-S1/G-M2/G-U1 보조 문서 3종(`stage-e-business-account-policy`, `bm-logging-coverage`, `bm-ui-screen-map`); 갭 표 G-S1·G-U1·G-M2 링크 |
| 2026-05-11 | 코드 정렬 점검 주석; G-M1 운영 runbook(`bm-cron-runbook.md`)·`stage-worker` cron 헤더·`stage-overview` 데모 제외 |
