...
export default {
  ...
  run(input, std) {
    ...
    // 처짐 (E는 CD 미적용 — 표 3.1-7 주1)
    const wNmm = w; // kN/m = N/mm
    const delta = (5 * wNmm * Math.pow(L, 4)) / (384 * g.E * I) + (P * 1e3 * Math.pow(L, 3)) / (48 * g.E * I); // mm
    ...
    return {
      ...
      checks,
      intermediate: {
        speciesLabel: sp.label, grade, CD, base: { Fb: g.Fb, Fv: g.Fv, E: g.E },
        S_mm3: Math.round(S), I_mm4: Math.round(I), M_kNm: +M.toFixed(2), V_kN: +V.toFixed(2),
      },
      notes: [
        `기준허용응력 ${sp.label} ${grade}등급 (표 3.1-4) × CD ${CD} (표 3.1-7, ${input.duration ?? 'tenYears'})`,
        'v1 보정: CD만 — 습윤 CM·온도 Ct·치수 CF·좌굴안정 CL 미적용(건조사용·상시온도·횡지지 가정). 옥외 상시습윤은 CM 적용 시 더 불리.',
