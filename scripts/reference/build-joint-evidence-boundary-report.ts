...
const cases = requests.requests.map(request => {
  ...
  const verdict = evaluateJointEvidenceRelease({ provenance: 'native-cad', sourceHash: result.sourceHash, artifactHashes: [result.artifactHash], jointCount: result.joints.length, semanticsComplete: result.jointSemanticsComplete, reviewerApproved: false });
