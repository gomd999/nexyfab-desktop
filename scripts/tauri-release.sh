...
# parent .env 로드 — TAURI_SIGNING_PRIVATE_KEY 등을 한 곳에 모음.
# 이미 shell 에 export 된 값은 우선 (OS/CI env 덮어쓰지 않음).
PARENT_ENV="$(cd "$ROOT/../.." && pwd)/.env"
...
cmd_gen_keys() {
  ...
  # tauri.conf.json 의 updater.pubkey 자동 교체
  ...
  if [[ -f "$conf" ]]; then
    node -e "
      ...
      console.log('  → tauri.conf.json.updater.pubkey 업데이트됨');
    "
  fi
  ...
  echo "  tauri.conf.json: 자동 패치됨 (수동 편집 불필요)"
  echo ""
...
cmd_env() {
  ...
  if [[ $found -gt 0 ]]; then
    ...
    echo "  nexyfab.com/releases/$VERSION/NexyFab_${VERSION}_x64_ko-KR.msi"
