...
test('keeps immutable migrations ordered before the remote CAD agent state migration', () => {
  ...
  assert.deepEqual(migrations.map(item => item.version), [2026082001, 2026082002, 2026082101, 2026082102, 2026082201, 2026082202, 2026082203, 2026082204, 2026082205, 2026082206, 2026082207, 2026082208, 2026082301, 2026082401, 2026082402, 2026082403]);
...
test('2301 is an append-only mapping migration and does not rewrite prior SQL', () => {
  ...
  assert.match(migrations.at(-4).sqlPath, /db-postgres-migration-2026082301\.sql$/);
  assert.notEqual(migrations.at(-4).sqlPath, migrations.at(-5).sqlPath);
});

test('2401 adds the authority-owned Canonical CAD V2 journal without rewriting prior SQL', () => {
  const migrations = orderedMigrationInputs();
  const sql = readFileSync(migrations.at(-3).sqlPath, 'utf8');
  assert.match(migrations.at(-3).sqlPath, /db-postgres-migration-2026082401\.sql$/);
...
test('2402 adds durable AI Design V10 CAS heads and immutable artifacts', () => {
  ...
  assert.match(migrations.at(-2).sqlPath, /db-postgres-migration-2026082402\.sql$/);
...
test('2403 adds a leased AI-to-Precision outbox and immutable receipt ledger', () => {
  const migration = orderedMigrationInputs().at(-1);
