...
describe('★ 템플릿이 판정을 갖는가 (260801b)', () => {
   ...
   */
  /**
   * ⚠ 260802 — **이 목록이 비었다.** 4건 전부 해소됐다:
   *   · `civil/*` 3건 → `runDomainSafetyCheck` 에 **`civil` 분기가 아예 없었다.**
   *     계산기 검증은 도달하고 있었으므로 「검토가 없다」가 아니라 「안전검토가 없다」였다.
   *     형상 비례 검토(`civil-check.mjs`)를 붙여 판정 8건이 생겼다.
   *     ⚠ 전도·활동·지지력은 **계산기가 이미 한다** — 중복하지 않았다.
   *   · `mech/flanged_fitting` → 「형상 검토 대상이 아니다」고 적어 뒀는데 **틀렸다.**
   *     플랜지 두 장의 `bcd`·`boltCount`·`boltHoleD` 일치와 보어↔관 외경은 **하중 없이도**
   *     판정된다. 안 맞으면 실제로 조립이 안 된다.
   *
   * 목록을 비운 채로 둔다 — 다시 채워야 할 일이 생기면 **이유를 반드시 적는다.**
   * 이 검사의 요지는 「면제에는 근거가 붙어야 한다」이지 「면제가 있어야 한다」가 아니다.
   */
  /**
   * ⚠ 260803 — 여기에 코리더를 넣을 뻔했다가 **넣지 않기로 했다.**
   * 도로 기하구조 판정은 KDS 도로설계기준 영역이라 지금 카탈로그 계약(전 종이 소비자에게
   * 판정을 전달한다)을 못 지킨다. 면제를 선언해 카탈로그에 남기는 대신 **카탈로그에서 빼고
   * MCP 도구(`build_corridor`)로 노출**했다 — 「닿지 않는 코드」로 두지 않으면서 계약도 안 어긴다.
   * > 계약을 못 지키는 것을 면제로 통과시키는 것보다, 그 계약이 없는 자리에 두는 것이 낫다.
   */
