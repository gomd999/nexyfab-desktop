...
describe('해당 없음과 판정 불가를 구별한다', () => {
  ...
  it('기둥이 있으면 벽식으로 넘겨 침묵시키지 않는다 — 사유를 남긴다', () => {
    ...
    // ⚠ 260730 갱신: steel_canopy 는 이제 **캐노피 검토**로 간다(canopyMeta 디스패치).
    //   「하중경로 대상 아님」을 남기는 것에서 나아가 **그 대신 볼 것**이 생겼다 —
    //   캐노피를 지배하는 것은 중력이 아니라 풍 상향력이다. 원래 의도(침묵시키지 않는다)는
    //   더 강하게 지켜진다: 사유만 남기는 것이 아니라 실제 검토가 붙었다.
