 ...
 *
 * ## ⚠⚠ 그리고 그것만으로는 부족했다 (260803 정정)
 * 이 인코더는 맞았지만 **넣는 위치가 틀렸다.** 결과를 OCCT 라이터에 넘기면 라이터가
 * 우리 역슬래시를 규격대로 다시 이중화해 파일에는 `\\X2\\…\\X0\\` 가 쓰였고,
 * 뷰어는 그것을 **리터럴 텍스트**로 읽는다 — 한글이 안 나온다.
 * 당시 확인은 「파일에 비ASCII 0」뿐이었고 **디코드해 보지 않았다.**
 * → **바이트 검사는 인코딩 검사가 아니다.** 왕복 검증은 `step-assembly-tree.test.ts` 가 한다.
 *   지금 `stepSafeName` 은 방출 경로가 아니라 `escapeStepNonAscii` 의 공용 코어로 쓰인다.
 */
