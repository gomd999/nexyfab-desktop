...
type StepResult = { step: string; importNotes: string[]; tree: { nauo: number; products: number; groups: string[] } | null };
...
describe('★① 조립 트리(NAUO)', () => {
  it('NAUO 가 부품 수만큼 있다 — 0 이면 트리가 아니라 평면 나열이다', async () => {
    ...
    // root + 계통 + 부품 = PRODUCT, 그중 root 를 뺀 수만큼 NAUO 가 생긴다.
    expect(r.tree!.nauo).toBe(r.tree!.products - 1);
  });
  ...
  it('솔리드는 하나도 잃지 않았다 — 트리를 얻으려고 형상을 잃지 않는다', async () => {
    const r = await stepOnce();
    expect(count(r.step, /MANIFOLD_SOLID_BREP/g)).toBe(25);
  });
