...
export function supportCheck(items, { tol = 8, minBear = 15, fastenBear = 40, embedMm = 2 } = {}) {
  ...
  const bearsOn = (a, b) => {
    ...
    // mech 조인트 선언 체결(260718c — 로봇·기구류): role {joint,link,gripper,gear,shaft} 쌍이
    // 근접(전 축 갭 ≤40mm — 풀백/보어 구간은 축·베어링·키가 채움 관례 명시·한 축 이상
    // 실겹침 ≥minBear)하면 볼팅/키/감속기 체결로 연결 — 선언 역할 한정.
    ...
    /**
     * 관통 체결구(260803 — 힌지 핀·마찰 와셔·리벳): `role:'fastener'` 로 선언된 부품이
     * 지지된 부품과 **세 축 모두 실겹침**(=물리적 관통/삽입)이면 체결로 인정한다.
     *
     * ⚠ 왜 필요한가 — **자기 지름이 `minBear`(15mm)보다 작은 횡방향 하드웨어는 AABB
     *   겹침으로 영원히 확인되지 않는다.** 데스크 거치대 힌지 핀은 ⌀8, 마찰 와셔는 t1 이라
     *   `ox`·`oz` 가 8 을 넘을 수 없다(자기 지름이 상한이다). 기존 `four_bar` 의 핀이
     *   통과하는 이유는 **수직 ⌀16** 이라 겹침이 16 이기 때문이고, 그건 우연히 임계를
     *   넘은 것이지 규칙이 횡방향 소경 핀을 다룰 수 있다는 뜻이 아니었다.
     * ⚠ 부유로 두면 **설계자가 고칠 수 없는 오탐**이 된다 — ⌀8 핀을 ⌀16 으로 키우는 것은
     *   검사기를 만족시키려고 제품을 바꾸는 것이다. 그건 게이트가 설계를 지배하는 것이다.
     * ⚠ 그래서 조건을 **관통**으로 못 박는다: 단순 근접(갭 허용)이 아니라 세 축이 전부
     *   `>0` 이어야 한다. 공중에 뜬 볼트는 아무것도 관통하지 않으므로 여전히 부유다.
     * ⚠ 다른 예외들과 같이 **선언 기반**이다 — `role` 을 붙이지 않은 부품에는 적용되지 않는다.
     */
    if ((a.role === 'fastener' || b.role === 'fastener') && ox > 0 && oy > 0 && oz > 0) return true;
    return false;
  };
  let moved = true;
  while (moved) {
