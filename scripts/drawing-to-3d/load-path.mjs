...
export function loadPathCheck(assembly, params = {}) {
  ...
  if (xs.length < 2 || ys.length < 2) {
    ...
    // ⚠ 260729 P1-6: `notApplicable` 을 **달지 않아서** 디스패치가 벽식 검토로 넘기지
    //   못했다. 문장은 "벽·일방향 골조는 별도 검토가 필요하다"고 정확히 적고 있는데
    //   정작 그 검토를 부르지 않는 상태였다 — 있는 검사의 배선이 끊긴 자리(형태 ①).
    return {
      ok: false, notApplicable: true, scope: 'grid',
