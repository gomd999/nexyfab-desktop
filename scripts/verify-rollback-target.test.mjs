...
const commercialMigrations = [
  ...
  2026082208, 2026082301, 2026082401, 2026082402, 2026082403,
];
...
const healthy = {
  ...
  release: { migrationVersion: 2026082403, migrationChecksums: Object.fromEntries(commercialMigrations.map(version => [version, 'a'.repeat(64)])), registryRoles: 3, registryFingerprintsUnique: true, i18n: { status: 'QUALIFIED', sourcePairs: 2711, translatedPairs: 2711 }, sevenDay: { status: 'QUALIFIED' } },
};
...
test('rollback verifier requires the exact append-only commercial migration set when release evidence is supplied', () => {
  ...
  assert.ok(issues.some(issue => issue.includes('2026082403/checksum')));
});
