...
import { buildComplexBenchmarkReportV2, DEFAULT_COMPLEX_BENCHMARK_POLICY_V2, type ComplexBenchmarkCaseV2 } from '../src/lib/ai/complexProductBenchmarkV2';
...
async function main() {
  ...
  if (!casesPath || !statePath || !executorPath) throw new Error('Usage: --cases=cases.json --state=campaign-state.json --executor=executor.mjs [--families=gearbox,machine_skid] [--resume]');
  ...
  const requiredFamilies = value('families')?.split(',').map(item => item.trim()).filter(Boolean) as ComplexBenchmarkFamily[] | undefined;
  const state = await runComplexCampaign(cases, executor, initial, { requiredFamilies, onCheckpoint: checkpoint => atomicJson(resolvedState, checkpoint) });
  await atomicJson(resolvedState, state);
  const report = buildComplexBenchmarkReportV2(cases, state.results, { ...DEFAULT_COMPLEX_BENCHMARK_POLICY_V2, requiredFamilies: state.requiredFamilies }); await atomicJson(`${resolvedState}.report.json`, report);
